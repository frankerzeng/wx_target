define(function(require){
    return {
        ...
        ubb2html : function (sUBB)
        {
            ...
            sHtml=sHtml.replace(/\[url\s*=\s*([^\]"]+?)(?:\,([\w]*))?(?:"[^\]]*?)?\s*\]\s*([\s\S]*?)\s*\[\/url\]/ig, function(all,url,target,text){
        ...
        html2ubb : function (sHtml)
        {
            ...
            sUBB=sUBB.replace(/<a([^>]*)>\s*([\s\S]*?)\s*<\/a>/ig,function(all, attr, text){
                var url=attr.match(/href=(["'])\s*(.+?)\s*\1/i);
                var target=attr.match(/target=(["'])\s*(.+?)\s*\1/i);
                var tag = 'url';
                url = url && url[2];
                ...
                return '['+tag+'='+url+target+']'+text+'[/'+tag+']';
            });
