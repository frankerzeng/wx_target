/**
 * 简单的UI组件，参考的bootstrap
 */
 
define(function (require) {
    "use strict";
    var now = require('now');

    !function ($) {
        /**
         * 清空资源
         * @author 欧远宁
         */
        $.fn.dispose = function () {
            return this.each(function () {
                $('[_nui_dispose]', $(this)).each(function () {
                    var el = $(this),
                        ui = el.attr('_nui_dispose'),
                        cls = el.data(ui);
                    if (cls && el[cls]) {
                        el[cls]('dispose');
                    }
                });
            });
        };
        
        /**
         * 前端转义
         * @author 曾林淘
         */
        $.getStrText = function (str) {
            var html = $('#_nui_html_filter').text(str).html();
            $('#_nui_html_filter').html('');
            return $.trim(html);
        };
        
        $.getStrHtml = function (str) {
            var html = $('#_nui_html_filter').html(str).text();
            $('#_nui_html_filter').html('');
            return $.trim(html);
        };

        /**
         * 对一个select进行数据填充
         * @author 欧远宁
         * @param object data  需要显示的数据内容，{k=v}的形式,如果k包含cmd，则使用后台请求
         * @param string def   默认值
         * @param string all   如果有全部选择的话，其文字是什么。为空则没有
         * @param array  rmKey 需要移除的键值
         */
        $.fn.sltFill = function (data, def, all, rmKey) {
            def = def === undefined ? '':def;
            return this.each(function () {
                var html = '',
                     ck = '',
                     self = $(this);

                if (def === undefined) {
                    def = '';
                }
                if (all) {
                    html = '<option value="">' + all + '</option>';
                }

                var fill = function (json) {
                    $.each(json, function (key, val) {
                        if($.inArray(key, rmKey) === -1){
                            ck = (key === def) ? ' selected="selected"' : '';
                            html += '<option value="' + key + '"' + ck + '>' + val + '</option>';
                        }
                    });

                    self.html(html);
                };

                if (data.cmd !== undefined) {
                    now.cmd(data.cmd, data.param, function (ret) {
                        fill(ret.rows);
                        if (data.okfun !== undefined) {
                            data.okfun(ret);
                        }
                    });
                } else {
                    fill(data);
                }
            });
        };

        /**
         * 得到一个模版结果
         * @author 欧远宁
         * @param string tpl   模版url
         * @param object para  后端请求的参数
         * @param function fun 完成后的回调
         * @param boolean sync 同步还是异步请求
         */
        $.fn.tpl = function (tpl, para, fun, sync) {
            sync = sync ? true : false;
            return this.each(function () {
                var _me = $(this);
                if ($.isFunction(para)) {
                    fun = para;
                    para = {};
                }
                var _cb = function (data) {
                    now.tpl(tpl, data, function (html) {
                        _me.html(html);
                        now.removeFun();//移除没有权限的功能点
                        if ($.isFunction(fun)) {
                            fun(data);
                        }
                    }, sync);
                };
                if (para._c) {
                    var cmd = para._c;
                    delete para._c;
                    now.cmd(cmd, para, function (ret) {
                        _cb($.extend(para, ret));
                    });
                } else {
                    _cb(para);
                }
            });
        };

        /**
         * 一个简单的弹窗，封装了modal
         * title：标题
         * msg：提示语
         * btn：[
         *            {text:'确定',okfun:function(){},primary:true},
         *            {text:'取消'}
         *        ]
         */
        $.dialog = function (title, msg, btn, notClose) {
            var el = $('#_nui_dialog'),
                 footer = '',
                 primary = '';

            if (notClose) {
                $('button.close', el).hide();
            } else {
                $('button.close', el).show();
            }
            $('#_nui_dialog').modal({backdrop: 'static'}).modal('show');
            $('h3', el).html(title);
            $('.modal-body', el).html(msg);
            btn = typeof btn === 'undefined' ? [] : btn;

            $.each(btn, function (i, e) {
                primary = '';
                if (e.primary) {
                    primary = 'btn-primary';
                }
                footer += '<button class="btn ' + primary + '" name="' + e.text + '">' + e.text + '</button>';
            });

            $('.modal-footer', el).html(footer);

            $.each(btn, function (i, e) {
                $('.modal-footer', el).find('button[name=' + e.text + ']').on('click', function () {
                    if(!e.nohide)el.modal('hide');
                    if (typeof e.okfun === "function" ) {
                        e.okfun();
                    }
                });
            });
        };

        /**
         * 简化$.dialog()的确认框
         */
        $.confirm = function (msg, okfun, title, btn, cancelfun, notClose) {
            title = title || '请确定';
            btn = btn || '确定';
            cancelfun = cancelfun || null;
            $.dialog(title, msg, [
                {text: btn, primary: true, okfun: okfun},
                {text: '取消',primary: false, okfun:cancelfun}
            ], notClose);
        };

        var _int = -1;
        var _alertInt = -1;

        /**
         * 弹出错误警告信息
         */
        $.alertMsg = function (title, msg, close) {
            var el = $('#_nui_alertmsg');
            if (el.is(':visible')) {
                if (_alertInt > -1) {
                    window.clearTimeout(_alertInt);
                }
                el.hide();
                $('#_nui_mask').hide();
            }

            if (typeof(msg) === 'undefined') {
                msg = '';
            }

            $('h4', el).html(title);
            $('span', el).html(msg);
            $('#_nui_mask').show();

            var $b = $(document),
                 $w = $(window),
                 sh = $b.scrollTop(),
                 sw = $b.scrollLeft(),
                 wh = $w.height(),
                 ww = $w.width();
            el.css({
                top: sh + wh / 2,
                left: sw + ww / 2,
                position: "absolute"
            }).show();

            if (!close) {
                //自动关闭提示窗口
                _alertInt = setTimeout(function () {
                    $('#_nui_alertmsg .close').click();
                }, 3000);
            }
        };

        /**
         * 显示操作成功信息
         */
        $.actMsg = function (title, msg) {
            var el = $('#_nui_actmsg');
            if (_int > -1) {
                window.clearTimeout(_int);
                el.hide();
                $('#_nui_mask').hide();
            }


            $('h4', el).html(title);
            $('span', el).html(msg);
            el.show();
            _int = window.setTimeout(function () {
                el.fadeOut(500);
                _int = -1;
            }, 3000);
        };

        var _msg = '<div id="_nui_mask" class="nui-mask hide"></div>';
        $('body').append(_msg);

        var _alert = '<div id="_nui_alertmsg" class="nui-alertmsg alert alert-block hide">'+
             '<button type="button" class="close">&times;</button>'+
             '<h4></h4><span></span></div>';
        $('body').append(_alert);

        var _act = '<div id="_nui_actmsg" class="nui-actmsg alert alert-block alert-success hide">'+
             '<button type="button" class="close">&times;</button>'+
             '<h4></h4><span></span></div>';
        $('body').append(_act);

        var _dialog = '<div class="modal hide fade" id="_nui_dialog">'+
             '<div class="modal-header">'+
             '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>'+
             '<h3></h3>'+
             '</div>';
        _dialog += '<div class="modal-body"></div>';
        _dialog += '<div class="modal-footer"></div>'+
                '</div>';
        $('body').append(_dialog);
        
        var _filter = '<span id="_nui_html_filter" style="display: none"></span>';
        $('body').append(_filter);

        $('.close', $('#_nui_alertmsg')).on('click', function (e) {
            if (_alertInt > 0) {
                window.clearTimeout(_alertInt);
            }
            $('#_nui_alertmsg').fadeOut(500);
            $('#_nui_mask').hide();
        });

        $('.close', $('#_nui_actmsg')).on('click', function (e) {
            if (_int > 0) {
                window.clearTimeout(_int);
            }
            $('#_nui_actmsg').fadeOut(500);
        });

        $('.close', $('#_nui_dialog')).on('click', function (e) {
            $('#_nui_dialog').modal('hide');
        });
    }(jQuery);

    /**
     * 简单的聊天实现，支持1对1和群聊
     */
    !function ($) {
        var nowChat = null;

        $._chatReady = function () {
            nowChat._initOk();
        };
        $._chatLoginOk = function () {
            nowChat._loginOk();
        };
        $._chatClose = function () {
            nowChat._chatClose();
        };
        $._chatErr = function (code, msg) {
            nowChat._chatErr(code, msg);
        };
        $._chatData = function (data) {
            nowChat._chatData(data);
        };

        var Chat = function (element, options) {
            if (nowChat) {
                alert('同时只能存在一个聊天实例');
            }
            this.options = options;
            this.$element = $(element);
            this.$element.addClass(options.css);
            this.swf = null;
            nowChat = this;
            this.intterval = 0;
            this._init();
        };
        Chat.prototype = {
            constructor: Chat, _init: function () {
                var el = this.$element;
                el.flash({
                    width: 1,
                    height: 1,
                    src: now.cfg.nowjs + 'chat.swf'
                });
            }, _initOk: function () {
                var self = this,
                     el = this.$element,
                     op = this.options;

                el.flash('getSwf', function (swf) {
                    self.swf = swf;
                    swf.initChat(op.host, op.port, op.uid, op.name, op.rand, op.code, op.group);
                });

            }, _loginOk: function () {
                var e = $.Event('chatLoginOk');
                this.$element.trigger(e);

                //发送心跳包
                this.intterval = setInterval(function () {
                    this.swf.sendPing();
                }, 2000);
            }, _chatErr: function (code, msg) {
                var e = $.Event('chatErr');
                this.$element.trigger(e, code, msg);
            }, _chatClose: function () {
                var e = $.Event('chatClose');
                this.$element.trigger(e);
            }, _chatData: function (data) {
                var e = $.Event('chatData');
                this.$element.trigger(e, data);
            }, sendMsg: function (para) {
                this.swf.sendMsg(para.to, para.msg);
            }, dispose: function () {
                this.$element.html('');
                clearInterval(this.intterval);
                if (window.nowchat!==undefined) {
                    window.nowchat = null;
                }
            }
        };
        $.fn.chat = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('chat'),
                     os = (typeof option === 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.chat.defaults, typeof option === 'object' && option);
                    $this.data('chat', (data = new Chat(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, typeof option === 'object' && option);
                }

                if (os) {
                    data[option](para);
                }
            });
        };

        $.fn.chat.defaults = {
            host: '127.0.0.1',
            port: 2008,
            uid: '',
            name: '',
            group: '',
            rand: '',
            code: '',
            css: 'nui-chat'
        };

    }(jQuery);

    /**
     * 表单处理
     */
    !function ($) {
        var Frm = function (element, options) {
            this.options = options;
            this.$element = $(element);
            this.$element.addClass(options.css);
            var self = this;
            $(options.submit).on('click', function () {
                $(this).attr('disabled', 'disabled');
                self.post();
            });
            $(options.submit).on('keydown', function (e) {
                if (e.keyCode === 13) {
                    return false;
                }
            });
        };

        Frm.prototype = {
            constructor: Frm, val: function (para) {
                para = $.extend({}, this.options.para, para);

                var ok = true,
                     el = this.$element,
                     encrypt_pwd = {};

                el.find(':input').each(function () {
                    var _input = $(this);
                    var _el = $(this),
                         ck = _el.attr('check');

                    if (_el.data('editor')) {
                        _el.editor('getUbb');
                    }
                    var tmp = _el.val();
                    if (tmp == null) {
                        tmp = "";
                    }
                    var val = tmp.replace(/(\s*$)/g, "");

                    if (val!=='' && _el.attr('encrypt') && _el.attr('encrypt')==="true") {
                        encrypt_pwd[_el.attr('name')] = 1;
                    }

                    //如果是drop组件
                    if(_el.hasClass('_drop-value')){
                        _el = _el.parent().parent().parent().parent();
                    }else if(_el.hasClass('_muti-drop-value')){
                        _el = _el.parent().parent().parent();
                    }

                    if (ck) {
                        var arr = ck.split(' '),
                             ret = now.val(arr, val),
                             _ep = _el.parent();
                        if ($('span.help-inline', _ep).length === 0) {
                            if( _el.attr('br') === '' ){
                                _ep.append('<br/><span class="help-inline"></span>');
                            }else{
                                _ep.append('<span class="help-inline"></span>');
                            }
                        }
                        var $msg = $('span.help-inline', _ep);
                        if (ret === '') {
                            _input.removeAttr('err');
                            if(_ep.parents('.control-group').find('[err]').length === 0){
                                _ep.parents('.control-group').removeClass('error').addClass('success');
                            }
                            if(_ep.parents('.form_inline1st').find('[err]').length === 0){
                                _ep.parents('.form_inline1st').removeClass('error').addClass('success');
                            }
                            if (para.ok_flag) {
                                $msg.html('<i class="icon-ok"></i>').show();
                            }
                            if (!para.ok_flag) {
                                $msg.html('');
                            }

                        } else {
                            _ep.parents('.control-group').removeClass('success').addClass('error');
                            _ep.parents('.form_inline1st').removeClass('success').addClass('error');
                            _input.attr('err', '');
                            var title = _el.attr('title');
                            title = title || ret;
                            $msg.html(ret).show();
                            ok = false;
                        }
                    }
                });

                // 对第一个出现错误的控件聚焦
                el.find('[err]').eq(0).focus();

                if (ok || window.uitest) {
                    var formParam = el.find(':input').serializeArray();
                    var _p = {},
                        _val = '';
                    var Md5 = require('md5');
                    $.each(formParam, function () {
                        _val = this.value.replace(/(\s*$)/g, "");
                        if (encrypt_pwd[this.name]!==undefined) {
                            _val = Md5.hex_md5('wvhe2ws' + _val);
                        }
                        _p[this.name] = _p[this.name] === undefined ?_val : _p[this.name] + ',' + $.trim(this.value);
                    });
                    $.each(_p, function (k, v) {
                        para[k] = v;
                    });
                }
                return [ok, para];

            }, fill: function (para) { //进行数据填充
                var el = this.$element,
                     data = para.data,
                     tbl = para.tbl,
                     ubb2 = require('ubb2');
                el.find('[name]').each(function () {
                    var key, val;

                    if (typeof tbl === 'string' && tbl.length > 0) {
                        key = $(this).attr('name').replace(tbl + '[', '').replace(']', '');
                        val = data[key];
                    } else if (typeof tbl === 'object') {
                        key = $(this).attr('name');
                        $.each(tbl, function (k, v) {
                            key = key.replace(v + '[', '').replace(']', '');
                        });
                        val = data[key];
                    } else {
                        key = $(this).attr('name');
                        val = data[key];
                    }
                    if (val === undefined) {
                        return;
                    }

                    if (this.nodeName === 'INPUT' ||
                        this.nodeName === 'SELECT' ||
                        this.nodeName === 'TEXTAREA') {
                        switch (this.type) {
                            case 'hidden':
                            case 'password':
                            case 'text':
                            case 'textarea':
                                if ($(this).hasClass('nui-editor') && typeof(val) === 'string') {
                                    $(this).editor('setData', val);
                                }else if(typeof(val) === 'string'){
                                    val = now.htmlDecode(val);
                                }
                                $(this).val(val);
                                break;
                            case 'select-one':
                                $(this).val(val);
                                break;
                            case 'select-multiple':
                                //不处理
                                break;
                            case 'checkbox'://必须以 , 隔开的数据
                                var new_val = '' + val;
                                if ($.inArray($(this).val(), new_val.split(',')) >= 0) {
                                    $(this).attr('checked', true);
                                }
                                break;
                            case 'radio':
                                if ($(this).val() === (val + '') ) {
                                    $(this).attr('checked', true);
                                }
                                break;
                        }
                    } else if (this.nodeName === 'IMG') {
                        $(this).attr('src', val);
                    } else {
                        //ubb2解码
                        $(this).html(val);
                    }
                });
            }, clear: function () { //进行清理
                var el = this.$element;
                $('.control-group', el).removeClass('error').removeClass('success');
                $('.form_inline1st', el).removeClass('error').removeClass('success');
                $('span.help-inline', el).hide();

                //找到需要清空的其他组件
                el.find('div._clear').each(function () {
                    if ($(this).data('mutidrop')) {
                        $(this).mutidrop('setDef', {});
                    } else if ($(this).data('upload')) {
                        $(this).upload('_clear');
                    } else if ($(this).data('drop')) {
                        $(this).drop('clear');
                    }
                });

                el.find(':input').each(function () {
                    switch (this.type) {
                        case 'hidden':
                        case 'password':
                        case 'text':
                            $(this).val('');
                            break;
                        case 'textarea':
                            $(this).val('');
                            if ($(this).data('editor')) {
                                $(this).editor('setData', '');
                            }
                            break;
                        case 'select-one':
                            var obj = $(this).find("option[selected='selected']");
                            if (obj.length > 0) {//有默认值的显示默认值
                                obj.attr('selected', true);
                            } else {//初始化第一个为选中值
                                $(this).find('option:first').attr('selected', true);
                            }

                            break;
                        case 'checkbox':
                        case 'select-multiple':
                        case 'radio':
                            //全部设置为false
                            $(this).removeAttr('checked');
                    }
                });
            }, clearCheck: function () {//清理检查信息，不清理表单
                var el = this.$element;
                $('.control-group', el).removeClass('error').removeClass('success');
                $('.form_inline1st', el).removeClass('error').removeClass('success');
                $('span.help-inline', el).hide();
            }, getPostData: function () { //获取请求数据
                var $this = this,
                    op = this.options,
                    el = this.$element;

                var para = {};
                var formParam = el.find(':input').serializeArray();
                var _p = {};
                $.each(formParam, function () {
                    _p[this.name] = _p[this.name] === undefined ? $.trim(this.value)
                        : _p[this.name] + ',' + $.trim(this.value);
                });
                $.each(_p, function (k, v) {
                    para[k] = v;
                });

                return para;
            }, post: function (para) {
                var $this = this,
                     op = this.options,
                     ret = $this.val(para),
                     el = $this.$element;
                if (ret[0]) {
                    el.trigger($.Event('valOk'), ret[1]);
                    op.postData = ret[1];

                    if (op.cmd) {
                        ret[1]._c = op.cmd;
                        if (op.alertMsg !== '') {
                            $.alertMsg('提示', op.alertMsg, 1);
                        }

                        now.cmd(op.cmd, $.extend(ret[1], op.para), function (ret) {
                            if (op.alertMsg !== '') {
                                $('#_nui_alertmsg .close').click();
                            }
                            var e = $.Event('cmdReturn');
                            el.trigger(e, ret);

                            el.trigger($.Event('cmdOk'), ret);
                            $(op.submit).removeAttr('disabled');
                        }, function (ret) {
                            if (op.alertMsg !== '') {
                                $('#_nui_alertmsg .close').click();
                            }
                            var e = $.Event('cmdReturn');
                            el.trigger(e, ret);

                            if (op.alertErr) {
                                $.alertMsg('出错提示', ret.m);
                            }
                            el.trigger($.Event('cmdError'), ret);
                            $(op.submit).removeAttr('disabled');
                        }, false);
                    } else {
                        el.trigger($.Event('setOk'), ret[1]);
                        $(op.submit).removeAttr('disabled');
                    }
                } else {
                    el.trigger($.Event('valError'));
                    $(op.submit).removeAttr('disabled');
                }
            }, enabled: function (para) {
                var $btn = $(this.options.submit);
                if (para) {
                    $btn.removeAttr('disabled');
                } else {
                    $btn.attr('disabled', 'disabled');
                }
            }
        };

        $.fn.frm = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('frm'),
                     os = (typeof option === 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.frm.defaults, typeof option === 'object' && option);
                    $this.data('frm', (data = new Frm(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, typeof option === 'object' && option);
                }

                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.frm.defaults = {
            cmd: '',//发送的请求
            submit: '',//点击的按钮
            para: {},//附加参数
            ok_flag: false,//是否显示校验通过的那个 勾
            alertErr: true, //是否自动弹出错误提示？
            css: 'nui-frm',
            alertMsg: ''
        };

        $(document).on('click.frm.data-api', '[data-api="frm"]', function (e) {
            var el = $(this),
                 $target = $(el.attr('data-target'));
            if (!el.data('frm')) {
                var opt = {};
                if (el.attr('data-cmd')) {
                    opt = {cmd: el.attr('data-cmd')};
                }
                $target.frm(opt);
            }
            $target.frm('post');
        });
    }(jQuery);

    /**
     * 快速表单创建
     * $('#dom').quickfrm({
 *    fields : [
 *      {name:'user[account]', mdl:'base', text:'帐号', check:'must str-6-20', type:'text', edit:true, title:'', id:'' val="aaa"}
 *    ],
 *    submit : '提交',
 *    cmd : 'aa',
 *    para : {},
 *    title : '',
 *    direct : 'horizontal',
 *    class : 'well'
 * });
     */
    !function ($) {
        var QuickFrm = function (element, options) {
            this.options = options;
            this.$element = $(element);
            this.$element.addClass(options.css).addClass(options.otherCss);
            this._firstEdit = -1;
        };

        var _fieldId = 0;

        //每个字段的默认值
        var _defField = {
            title: '',    //标题
            type: 'text', //类型 text checkbox select textarea
            id: '',       //id
            val: '',       //值
            must: false,  //是否必填
            edit: true,   //是否可编辑
            check: '',    //额外的检查项
            attr: '',      //特殊属性  {'data-api':'datepick',placeholder:"最大操作时间"}
            rows: '3',     //只在textarea有效
            cols: '30',     //只在textarea有效
            css: 'input-medium',
            pcss: '' // 为control-group标签添加样式
        };

        QuickFrm.prototype = {
            constructor: QuickFrm,
            _init: function () {
                var self = this,
                     el = self.$element,
                     op = self.options,
                     fLen = op.fields.length,
                     _check = '';

                if (op.width>0) {
                    el.attr('style', 'width:'+op.width+'px;');
                }

                el.focus(function () {
                    if (self._firstEdit >= 0) {
                        //$('.controls:eq('+self._firstEdit+') :first-child', el).focus();
                    }
                });

                if (op.enter === true && op.direct == 'v') {
                    //如果是表单，并且设置回车直接提交，则绑定回车的submit
                    el.keyup(function (event) {
                        if (event.keyCode === 13) {
                            $("button[type=submit]", el).trigger("click");
                        }
                    });
                }

                var add_html = ['', '', ''];
                var cls = '';
                var left_cls = '';
                if (op.direct === 'v') {
                    add_html[0] = '<div class="control-group">';
                    add_html[1] = '<div class="controls">';
                    add_html[2] = '</div></div>';
                    cls += 'form-horizontal ';
                }
                if (op.direct === 'h') {
                    add_html[0] = '<li class="form_inline1st">';
                    add_html[1] = '<div class="pull-left">';
                    add_html[2] = '</div></li>';
                    cls += 'form-inline clearfix ';
                }

                var html = '<div class="' + cls + op.frmClass + '">';

                var _getCheck = function (_fld) {
                    var _ret = _fld.must ? 'must ' + _fld.check : _fld.check;
                    if (_fld.check) {
                        return ' check="' + _ret + '"';
                    }
//                    ret = +_ret;
                    if (_fld.mdl && _fld.tbl) {
                        /*var arr = _fld.name.split('['),
                             mdl = now.cfg.check[_fld.mdl],
                             fldName = arr[1].substr(0, arr[1].length - 1);*/
                        var mdl = now.cfg.check[_fld.mdl];

                        if (mdl && _fld.tbl && _fld.name) {
                            if (_ret !== '') {
                                _ret += ' ';
                            }
                            _ret += mdl[_fld.tbl][_fld.name];
                        }
                    }
                    return ' check="' + _ret + '"';
                };

                if (op.title && !op.dialog) {
                    html += '<h4>' + op.title + '</h4>';
                }

                var selectJoinData = {};
                for (var i = 0; i < fLen; i++) {

                    var _fld = $.extend({}, _defField, op.fields[i]),
                         _id = (_fld.id === '') ? '__fld' + (_fieldId++) : _fld.id;
                    _fld.id = _id;
                    op.fields[i] = _fld;

                    // 验证mdl和name是否填错是否填错
                    if(now.cfg.check && _fld.mdl && _fld.name){
                        // 验证quickfrm的mdl和name是否填错
                        //var reg = /(.*?)\[(.*?)\]/;
                        //var matchArr = _fld.name.match(reg);

                        if(now.cfg.check[_fld.mdl] === undefined){
                            console.log(_fld.text + '：mdl 填写错误，不存在这个库');
                        //}else if(matchArr === null){
                        //    console.log(_fld.text + '：name='+_fld.name+'填写有误,格式应该是：表名[字段名]');
                        }else if(now.cfg.check[_fld.mdl][_fld.tbl] === undefined){
                            console.log(_fld.text + '：'+_fld.mdl+'库不存在表'+_fld.tbl+', 填写错误!');
                        }else if(now.cfg.check[_fld.mdl][_fld.tbl][_fld.name] === undefined){
                            console.log(_fld.text + '：'+_fld.mdl+'库中表'+_fld.tbl+'不存在字段'+_fld.name+', 填写错误!');
                        }
                    }

                    if (op.check) {
                        _check = _getCheck(_fld);
                    }

                    var attr = '';
                    $.each(_fld.attr, function (i, e) {
                        attr += i + '="' + e + '" ';
                        if (i === 'check') {
                            _check = '';
                        }
                    });

                    var _label_attr = '';
                    if (_fld.label_attr) {
                        $.each(_fld.label_attr, function (i, e) {
                            _label_attr += i + '="' + e + '" ';
                        });
                    }

                    if (_fld.type === 'div' || op.direct === 'h') {
                        left_cls = 'pull-left';
                    } else {
                        left_cls = '';
                    }

                    var controlGroupHtml_0 = add_html[0];
                    // 为controls-group添加样式
                    if( _fld.pcss ){
                        controlGroupHtml_0 = controlGroupHtml_0.replace(/class="(control-group)"/g, 'class="$1 '+ _fld.pcss +'"');
                        controlGroupHtml_0 = controlGroupHtml_0.replace(/class="(form_inline1st)"/g, 'class="$1 '+ _fld.pcss +'"');
                    }
                    html += controlGroupHtml_0;
                    if( _fld.showLabel === undefined || _fld.showLabel !== false){
                        html += '<label class="control-label ' + left_cls + '" for="' + _id + '"  ' + _label_attr + '>' + _fld.text + (_fld.must ? '<span class="field-must">*</span>' : '') + '</label>';
                    }
                    html += add_html[1];
                    if (!_fld.edit) {
                        var paddingTop = '';
                        if(op.direct === 'h'){
                            paddingTop = '5';
                        }else{
                            paddingTop = '5';
                        }

                        // 不需要编辑
                        if(_fld.name === undefined){
                            html += '<span style="vertical-align:-'+paddingTop+'px;" class="' + _fld.css + '" ' + _check + ' id="' + _id + '" ' + attr + '>' + _fld.val + '</span>';
                        }else{
                            html += '<span style="vertical-align:-'+paddingTop+'px;" name="' + _fld.name + '" class="' + _fld.css + '" ' + _check + ' id="' + _id + '" ' + attr + '>' + _fld.val + '</span>';
                        }
                    } else {
                        if (self._firstEdit === -1) {
                            self._firstEdit = i;
                        }
                        if (_fld.type === 'select') {
                            html += '<select name="' + _fld.name + '" class="' + _fld.css + '" id="' + _id + '"' + _check + ' ' + attr + '>';
                            if (_fld.selectData) {
                                if (_fld.selectAll) {
                                    if(typeof(_fld.selectAll) === 'string'){
                                        html += '<option value="">' + _fld.selectAll + '</option>';
                                    }else{
                                        html += '<option value="'+_fld.selectAll.val+'">' + _fld.selectAll.text + '</option>';
                                    }
                                }
                                if (typeof _fld.selectData === 'string') {
                                    var sltPara = {};
                                    if (_fld.selectPara) {
                                        sltPara = _fld.selectPara;
                                    }
                                    now.cmd(_fld.selectData, sltPara, function (ret) {
                                        $.each(ret.o.rows, function (key, val) {
                                            if ($.inArray(key, _fld.removeData) === -1) {
                                                var slt = _fld.val === key || key === ret.selected? ' selected="selected"' : '';
                                                html += '<option value="' + key + '"' + slt + '>' + val + '</option>';
                                            }
                                        });
                                    }, '', false);

                                    //联动数据
                                    if(_fld.selectJoin){
                                        if(selectJoinData[_fld.selectJoin] === undefined){
                                            selectJoinData[_fld.selectJoin]=[]
                                        }
                                        selectJoinData[_fld.selectJoin].push({cmd:_fld.selectData, para:sltPara, id:_id, type:'select'});
                                    }
                                } else {
                                    $.each(_fld.selectData, function (key, val) {
                                        if ($.inArray(key, _fld.removeData) == -1) {
                                            var slt = (_fld.val  == key && _fld.val !== '')? ' selected="selected"' : '';
                                            html += '<option value="' + key + '"' + slt + '>' + val + '</option>';
                                        }
                                    });
                                }
                            }
                            html += '</select>';
                        } else if (_fld.type === 'textarea') {
                            if (_fld.css === 'input-medium') {
                                _fld.css = 'input-xlarge';
                            }
                            html += '<textarea name="' + _fld.name + '"' + _check + ' class="' + _fld.css + '" id="' + _id + '" rows="' + _fld.rows + '" cols="' + _fld.cols + '" ' + attr + '>' + _fld.val + '</textarea>';
                        }else if(_fld.type === 'hr'){
                            // 分割线
                            html += '<hr/>';
                            if( typeof _fld.title !== 'undefined' ) html += '<h3>'+ _fld.title +'</h3>';
                        }else if (_fld.type === 'div') {
                            if (_fld.css === 'input-medium') {
                                _fld.css = '';
                            }
                            if (_fld.val) {
                                html += '<div class="' + _fld.css + ' ' + left_cls + '" id="' + _id + '" ' + attr + '>' + _fld.val + '</div>';
                            } else {
                                html += '<div class="' + _fld.css + ' ' + left_cls + '" id="' + _id + '" ' + attr + '></div>';
                            }
                        } else if (_fld.type === 'button') {
                            html += '<button class="btn" id="' + _fld.id + '" ' + attr + '>' + _fld.val + '</button>';
                        } else if (_fld.type === 'radio') {//单选框
                            html += '<span id="' + _fld.id +'">';

                            if (_fld.sourceData !== undefined) {
                                var checked = '';
                                $.each(_fld.sourceData, function (k, v) {
                                    checked = (k === _fld.val ? 'checked' : '');
                                    html += '<input name="' + _fld.name + '" type="' + _fld.type + '" value="' + k + '" ' + checked + ' ' + _check + ' ' + attr + ' autocomplete="off" />';
                                    html += v + '&nbsp;&nbsp;&nbsp;';
                                });
                            }
                            html += '</span>';
                        } else {
                            html += '<input name="' + _fld.name + '" type="' + _fld.type + '" value="' + _fld.val + '"' + _check + ' class="' + _fld.css + '" id="' + _id + '" ' + attr + ' autocomplete="off" />';
                        }
                    }

                    if (_fld.lineFeed) {
                        //html += '<br/><br/>';
                    }

                    html += add_html[2];
                }
                var smtId = '__submit' + (_fieldId++);

                if (op.dialog) {
                    var tmp = '';
                    tmp += '<div class="modal-header">';
                    if (op.close === true) {
                        tmp += '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>';
                    }
                    if (op.title) {
                        tmp += '<h3>' + op.title + '</h3>';
                    }
                    else {
                        tmp += '<h3>&nbsp;&nbsp;&nbsp;</h3>';
                    }
                    tmp += '</div>';
                    if (op.scroll === false) {
                        tmp += '<div class="modal-body" style="overflow:visible;">' + html + '</div></div>';
                    } else {
                        if (op.height) {
                            tmp += '<div class="modal-body" style="height:' + op.height + 'px;max-height:' + op.maxHeight + 'px;">' + html + '</div></div>';
                        } else {
                            tmp += '<div class="modal-body" style="max-height:' + op.maxHeight + 'px;">' + html + '</div></div>';
                        }
                    }
                    if (op.submit) {
                        tmp += '<div class="modal-footer">'+
                            '<button class="btn ' + (op.btnDefault ? '' : 'btn-primary') + '" id="' + smtId + '" type="submit">' + op.submit + '</button>'+
                            '</div>';
                    }

                    el.addClass('modal hide fade').attr({
                        role: 'dialog',
                        'aria-hidden': 'true'
                    }).html(tmp).modal({
                            backdrop: 'static',
                            keyboard: false,
                            show: false
                        });
                } else {
                    if (op.submit) {
                        html += add_html[0]+
                             add_html[1]+
                             '<button class="btn ' + (op.btnDefault ? '' : 'btn-primary') + ' ' + op.submitCss + '" id="' + smtId + '" type="submit">' + op.submit + '</button>'+
                             add_html[2];
                    }
                    el.html(html);
                }
                for (var i=0; i < fLen; i++) {
                    var _fld = op.fields[i];
                    if (_fld.nui) {
                        $.each(_fld.nui, function (k, v) {
                            $('#' + _fld.id)[k](v);

                            if(k==='drop' && _fld.selectJoin){
                                if(selectJoinData[_fld.selectJoin] === undefined){
                                    selectJoinData[_fld.selectJoin]=[]
                                }
                                selectJoinData[_fld.selectJoin].push({para: v.para, id:_fld.id, type:'drop'});
                            }
                        });
                    }
                }

                el.frm({
                    cmd: op.cmd,
                    para: op.para,
                    submit: '#' + smtId
                });

                //下拉框联动事件
                $.each(selectJoinData, function(kind, selectJoinArr){
                    var joinLen = selectJoinArr.length-1,
                        namePara,
                        $tmpObj;
                    $.each(selectJoinArr, function(i, selectObj){
                        if(i<joinLen){
                            $('#'+selectObj.id).change(function(){
                                now.setSync(false);
                                $.each(selectJoinArr, function(k, selChangeObj){
                                    if(k>=i+1){
                                        namePara = {};
                                        for(var t=k-1; t>=0; t--){
                                            $tmpObj = $('#'+selectJoinArr[t]['id']);
                                            namePara[$tmpObj.attr('name')] = $tmpObj.val();
                                        }

                                        if(selChangeObj.type === 'select'){
                                            $('#'+selChangeObj.id).sltFill({
                                                cmd : selChangeObj.cmd,
                                                param : $.extend(namePara, selChangeObj.para)
                                            },'', '全部');
                                        }else{
                                            $('#'+selChangeObj.id).drop({para:$.extend(namePara, selChangeObj.para)}).drop('reload');
                                        }
                                    }
                                });
                                now.setSync(true);
                            });
                        }
                    });
                });

                setTimeout(function () {
                    el.trigger($.Event('reloadOk'), {smtId: smtId});
                }, 1000);
            }
        };

        $.fn.quickfrm = function (option) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('quickfrm'),
                     options = $.extend({}, $.fn.quickfrm.defaults, typeof option === 'object' && option);

                if (!data) {
                    $this.data('quickfrm', (data = new QuickFrm(this, options)));
                    data._init();
                }

            });
        };

        $.fn.quickfrm.defaults = {
            fields: [],  //{name:'user[account]', mdl:'base', text:'帐号', check:'must str-6-20', type:'text', edit:true, title:'', id:''}
            cmd: '',
            submit: '提交', //空则不显示submit按钮
            submitCss: '',
            btnDefault: false,//提交按钮是否为默认按钮样式
            dialog: false, //是否弹出框
            mask: '',      //是否遮住某个div
            para: {},      //额外参数，默认为空
            title: '',     //标题，默认为空
            direct: 'v',  //'v' 纵向  ,'h'横向
            frmClass: 'well',         //表单div的class
            check: true,         //是否需要表单验证，默认要验证(一般作为搜索表单时，direct=h时，需要设置check为false，避免死循环)
            close: true,
            scroll: false,
            css: 'nui-quickform',
            otherCss: '',//附加样式 两列显示(mul-quickform),label加宽(xlong-label,xxlong-label,xxxlong-label)
            height: '', //弹窗高度
            enter: true, //回车直接提交
            maxHeight: 480,
            width: 0
        };

    }(jQuery);

    /**
     * 城市联动选择
     */
    !function ($) {
        var SltTown = function (element, options) {
            options.text = '';
            this.options = options;
            this.$element = $(element);
            this.$element.addClass(options.css);
            this._init();
        };

        var townId = 0;
        var townArr = null;

        SltTown.prototype = {
            constructor: SltTown, _init: function () {
                /* 初始化 */
                 var el = this.$element,
                     op = this.options,
                     html = '';

                var tid = op.tid = 'town' + (townId++),
                    _showNum = parseInt(op.showNum);

                if (!townArr) {
                    $.ajax({
                        url: now.cfg.nowjs + 'town.min.js',
                        type: 'GET',
                        async: false,
                        dataType: 'json',
                        success: function (ret) {
                            townArr = ret;
                        }
                    });
                }

                html += '<select id="' + tid + '_A" style="width:150px">';
                //省
                if (op.hasAll) {
                    html += '<option value="-1">' + op.allTip['province'] + '</option>';
                }
                $.each(townArr, function (key, val) {
                    html += '<option value="' + key + '">' + val.name + '</option>';
                });
                html += '</select>';
                //显示数目为3时才填充第三个下拉框
                if (3 === _showNum) {
                    html += '<select id="' + tid + '_B" style="width:110px"></select>';
                    html += '<select id="' + tid + '_C" name="' + op.field + '" style="width:110px" check="' + op.check + '"></select>';
                } else if (2 === _showNum) {
                    html += '<select id="' + tid + '_B"  name="' + op.field + '" style="width:110px" check="' + op.check + '"></select>';
                }

                el.html(html);

                $('#' + tid + '_A').change(function () {
                    var val = $(this).val();
                    html = '';
                    if(op.hasAll){
                        html += '<option value="' + val + '">' + op.allTip['city'] + '</option>';
                    }
                    if (-1 !== parseInt(val)) {
                        $.each(townArr[val].child, function (key, val) {
                            html += '<option value="' + key + '">' + val.name + '</option>';
                        });
                    }
                    $('#' + tid + '_B').html(html).change();
                });

                $('#' + tid + '_B').change(function () {
                    var val = $(this).val();
                    html = '';
                    //显示数目为3时才填充第三个下拉框
                    if (3 === _showNum) {
                        if (op.hasAll) {
                            html += '<option value="' + val + '">' + op.allTip['town'] + '</option>';
                        }
                        if (-1 !== parseInt(val)) {
                            $.each(townArr[val.substr(0, 2) + '0000'].child[val.substr(0, 4) + '00'].child, function (key, val) {
                                html += '<option value="' + val.code + '">' + val.name + '</option>';
                            });
                        }
                        $('#' + tid + '_C').html(html).change();
                    } else if (2 === _showNum) {
                        op.text = $('#' + tid + '_A').find("option:selected").text() +
                            $(this).find("option:selected").text();
                        var e = $.Event('townChange');
                        el.trigger(e, {data: op.data, text: op.text});
                    }
                });

                if (3 === _showNum) {
                    $('#' + tid + '_C').change(function () {
                        //op.data = $(this).val();
                        op.text = $('#' + tid + '_A').find("option:selected").text()+
                            $('#' + tid + '_B').find("option:selected").text()+
                            $(this).find("option:selected").text();
                        var e = $.Event('townChange');
                        el.trigger(e, {data: op.data, text: op.text});
                    });
                }


                //默认选中哪项
                if (op.data) {
                    op.data += '';
                    $('#' + tid + '_A').val(op.data.substr(0, 2) + '0000');
                    $('#' + tid + '_A').change();
                    $('#' + tid + '_B').val(op.data.substr(0, 4) + '00');
                    $('#' + tid + '_B').change();
                    if (3 === _showNum) {
                        $('#' + tid + '_C').val(op.data);
                    }
                } else {
                    $('#' + tid + '_A').change();
                    $('#' + tid + '_B').change();
                }
            }, getTown: function () {
                var op = this.options;
                return [op.data, op.text];
            }, setTown: function (para) {
                var op = this.options,
                    _showNum = op.showNum;

                $('#' + op.tid + '_A').val(para.substr(0, 2) + '0000');
                $('#' + op.tid + '_A').change();
                $('#' + op.tid + '_B').val(para.substr(0, 4) + '00');
                $('#' + op.tid + '_B').change();
                if (3 === _showNum) {
                    $('#' + op.tid + '_C').val(para);
                }
            }
        };

        $.fn.slttown = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('slttown'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.slttown.defaults, typeof option === 'object' && option);
                    $this.data('slttown', (data = new SltTown(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };


        $.fn.slttown.defaults = {
            all: '',    //选择所有时候显示的文字
            data: '',  //当前选中的值
            field: '',   //字段名字
            check: '',//验证信息
            css: 'nui-slttown',
            hasAll: false,//是否展示头部
            allTip: {//当hasAll为true时才使用该参数
                province: "选择省份",
                city: "选择市",
                town: "选择区县"
            },
            showNum: 3//要显示的联动框数量，目前只有2,3
        };
    }(jQuery);

    /**
     * 数字输入按钮
     * 属性：decimal : 精度，默认0，为没有小数，1为1位小数
     *     negative : true/false 是否允许负数，默认为false
     * <input type="text" data-api="numinput" data-decimal="0" data-negative="false"/>
     */
    !function ($) {
        var NumInput = function (element, options) {
            this.options = options;
            this.$element = $(element);
            this.$element.addClass(options.css);
            this.allowed = '0123456789';
            this.reg = '[^0123456789';
        };

        NumInput.prototype = {
            constructor: NumInput, _init: function () {
                var self = this,
                     el = this.$element,
                     opt = this.options,
                     de = el.attr('data-decimal'),
                     neg = el.attr('data-negative');

                if (de) {
                    opt.decimal = parseInt(de, 10);
                }
                if (neg) {
                    opt.negative = (neg === 'true');
                }

                if (opt.negative) {
                    self.reg += '\\-';
                    self.allowed += '-';
                }
                if (opt.decimal > 0) {
                    self.reg += '\\.';
                    self.allowed += '.';
                }
                self.reg += ']';
                self.reg = new RegExp(self.reg, 'g');

                var check = function () {
                    var v = el.val().replace(self.reg, ""),
                        vl = v.length;
                    if (v) {
                        //如果第一个是.则自动补成0.
                        if(v.substr(0,1) === '.'){
                            v = '0'+v;
                        }
                        var dl = v.lastIndexOf('.');
                        if (v.indexOf('.') !== dl) {
                            v = v.substring(0, dl) + v.substr(dl + 1, vl);
                        }
                        dl = v.lastIndexOf('-');
                        if (v.indexOf('-') !== dl || v.indexOf('-') > 1) {
                            v = v.substring(0, dl) + v.substr(dl + 1, vl);
                        }
                        dl = v.lastIndexOf('.');
                        if (dl > 0 && vl - dl - 1 > opt.decimal) {
                            v = v.substring(0, dl + 1 + opt.decimal);
                        }
                    }
                    el.val(v);
                };

                el.css('ime-mode', 'disabled').on('keyup',function (e) {
                    check();
                    e.preventDefault();
                }).on('cut paste', function (e) {
                        return false;
                    });
                check();
            }
        };

        $.fn.numinput = function (option) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('numinput');

                if (!data) {
                    var options = $.extend({}, $.fn.numinput.defaults, typeof option === 'object' && option);
                    $this.data('numinput', (data = new NumInput(this, options)));
                    data._init();
                }
            });
        };

        $.fn.numinput.defaults = {
            decimal: 0,
            negative: false,
            css: 'nui-numinput'
        };

        $(document).on('keyup.numinput.data-api', '[data-api="numinput"]', function (e) {
            if (!$(this).data('numinput')) {
                $(this).numinput({});
            }
        });
    }(jQuery);

    !function ($) {
        var _isIe = /msie/.test(navigator.userAgent.toLowerCase());
        var _flashVersion = function () {
            if (_isIe) {
                var key = 'ShockwaveFlash.ShockwaveFlash',
                    axo = null;
                try {
                    axo = new ActiveXObject(key + '.7');
                } catch (e) {
                    try {
                        axo = new ActiveXObject(key + '.6');
                        return [6, 0, 21];
                    } catch (e) {
                    }

                    try {
                        axo = new ActiveXObject(key);
                    } catch (e) {
                    }
                }
                if (axo !== null) {
                    return axo.GetVariable("$version").split(" ")[1].split(",");
                }
            } else {
                var p = navigator.plugins;
                var f = p['Shockwave Flash'];
                if (f && f.description) {
                    return f.description.replace(/([a-zA-Z]|\s)+/, "").replace(/(\s+r|\s+b[0-9]+)/, ".").split(".");
                } else if (p['Shockwave Flash 2.0']) {
                    return ['2', '0', '0'];
                }
            }
            return [];
        }();
        var _flash_id = 0;

        var Flash = function (element, options) {
            this.options = options;
            this.$element = $(element);
            this.$element.addClass(options.css);
            this._init();
        };

        Flash.prototype = {
            constructor: Flash, _init: function () {
                var e = this.$element,
                     s = this.options,
                     a = s.availattrs,
                     p = s.availparams,
                     rv = s.version.split('.'),  // Get required version array.
                     o = '<object', // Open output string.
                     attr = function (a, b) {
                        return ' ' + a + '="' + b + '"';
                    },
                    param = function (a, b) {
                        return '<param name="' + a + '" value="' + b + '" />';
                    };

                s.id = '_flash_' + (_flash_id++);
                s.name = s.id;

                // Set codebase, if not supplied in the settings.
                if (!s.codebase) {
                    s.codebase = 'http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=' + rv.join(',');
                }

                // Use express install swf, if necessary.
                if (s.express) {
                    for (var i in cv) {
                        if (parseInt(cv[i], 10) > parseInt(rv[i], 10)) {
                            break;
                        }
                        if (parseInt(cv[i], 10) < parseInt(rv[i], 10)) {
                            s.src = s.express;
                        }
                    }
                }

                // Convert flashvars to query string.
                if (s.flashvars) {
                    s.flashvars = decodeURIComponent($.param(s.flashvars));
                }

                // Set browser-specific attributes
                a = _isIe ? a.concat(['classid', 'codebase']) : a.concat(['pluginspage', 'name']);

                // Add attributes to output buffer.
                var k;
                for (k in a) {
                    var n = (k == $.inArray('src', a)) ? 'data' : a[k];
                    o += s[a[k]] ? attr(n, s[a[k]]) : '';
                }
                o += '>';

                // Add parameters to output buffer.
                for (k in p) {
                    n = (k == $.inArray('src', p)) ? 'movie' : p[k];
                    o += s[p[k]] ? param(n, s[p[k]]) : '';
                }
                // Close and swap.
                o += '</object>';
                if (s.replace) {
                    e.replaceWith(o);
                } else {
                    e.html(o);
                }
            }, getSwf: function () {
                var movie = this.options.name;
                if (_isIe) {
                    return window[movie];
                } else {
                    return document[movie];
                }
            }
        };

        $.fn.flash = function (option, para, cb) {
            if (_flashVersion.length === 0) {
                alert('您需要先安装flash，才能进行操作');
                return;
            }
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('flash'),
                     os = (typeof option === 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.flash.defaults, typeof option == 'object' && option);
                    $this.data('flash', (data = new Flash(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, typeof option == 'object' && option);
                    data._init();
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.flash.defaults = {
            replace: false, //是否进行替换，如果是进行了替换。那么替换后的dom就不会有fn.flash的方法了
            express: '',    //expressInstall.swf的路径
            width: 300,
            height: 200,
            classid: 'clsid:D27CDB6E-AE6D-11cf-96B8-444553540000',
            pluginspage: 'http://get.adobe.com/flashplayer',
            type: 'application/x-shockwave-flash',
            css: 'nui-flash',
            wmode: 'Opaque',
            availattrs: [
                'id',
                'class',
                'width',
                'height',
                'src',
                'type'
            ],
            availparams: [
                'src',
                'bgcolor',
                'quality',
                'allowscriptaccess',
                'allowfullscreen',
                'flashvars',
                'wmode'
            ],
            version: '10.0.0'
        };
    }(jQuery);

    /**
     * 一个单文件上传组件：
     * 使用示例：
     * <div id="divUpload"></div>
     *
     * $('#divUpload').upload({
 *   btn : '选择上传文件',
 *   frmName : '隐藏表单的名字',
 *   frmId : '隐藏表单的id',
 *   filter : {'*.doc, *.docx':'文旦文件','*.png':'png图片文件'},
 *   alertErr : true, //默认为true，也就是指弹出错误提示
 *   fileSize : 120,  //文件大小
 *   fileNum : 1  //一次可上传多少个文件，1表示一个，0表示不限制，3表示最多3个。
 * }).on('selectOk', function(e){
 */
    !function ($) {
        var _uploadId = 0;
        var _uploadTask = {};

        $._uploadSelectOk = function (id, data, name, size) {
            if (_uploadTask[id]) {
                _uploadTask[id]._selectOk(data, name, size);
            }
        };
        $._uploadSelectErr = function (id, code, msg) {
            if (_uploadTask[id]) {
                _uploadTask[id]._selectErr(code, msg);
            }
        };
        $._uploadOk = function (id, ret, name, size) {
            if (_uploadTask[id]) {
                _uploadTask[id]._uploadOk(ret, name);
            }
        };

        var Upload = function (element, options) {
            if ($(element).attr('id')!==undefined) {
                options.uploadId = $(element).attr('id');
            }else {
                options.uploadId = '_upload_' + (_uploadId++);
            }

            this.options = options;
            this.$element = $(element).attr('_nui_dispose', 'upload');
            this.$element.addClass(options.css).addClass('_clear');
            _uploadTask[options.uploadId] = this;
            this._init();
        };

        Upload.prototype = {
            constructor: Upload, _init: function () {
                var el = this.$element,
                     op = this.options,
                     html = '';

                if (op.frmName) {
                    op.frmDataId = '_upload_data_' + (_uploadId++);
                    html += '<input type="hidden" name="' + op.frmName + '_data" id="' + op.frmDataId + '"/>';
                    op.frmNameId = '_upload_file_' + (_uploadId++);
                    html += '<input type="hidden" name="' + op.frmName + '_name" id="' + op.frmNameId + '" readOnly="readOnly"/>';
                    op.frmNameShowId = '_upload_filename_show_' + (_uploadId++);
                }

                el.flash({
                    width: op.width + 2,
                    height: op.height + 2,
                    wmode: op.wmode,
                    flashvars: {
                        id: op.uploadId,
                        fileSize: op.fileSize,
                        fileExt: op.fileExt,
                        fileDesc: op.fileDesc,
                        fileNum: op.fileNum,
                        uploadUrl: op.uploadUrl == '' ? '' : escape(op.uploadUrl + '&_s=' + now.uniqid()),
                        fieldName: op.fieldName,
                        para: op.para,
                        txt: op.btn
                    },
                    src: now.cfg.nowjs + 'upload.swf?' + Math.random() * 1000
                }).prepend(html);

                el.on('click', '.upload-close', function () {
                    $('#' + op.frmNameShowId).parent().find('br').remove();
                    $('#' + op.frmNameShowId).remove();
                    $('#' + op.frmDataId).val('');
                    $('#' + op.frmNameId).val('');

                    // 抛出关闭ok
                    el.trigger($.Event('closeOk'), {});
                });


                $(document).on('click', '.close', function () {
                    $('#' + op.frmNameShowId).parent().find('br').remove();
                    $('#' + op.frmNameShowId).remove();
                    $('#' + op.frmDataId).val('');
                    $('#' + op.frmNameId).val('');
                });

            }, _selectOk: function (data, name, size) {
                var el = this.$element,
                    op = this.options;
                if (op.frmName) {
                    $('#' + op.frmNameShowId).parent().find('br').remove();
                    $('#' + op.frmNameShowId).remove();
                    $('#' + op.frmDataId).val(data);
                    $('#' + op.frmNameId).val(name);
                    el.prepend('<div id="' + op.frmNameShowId + '"><span style="float:left;">' + name + '</span><span style="float:left;" class="upload-close"></span></div><br>');
                }

                if (op.retmore !== undefined) {
                    this.$element.trigger($.Event('selectOk'), {data: data, name: name});
                } else {
                    this.$element.trigger($.Event('selectOk'), data);
                }
            }, _selectErr: function (code, msg) {
                alert(msg);
//              if (this.options.alertMsg) {
//                  $.alertMsg('错误提示', msg);
//              }
                this.$element.trigger($.Event('selectErr'), code, msg);
            }, _uploadOk: function (ret, name) {
                var el = this.$element,
                    op = this.options;

                var dataRet = $.parseJSON(ret);

                if (!dataRet.s) {
                    alert(dataRet.m);
                    return;
                }
                if (op.frmName) {
                    $('#' + op.frmNameShowId).parent().find('br').remove();
                    $('#' + op.frmNameShowId).remove();

                    if(dataRet.path === undefined){
                        $('#' + op.frmDataId).val(ret);
                    }else{
                        $('#' + op.frmDataId).val(dataRet.o.path);
                        name = dataRet.o.name;
                    }
                    $('#' + op.frmNameId).val(name);
                    el.prepend('<div id="' + op.frmNameShowId + '"><span style="float:left;" title="'+name+'">' + (name.length > 15 ? (name.substr(0, 15) + '...') : name) + '</span><span style="float:left;" class="upload-close"></span></div><br>');
                }

                if (op.retmore != undefined) {
                    this.$element.trigger($.Event('uploadOk'), {data: dataRet, name: name});
                } else {
                    this.$element.trigger($.Event('uploadOk'), dataRet);
                }
            }, _clear: function () {
                var op = this.options;

                $('#' + op.frmNameShowId).parent().find('br').remove();
                $('#' + op.frmNameShowId).remove();
                $('#' + op.frmDataId).val('');
                $('#' + op.frmNameId).val('');
            }, dispose: function () {
                delete _uploadTask[this.options.uploadId];
            },clear: function () {
                this._clear();
            }, setDef : function(data){
                //设置默认值
                var el = this.$element,
                    op = this.options;
                $('#' + op.frmDataId).val(data.data);
                $('#' + op.frmNameId).val(data.name);
                el.prepend('<div id="' + op.frmNameShowId + '"><span style="float:left;">' + data.name + '</span><span style="float:left;" class="upload-close"></span></div><br>');
            }
        };

        $.fn.upload = function (option, para) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('upload'),
                     os = (typeof option === 'string');
                if (!data) {
                    var options = $.extend({}, $.fn.upload.defaults, typeof option === 'object' && option);
                    $this.data('upload', (data = new Upload(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, typeof option === 'object' && option);
                    data.dispose();
                    $(self._swf).remove();
                    data._init();
                }

                if (os) {
                    data[option](para);
                }
            });
        };

        $.fn.upload.defaults = {
            btn: '浏览文件',
            width: 70,
            height: 26,
            wmode: 'opaque',
            fontSize: 12,
            frmName: '',
            frmId: '',
            alertErr: true,
            fileExt: '*.*',
            fileDesc: '所有文件类型',
            fileSize: 120,
            fileNum: 1,
            fieldName: 'FileData',
            uploadUrl: '',//当上传到后端时需要指定地址，并带上_s参数
            para: {},
            css: 'nui-upload'
        };
    }(jQuery);


    /**
     * 一个日期选择控件
     * 属性：
     * <input type="text" data-api="datepick" data-time="false"/>
     */
    !function ($) {
        var _id = '_nui_datepick';
        var _uuid = 0;
        var DatePick = function (element, options) {
            this.options = options;
            this.$element = $(element).attr('_nui_dispose', 'datepick').attr('readonly','readonly').addClass(options.elCss);
            this.$element.addClass(options.css);
            this._init();
        };
        var _num = function (m) {
            return m < 10 ? '0' + m : m + '';
        };
        var _getYm = function (y, m) {//得到前一月或下一月
            y = (m < 0) ? y - 1 : ((m > 12) ? y + 1 : y);
            m = (m < 0) ? 12 : ((m > 12) ? 1 : m);
            return y + '-' + _num(m) + '-';
        };
        var _getYmd = function (d) {
            var day = d || new Date(),
                 _y = day.getFullYear(),   //年
                 _m = day.getMonth() + 1,   //月
                 _d = day.getDate();      //日
            return _y + '-' + _num(_m) + '-' + _num(_d);
        };
        var _getHmi = function(d){

            var day = d||new Date(),
                _h = day.getHours(),
                _m =day.getMinutes(),
                _i = day.getSeconds();

                return _num(_h) + ':' + _num(_m) + ':' + _num(_i);
        };

        DatePick.prototype = {
            constructor: DatePick,
            _init: function () {
                var el = this.$element,
                     op = this.options,
                     _sltTime = '';

                op._uuid = _uuid++;
                el.on('click.' + op._uuid + '.datepick.data-api', function (e) {
                	
                    e.preventDefault();
                    e.stopPropagation();
                    var curOs = $(this).offset();
                    var val = el.val();                    
                    if (val && now.val(['date'], val) === '') {
                        $('#' + _id).data('day', val);
                    }
                    if (val == '' && op.time_refresh) {
                    	//没有选择时间，且设置需要清除上一次选择项
                    	//console.log('clear last choose');
                    	$('#' + _id).data('day', _getYmd());
                    	$('#' + _id).data('time', _getHmi());
                    }
                    //默认时间
                    var defDate = val !== '' ? val : op.date;

                    if(op.fixed){
                        $('#' + _id).css({'position':'absolute'});
                    }
                    // 初始化控件
                    $('#' + _id).datecontainer({ time: op.time, clear: op.clear, date: defDate }).css({
                        left: curOs.left,
                        top: curOs.top + 10 + el.height(),
                        'margin-left': '0px',
                        'margin-top': '0px',
                        'z-index': 10000
                    });


                    // 点击单元格
                    $('#' + _id).on('click.' + op._uuid + '.datepick.data-api', ".day a",function (e) {
                        if($(this).attr('class') != 'unactive'){
                            _sltTime = $(this).attr('ym') + _num($(this).text());
                            if (op.time === true) {
                                _sltTime += ' ' + $('#_date_hour').val() + ':' + $('#_date_minute').val();
                                if(op.show_secs)
                                    _sltTime += ':00';
                            }
                            el.val(_sltTime);

                            if(op.time){
                                //清除其他day里面的active
                                $('#'+ _id).find('.day a.active').removeClass('active');
                                //把本身设置为active
                                $(this).addClass('active');
                            }else{
                                $('#' + _id).off('click.' + op._uuid);
                                $('#' + _id).hide();
                            }

                        }
                    })
                        // 点击确定
                        .on('click.' + op._uuid + '.datepick.data-api', ".time-btn-ok",function (e) {
                        var _currDayObj = $('.day').find('a[class=active]');
                        _sltTime = _currDayObj.attr('ym') + _num(_currDayObj.html());
                        if (op.time === true) {
                            _sltTime += ' ' + $('#_date_hour').val() + ':' + $('#_date_minute').val();
                            if(op.show_secs)
                            _sltTime += ':00';
                        }
                        el.val(_sltTime);
                        $('#' + _id).off('click.' + op._uuid);
                        $('#' + _id).hide();
                    })
                        // 点击清除
                        .on('click.' + op._uuid + '.datepick.data-api', ".time-clear-ok",function (e) {
                        el.val('');
                        $('#' + _id).off('click.' + op._uuid);
                        $('#' + _id).hide();
                    })
                        .show();
                });

                $('html').on('click.' + op._uuid + '.datepick.data-api', function (e) {
                    $('#' + _id).off('click.' + op._uuid);
                    $('#' + _id).hide();
                });

            },
            hidden: function (e) {
                this.$element.hide();
            },
            dispose: function () {
                $('html').off('click.' + op._uuid);
            }
        };

        $.fn.datepick = function (option, para) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('datepick');                if (!data) {
                    var options = $.extend({}, $.fn.datepick.defaults, typeof option === 'object' && option);
                    $this.data('datepick', (data = new DatePick(this, options)));
                }

            });
        };

        $.fn.datepick.defaults = {
            time: false,//是否显示时间选择，默认false不显示，true显示
            clear: false,//是否显示清空按钮，默认false不显示，true显示
            date: '',
            css: 'nui-date',
            fixed:false, //是否固定窗口显示位置
            show_secs:true,
            time_refresh:false
        };

        $(document).on('focus.datepick.data-api', '[data-api="datepick"]', function (e) {
            console.log('focus datepick');
            if (!$(this).data('datepick')) {
                $(this).datepick({});
            }
        });

        //初始化
        var DateContainer = function (element, options) {
            this.options = options;
            this.$element = $(element);
            this.$element.addClass(options.css);
            this._init();
        };
        DateContainer.prototype = {
            constructor: DateContainer,
            _init: function () {
                var self = this,
                     el = this.$element,
                     op = this.options;
                //设置默认时间
                if ($.trim(op.date) === '') {
                    el.data('day', _getYmd());
                    el.data('time', _getHmi());
                } else {
                    el.data('day', op.date.split(' ')[0]);
                    el.data('time', op.time ? op.date.split(' ')[1]:_getHmi());
                }

                //?  选择年月
                $('.head a.title', el).on("click", function () {
                    $(el).find('.date-day').hide();
                    $(el).find('.date-year-month').show();
                    var day = new Date(el.data('day').replace(/-/g, '/')),
                         _y = day.getFullYear(),   //年
                         _m = day.getMonth() + 1;   //月
                    self.reloadYearMonth(_y, _m);
                });

                //? 年数减十年
                $('.date-year-month .ym-header .year-pre', el).on("click", function () {
                    var _y = parseInt($(".date-year-month .section-2 a.active", el).attr("data-value"), 10) - 10,
                         _m = parseInt($(".date-year-month .section-1 a.active", el).attr("data-value"), 10);
                    self.reloadYearMonth(_y, _m);
                });

                //? 年数加十年
                $('.date-year-month .year-next', el).on("click", function () {
                    var _y = parseInt($(".date-year-month .section-2 a.active", el).attr("data-value"), 10) + 10,
                         _m = parseInt($(".date-year-month .section-1 a.active", el).attr("data-value"), 10);
                    self.reloadYearMonth(_y, _m);
                });

                //? 点击选定月份
                $(".date-year-month .section-1 a", el).on("click", function () {
                    $(".date-year-month .section-1 a", el).removeClass("active");
                    $(this).addClass("active");
                });

                //? 选定年月
                $('.date-year-month .ym-ok', el).on("click", function () {
                    var _year = parseInt($(".date-year-month .section-2 a.active", el).attr("data-value"), 10);
                    var _month = parseInt($(".date-year-month .section-1 a.active", el).attr("data-value"), 10);

                    var arr = el.data('day').split('-'),
                         pday = new Date(_year, _month - 1, arr[2].split(' ')[0]);

                    el.data('day', _getYmd(pday));
                    self.reload();
                });

                //? 取消选定年月
                $('.date-year-month .ym-cancel', el).on("click", function () {
                    $(el).find('.date-day').show();
                    $(el).find('.date-year-month').hide();
                });

                //上一月
                $('.head a.pre', el).on('click', function () {
                    var arr = el.data('day').split('-');
                    var year = parseInt(arr[0], 10);
                    var month = parseInt(arr[1], 10);
                    var date = parseInt(arr[2].split(' ')[0], 10);
                    var maxLastDate = function(year, month){
                        var tmp = new Date(year, month-1, 0);
                        return tmp.getDate();
                    }(year, month);
                    if(date > maxLastDate){
                        date = maxLastDate;
                    }
                    var pday = new Date(year, month - 2, date);
                    el.data('day', _getYmd(pday));
                    self.reload();
                });

                //下一月
                $('.head a.next', el).on('click', function () {
                    var arr = el.data('day').split('-');
                    var year = parseInt(arr[0], 10);
                    var month = parseInt(arr[1], 10);
                    var date = parseInt(arr[2].split(' ')[0], 10);
                    var maxNextDate = function(year, month){
                        var tmp = new Date(year, month+1, 0);
                        return tmp.getDate();
                    }(year, month);
                    if(date > maxNextDate){
                        date = maxNextDate;
                    }
                    var pday = new Date(year, month, date);
                    el.data('day', _getYmd(pday));
                    self.reload();
                });

                $('#' + _id).on('click.datepick.data-api', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                });

                self.reload();

            },
            reload: function (_mDate) {
                var el = this.$element,
                     op = this.options;
                
                //设置默认日期
                if (typeof _mDate !== 'undefined' && _mDate !== '') {
                    // Firefox的BUG，当年月日后跟随时分秒时，无法解析破折号，必须将破折号转换为反斜杠
                    _mDate = _mDate.replace(/\-/g, '/');
                    el.data('day', _getYmd(new Date(_mDate)));
                    el.data('time', _getHmi(new Date(_mDate)));
                }
                var day = new Date(el.data('day').replace(/-/g, '/') + ' ' + el.data('time')),
                     _y = day.getFullYear(),   //年
                     _m = day.getMonth() + 1,   //月
                     _d = day.getDate(),      //日
                     _h = day.getHours(),       //小时
                     _M = day.getMinutes(),     //分钟
                     _ttl = new Date(_y, _m, 0).getDate(),      //本月共几天
                     _pttl = new Date(_y, _m - 1, 0).getDate(),   //上月共几天
                     _bday = -new Date(_y, _m - 1, 1).getDay(),  //本月1日是周几，0为周日
                     _str = '',
                     _new = 0,
                     _act;

                //更新年月
                $('.title', el).text(_y + '年' + _num(_m) + '月');

                //更新日期
                $('.day', el).html('');
                for (var i = 0; i < 6; i++) {
                    for (var j = 0; j < 7; j++) {
                        if (_bday < 0) {//上个月的时间
                            //张杰，修改日期控件显示上月日期少一天问题，2014-12-22
                            //上个版本代码：_str += '<a href="javascript:void(0)" class="unactive" ym=' + _getYm(_y, _m - 1) + '>' + (_pttl - (-_bday)) + '</a>';
                            _str += '<a href="javascript:void(0)" class="unactive" ym=' + _getYm(_y, _m - 1) + '>' + (_pttl - (-_bday) + 1) + '</a>';
                            _bday = _bday + 1;
                        } else {
                            _bday = _bday + 1;
                            if (_bday > _ttl) {//下个月的时间
                                _new++;
                                _str += '<a href="javascript:void(0)" class="unactive" ym=' + _getYm(_y, _m + 1) + '>' + _new + '</a>';
                            } else {
                                _act = (_bday === _d) ? 'active' : '';
                                _str += '<a href="javascript:void(0)" class="' + _act + '" ym=' + _getYm(_y, _m) + '>' + _bday + '</a>';
                            }
                        }
                    }
                    if (_new > 0 || _bday === _ttl) {
                        break;
                    }
                }

                $('.day', el).html(_str);

                //时间选择设置
                if (op.time === true) {

                    if( _h < 10){
                        _h = '0'+_h;
                    }
                    if(_M < 10){
                        _M = '0'+_M;
                    }
                    $('#_date_hour').find('option[value=' + _h + ']').attr('selected', 'selected');
                    $('#_date_minute').find('option[value=' + _M + ']').attr('selected', 'selected');
                    $('._date_time', el).show();
                } else {
                    $('._date_time', el).hide();
                }

                //清空按钮
                if (op.clear === true) {
                    $('._clear-time', el).show();
                } else {
                    $('._clear-time', el).hide();
                }


                //? 显示天数选择
                $(el).find('.date-day').show();
                $(el).find('.date-year-month').hide();

            },
            // 重载年月选择页面函数
            reloadYearMonth: function (year, month) {
                var el = this.$element;

                $(".date-year-month .section-1 a", el).removeClass("active");
                $(".date-year-month .section-1 a[data-value=" + month + "]", el).addClass("active");

                $(".date-year-month .section-2 li.dynamic", el).remove();

                var yearHead = parseInt(year / 10, 10);
                var str = "";
                for (var i = 0; i < 10; i++) {
                    var value = yearHead + "" + i;
                    if (value == year.toString()) {
                        str += '<li class="dynamic"><a href="javascript:void(0);" data-value="' + value + '" class="se-year active">' + value + '</a></li>';
                    }
                    else {
                        str += '<li class="dynamic"><a href="javascript:void(0);" data-value="' + value + '" class="se-year">' + value + '</a></li>';
                    }
                }
                $(".date-year-month .section-2 ul", el).append(str);

                $(".date-year-month .section-2 a", el).on("click", function () {
                    if ($(this).hasClass("default")) {
                        return;
                    }
                    $(".date-year-month .section-2 a", el).removeClass("active");
                    $(this).addClass("active");
                });
            }
        };
        $.fn.datecontainer = function (option) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('datecontainer'),
                     os = (typeof option === 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.datecontainer.defaults, typeof option === 'object' && option);
                    $this.data('datecontainer', (data = new DateContainer(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                    data.reload(data.options.date);
                }
            });
        };

        $.fn.datecontainer.defaults = {
            time: true,
            clear: false,
            date: '',
            css: 'nui-datepick',
            time_refresh: false
        };

        //日期模块
        //具体天数选择模块
        var str = '<div id="' + _id + '" class="dropdown-menu hide">' +
            '<div class="date-day">' +
            '<div class="head">' +
            '<a href="javascript:void(0);" class="pre"><i class="icon-chevron-left"></i></a>' +
            '<a href="javascript:void(0);" class="title"></a>' +
            '<a href="javascript:void(0);" class="next"><i class="icon-chevron-right"></i></a>' +
            '</div>' +
            '<div class="body">' +
            '<div class="week">' +
            '<span>日</span>' +
            '<span>一</span>' +
            '<span>二</span>' +
            '<span>三</span>' +
            '<span>四</span>' +
            '<span>五</span>' +
            '<span>六</span>' +
            '</div><div class="clearfix"></div>' +
            '<div class="day  clearfix">' +
            '</div>' +
            '<div class="_date_time">' +
            '<select id="_date_hour" style="width:50px; padding-left:3px;" class="pull-left">';
        for (var _i = 0; _i < 24; _i++) {
            if (_i < 10) {
                str += '<option value="0' + _i + '">0' + _i + '</option>';
            } else {
                str += '<option value="' + _i + '">' + _i + '</option>';
            }
        }
        str += '</select><div class="pull-left">&nbsp;:&nbsp;</div>' +
            '<select id="_date_minute" style="width:50px;  padding-left:3px;" class="pull-left">';
        for (_i = 0; _i < 60; _i++) {
            if (_i < 10) {
                str += '<option value="0' + _i + '">0' + _i + '</option>';
            } else {
                str += '<option value="' + _i + '">' + _i + '</option>';
            }
        }
        str += '</select>' +
            '<div class="pull-left">&nbsp;<button class="time-btn-ok btn-primary">确定</button></div>' +
            '</div>' +
            '<div class="_clear-time pull-left">&nbsp;<button class="time-clear-ok btn-primary">清空</button></div>' +
            '</div>' +
            '</div>';

        //年月选择模块
        str += '<div class="date-year-month">';
        str += '<div class="ym-header clearfix">';
        str += '<div class="section-1">';
        str += '<ul>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="1">一月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="2">二月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="3">三月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="4">四月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="5">五月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="6">六月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="7">七月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="8">八月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="9">九月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="10">十月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="11">十一月</a></li>';
        str += '<li><a href="javascript:void(0);" class="se-month" data-value="12">十二月</a></li>';
        str += '</ul>';
        str += '</div>';
        str += '<div class="section-2">';
        str += '<ul>';
        str += '<li><a href="javascript:void(0);" class="default year-pre"><i class="icon-chevron-left"></i></a></li>';
        str += '<li><a href="javascript:void(0);" class="default year-next"><i class="icon-chevron-right"></i></a></li>';
        str += '</ul>';
        str += '</div>';
        str += '</div>';
        str += '<div class="ym-footer">';
        str += '<a href="javascript:void(0);" class="ym-ok btn">确定</a>';
        str += '<a href="javascript:void(0);" class="ym-cancel btn">取消</a>';
        str += '</div>';
        str += '</div>';

        str += '</div>';
        $('body').append(str);

    }(jQuery);

    /**
     * 简单的图片显示
     * 可动态显示某个节点下的所有图片，或者传入额外的图片列表
     *
     * @author 金宁宝
     */
    !function ($) {
        var PicShow = function (element, options) {
            this.options = options;
            this.$element = $(element);
            this.$element.addClass(options.css);
            this._init();
        };

        PicShow.prototype = {
            constructor: PicShow, _init: function (e) {
                //图片显示的html代码
                this._getPicHtml();
            }, show: function (idx) {
                $('#_picshow').modal('show');
            }, _getPicHtml: function () {
                var op = this.options,
                     self = this,
                     el = this.$element,
                     _html = '',
                     _imgArr = el.find('img').css({cursor: 'pointer'}).on('click', function () {
                        self.show();
                    });

                _html = '<div class="carousel slide" id="_myCarousel">';
                _html += '<ol class="carousel-indicators">';
                $.each(_imgArr, function (key, imgObj) {
                    if (key == 0) {
                        _html += '<li data-slide-to="0" data-target="#_myCarousel" class="active"></li>';
                    } else {
                        _html += '<li data-slide-to="' + key + '" data-target="#_myCarousel" class=""></li>';
                    }
                });
                _html += '</ol>';

                _html += '<div class="carousel-inner">';
                $.each(_imgArr, function (key, imgObj) {
                    var _active = '',
                         _imgAlt = '';
                    if (key === 0) {
                        _active = 'active';
                    }
                    if (imgObj.alt) {
                        _imgAlt = imgObj.alt;
                    }
                    _html += '<div class="item ' + _active + '">';
                    _html += '<img alt="' + _imgAlt + '" src="' + imgObj.src + '">';
                    if (imgObj.alt) {
                        _html += '<div class="carousel-caption">' + imgObj.alt + '</div>';
                    }
                    _html += '</div>';
                });
                _html += '</div>';

                _html += '<a data-slide="prev" href="#_myCarousel" class="left carousel-control">‹</a>';
                _html += '<a data-slide="next" href="#_myCarousel" class="right carousel-control">›</a>';
                _html += '</div>';

                //显示弹窗
                var tmp = '';
                tmp += '<div class="modal-header">'+
                     '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>'+
                     '<h3>' + op.title + '</h3>'+
                     '</div>';
                tmp += '<div class="modal-body">' + _html + '</div></div>';

                $('body').append('<div id="_picshow" style="width:' + op.dialogWidth + 'px;"></div>');
                $('#_picshow').addClass('modal hide fade in').attr({
                    tabindex: '-1',
                    role: 'dialog',
                    'aria-hidden': 'true'
                }).html(tmp).modal({
                        backdrop: 'static',
                        show: false
                    });

                //初始化图片显示
                $('#_myCarousel', el).carousel();
            }
        };

        $.fn.picshow = function (option, para) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('picshow'),
                     os = (typeof option === 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.picshow.defaults, typeof option === 'object' && option);
                    $this.data('picshow', (data = new PicShow(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, typeof option === 'object' && option);
                }
            });
        };

        $.fn.picshow.defaults = {
            title: 'picture show', //弹窗标题
            dialogWidth: '', //弹窗宽度
            css: 'nui-picshow'
        };
    }(jQuery);


    /**
     * 一个颜色选择控件
     * 属性：
     * data : 当前默认颜色
     * 示例
     * <input type="text" data-api="colorpick"/>
     */
    !function ($) {
        var _id = '_nui_colorpick';
        var _uuid = 0;

        var ColorPick = function (element) {
            this.options = {_uuid: _uuid++};
            this.$element = $(element);
            this._init();
        };

        ColorPick.prototype = {
            constructor: ColorPick, _init: function (e) {
                var el = this.$element,
                     os = el.offset(),
                     op = this.options;

                el.on('click.' + op._uuid + '.colorpick.data-api', function (e) {
                    var val = el.val();
                    $('#' + _id + ' .head').text(val).css({'background-color': val});

                    $('#' + _id).css({
                        left: os.left,
                        top: os.top + 10 + el.height()
                    }).on('click.' + op._uuid + '.colorpick.data-api', ".body a",function (e) {
                            el.val($(this).attr('_color'));
                            $('#' + _id).off('click.' + op._uuid).hide();
                        }).show();

                    e.preventDefault();
                    e.stopPropagation();
                });

                $('html').on('click.' + op._uuid + '.colorpick.data-api', function (e) {
                    $('#' + _id).off('click.' + op._uuid).hide();
                });
            }
        };

        $.fn.colorpick = function () {
            return this.each(function () {
                var $this = $(this)
                    , data = $this.data('colorpick');

                if (!data) {
                    $this.data('colorpick', (data = new ColorPick(this)));
                }

            });
        };

        $(document).on('focus.colorpick.data-api', '[data-api="colorpick"]', function (e) {
            if (!$(this).data('colorpick')) {
                $(this).colorpick({});
            }
        });

        /*初始化*/
        var str = '<div id="' + _id + '" class="nui-colorpick hide">' +
            '<div class="head">' +
            '&nbsp;</div>' +
            '<div class="body">' +
            '</div>' +
            '</div>';
        $('body').append(str);
        str = '';
        var r, g, b, color;
        for (var i = 1; i < 7; i++) {
            r = Math.round(255 / i, 0).toString(16);
            for (var j = 1; j < 7; j++) {
                g = Math.round(255 / j, 0).toString(16);
                for (var k = 1; k < 7; k++) {
                    b = Math.round(255 / k, 0).toString(16);
                    color = '#' + r + g + b;
                    str += '<a style="background-color:' + color + ';" href="javascript:void(0);" _color="' + color + '">&nbsp;</a>';
                }
            }
        }
        $('#' + _id + ' .body').html(str);
        $('#' + _id + ' .body a').on('hover.colorpick.data-api', function (e) {
            var _c = $(this).attr("_color");
            $('#' + _id + ' .head').text(_c).css({'background-color': _c});
        });
    }(jQuery);


    /**
     * 简单的tree 属性：
     * data : 数据来源,[{id:'aa',pid:'aa',txt:'',other:''},{...}]
     * check : 是否显示check
     * 方法：
     * check(id/ids) check一些节点
     * checkAll() check所有节点
     * collapse(id/ids) 关闭某些节点
     * collapseAll() 关闭所有节点
     * expand(id/ids) 打开某些节点
     * expandAll() 打开所有节点
     * select(id) 选中某个节点
     * getSelect() 得到选中的节点
     * getCheck() 得到check的节点列表
     * insert(id, node) 在某个节点后面新增一个节点
     *
     * 事件：
     * select() 被选择
     * check() 被选中
     */
    !function ($) {
        var Tree = function (element, options) {
            // 许道松：解决IE7以下版本，当出现水平滚动条时肯定会出现垂直滚动条的问题
            if($.support.cssFloat === false && $.browser.version <= 7){
                $(element).css({"overflow-x":"scroll","overflow-y":"scroll"});
            }
            // end
            this.options = options;
            this.$element = $(element).addClass(options.css).html('<ul class="nav nav-list parent"><ul>');
            this._data = {};
            this._select = null;
            this._init();
        };

        Tree.prototype = {
            constructor: Tree, _init: function () {
                var self = this,
                    el = this.$element,
                    op = this.options;

                el.on('change', ':checkbox', function (e) {
                    if ($(this).is(":checked")) {//从选中修改为未选中
                        self._event('check', $(this).attr('_id'));
                        //$(':checkbox', $(this)).attr('checked', 'checked');
                        $(this).parent().find('input[type=checkbox]').attr('checked', true);
                    } else {//从未选中修改为选中
                        self._event('uncheck', $(this).attr('_id'));
                        //$(':checkbox', $(this)).removeAttr('checked');
                        $(this).parent().find('input[type=checkbox]').attr('checked', false);
                    }
                    self.$element.trigger($.Event('ckClick'),  $(this).attr('_id'));
                });

                el.on('click', 'i.fdr-tgl', function () {
                    var $i = $(this),
                        $parent = $i.parent(), $nf = $parent.find('a').eq(0).find('i').eq(0);

                    if ($i.hasClass('noline-close')) {
                        $i.removeClass('noline-close').addClass('noline-open');
                        $nf.removeClass('folder-close').addClass('folder-open');
                        $('>ul', $parent).show();
                    } else if ($i.hasClass('noline-open')) {
                        $i.removeClass('noline-open').addClass('noline-close');
                        $nf.removeClass('folder-open').addClass('folder-close');
                        $('>ul', $parent).hide();
                    }
                });

                el.on('click', 'a', function (e) {
                    var $this = $(this),
                        $parent = $this.parent();

                    //todo 以后修改为点击前面的icon做打开关闭
                    if (!$parent.hasClass('selected')) {
                        $('li.selected', el).removeClass('selected');
                        $parent.addClass('selected');
                    }

                    self._event('select', $(this).attr('_id'));
                });

                this.reload();
            }, reload: function () {
                var self = this,
                     op = this.options,
                     para = op.para;

                //重新加载目录树时已选项置空
                self._select = null;

                if (typeof op.data === 'string' && op.data !== '') {
                    now.cmd(op.data, para, function (ret) {
                        self._buildTree(ret.tree);
                    });
                } else {
                    self._buildTree(op.data);
                }
            }, _buildTree: function (data) {
                this._data = data;
                var str = '',
                     el = this.$element,
                     op = this.options,
                     _chNum = 0;
                var getStr = function (json, deep) {
                    var str = '';
                    $.each(json, function (idx, node) {
                        var _fid = node[op.fid],
                             _pid = node[op.pid];
                        str += '<li>';
                        if (op.check) {
                            str += '<input type="checkbox" _id="' + _fid + '" name="ckdata[' + (_chNum++) + ']" value="' + _fid + '" _pid="' + _pid + '" _deep="' + deep + '"';
                            if (node._checked) {
                                str += ' checked="checked"';
                            }
                            str += '/>';
                        }
                        var _cls = node._close ? 'close' : 'open';
                        var _cls1 = node._close ? 'close' : 'open';
                        var _tmp = '<a href="javascript:void(0);" _id="' + _fid + '" _pid="' + _pid + '" _deep="' + deep + '" title="' + node[op.txt] + '">';
                        if (node._child) {//有子节点
                            str += '<i class="switch noline-' + _cls1 + ' fdr-tgl"></i>' + _tmp + '<i class="folder-' + _cls + '"/>';
                        } else {
                            str += _tmp + '<i class="icon-docu"/>';
                        }

                        //处理显示字符长度
                        if (op.txtSize > 0) {
                            str += node[op.txt].substr(0, op.txtSize);
                        } else {
                            str += node[op.txt];
                        }
                        if (node._child) {//有子节点
                            str += '</a><ul class="folder nav nav-list';
                            if (_cls === 'close') {
                                str += ' hide';
                            }
                            str += '">';
                            str += getStr(node._child, deep + 1);
                            str += '</ul></li>';
                        } else {
                            str += '</a></li>';
                        }
                    });
                    return str;
                };
                str = getStr(data, 1);
                $('ul.parent', el).html(str);

				//增加加载完成事件
                el.trigger('build');

            }, _event: function (type, fid) {
                if (fid === '') {
                    return;
                }
                var self = this,
                     data = this._data,
                     _fid = self.options.fid;

                var search = function (json) {
                    var ret = '';
                    $.each(json, function (idx, node) {
                        if (ret) {
                            return;
                        }
                        if (node[_fid] === fid) {
                            ret = node;
                        } else if (node._child !== undefined) {
                            ret = search(node._child);
                        }
                    });
                    return ret;
                };
                var selected = search(data);
                if (selected) {
                    if (type === 'select') {
                        self._select = selected;
                    }
                    var e = $.Event(type);
                    self.$element.trigger(e, selected);
                }
            }, check: function (id) {
                $('input[_id="' + id + '"]', this.$element).attr('checked', 'checked');
            }, checkAll: function () {
                //选中所有
                $(':checkbox', this.$element).attr('checked', true);
            }, unCheckAll: function () {
                //取消所有选中项
                $(':checkbox', this.$element).attr('checked', false);
            }, collapse: function (id) {
                if (typeof id === 'string') {
                    $('ul[_id="' + id + '"]', this.$element).hide();
                } else {
                    var el = this.$element;
                    $.each(id, function (idx, _id) {
                        $('ul[_id="' + _id + '"]', el).hide();
                    });
                }
            }, collapseAll: function () {
                $('ul.folder', this.$element).hide();
            }, extend: function (id) {
                if (typeof id === 'string') {
                    $('ul[_id="' + id + '"]', this.$element).show();
                } else {
                    var el = this.$element;
                    $.each(id, function (idx, _id) {
                        $('ul[_id="' + _id + '"]', el).show();
                    });
                }
            }, extendAll: function () {
                $('ul.folder', this.$element).show();
            }, select: function (id) {
                setTimeout(
                    function () {
                        $('a[_id="' + id + '"]', this.$element).click();
                    },
                    1000);
            }, getSelect: function (para) {
                return this._select;
            }, getCheck: function () {
                var self = this,
                     el = self.$element,
                     op = self.options,
                     ret = [],
                     checked = {},
                     data = this._data.nowData,
                     len = data.length;

                $(':checked', el).each(function (idx, item) {
                    checked[$(this).attr('_id') + ''] = 1;
                });

                for (var i = 0; i < len; i++) {
                    if (checked[ data[i][op.fid] + '' ]) {
                        ret.push(data[i]);
                    }
                }
                return ret;
            }, getNode: function (id) {
                var self = this,
                    data = this._data;

                return self._getNode(data[0], id);
            }, _getNode: function (node, id) {
                var self = this,
                    selNode = null,
                    tmpNode;

                if(node.id === id){
                    selNode = node;
                    return selNode;
                }
                if(node._child && node._child.length>0){
                    $.each(node._child, function(k ,v){
                        tmpNode = self._getNode(v, id);
                        if(tmpNode !== null){
                            selNode = tmpNode;
                        }
                    });
                }
                return selNode;
            }
        };

        $.fn.tree = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('tree'),
                     os = (typeof option === 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.tree.defaults, typeof option === 'object' && option);
                    $this.data('tree', (data = new Tree(this, options)));
                } else if (!os) {
                    /*data.options = $.extend({}, data.options, typeof option === 'object' && option);
                    data._init();*/
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        return data[option](para);
                    }
                }
            });
        };

        $.fn.tree.defaults = {
            check: false,  //是否显示checkbox
            fid: 'id',    //用来唯一标记一行数据的字段
            pid: 'pid',   //用来标记父节点的id
            txt: 'name', //作为显示文字的字段
            data: {},     //取tree的数据，如果是string，则通过后台请求获取，后台必须返回一个tree的key值。
            para: {},      //发送ajax请求时候的额外参数
            css: 'nui-tree',
            txtSize: 0 //目录树显示文本长度，默认为0不限制
        };
    }(jQuery);

    /**
     * 简单的grid
     */
    !function ($) {
        var Grid = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._data = {load: false};
            this._select = null;  //当前点击到的行数据
            this._init();
        };

        Grid.prototype = {
            constructor: Grid, _init: function () { //初始化grid内容
                var self = this,
                     el = this.$element,
                     op = this.options;
                self._getHead();

                $('th.check :checkbox', el).click(function () {
                    if ($(this).is(':checked')) {
                        $('td.check :checkbox', el).attr('checked', 'checked');
                    } else {
                        $('td.check :checkbox', el).removeAttr('checked');
                    }
                });

                el.on('click', 'span.fdr-tgl', function () {
                    var $i = $(this).find('i'),
                         $fid = $i.attr('id');

                    if ($i.hasClass('folder-close')) {
                        $i.removeClass('folder-close').addClass('folder-open');
                        el.find('tr[pClass=' + $fid + ']').show();
                    } else if ($i.hasClass('folder-open')) {
                        $i.removeClass('folder-open').addClass('folder-close');
                        $('.' + $fid).hide();
                        el.find('.' + $fid).find('i.fdr-tgl').removeClass('folder-open').addClass('folder-close');
                    }
                });

                //显示隐藏域
                el.on('click', 'td.show-hidden i', function () {
                    var $i = $(this),
                         dataHidden = $i.attr('data-hidden');

                    if ($i.hasClass('icon-plus')) {
                        $i.removeClass('icon-plus').addClass('icon-minus');
                        el.find('tr.' + dataHidden).show();
                    } else if ($i.hasClass('icon-minus')) {
                        $i.removeClass('icon-minus').addClass('icon-plus');
                        el.find('tr.' + dataHidden).hide();
                    }
                });

                //绑定grid表头排序
                el.on('click', 'a.head-order', function () {
                    var $i = $(this),
                         currOrder = $(this).attr('order'),
                         orderLength = currOrder.length;
                    self.options.para.orderby = currOrder.toLocaleLowerCase();

                    //移除其他表头排序图标
                    $('i.order-ico', el).removeClass('icon-arrow-up');
                    $('i.order-ico', el).removeClass('icon-arrow-down');

                    //修改当前点击表头的图片和排列顺序
                    if (currOrder.toLocaleLowerCase().substr(orderLength - 4, 4) == 'desc') {
                        $('i.order-ico', $i).removeClass('icon-arrow-up');
                        $('i.order-ico', $i).addClass('icon-arrow-down');
                        $i.attr('order', currOrder.toLocaleLowerCase().replace('desc', 'asc'));
                    } else {
                        $('i.order-ico', $i).removeClass('icon-arrow-down');
                        $('i.order-ico', $i).addClass('icon-arrow-up');
                        $i.attr('order', currOrder.toLocaleLowerCase().replace('asc', 'desc'));
                    }

                    //按照排序重新加载列表
                    self.reload(false);
                });

                this.reload();
            }, reload: function (ttl) { //刷新显示
                var self = this,
                     el = this.$element,
                     op = this.options,
                     para = op.para;

                if (typeof op.data === 'string' && op.data !== '') {
                    para.pagesize = op.pagesize;
                    para.pageindex = (op.pageindex-1);
                    para.counttotal = ttl ? ttl : true;
                    
                    now.cmd(op.data, para, function (ret) {
                        if (ret.o[op.listKey].length <= 0 && op.pageindex > 1) {
                            op.pageindex = (op.pageindex-1);
                            self.reload();
                            return;
                        }
                        self._getBody(ret.o[op.listKey]);
                        if (!op.tree && op.pagesize > 0) {
                            self._getPage(ret.o.ttl);
                        }
                        //now.removeFun();//移除没有权限的功能点
                        el.trigger($.Event('reloadOk'), ret.o);
                    });
                } else {
                    if (op.pagesize > 0) {
                        self._getBody(op.data.slice((op.pageindex-1)*op.pagesize, op.pageindex*op.pagesize));
                    } else {
                        self._getBody(op.data);
                    }
                    if (!op.tree && op.pagesize > 0) {
                        self._getPage(op.data.length);
                    }
                    //now.removeFun();//移除没有权限的功能点
                    el.trigger($.Event('reloadOk'), op.data);
                }
                self._data._load = false;

                el.find('input[type=checkbox]').attr('checked', false);

            }, _event: function (type, fid) {//抛出event
                if (fid === '') {
                    return;
                }
                var self = this,
                     data = this._data.nowData;
                for (var i = 0; i < data.length; i++) {
                    if (fid === data[i][self.options.fid]) {
                        if (type === 'select') {
                            self._select = data[i];
                        }
                        var e = $.Event(type);
                        self.$element.trigger(e, data[i]);
                        break;
                    }
                }
            }, _getHead: function (type, fid) {//得到表头

                var self = this,
                     el = self.$element,
                     op = self.options,
                     fl = op.fields.length,
                     str = '';

                str = '<table class="table';
                if (op.strip) {
                    str += ' table-hover table-striped ';
                }
                if (op.row) {
                    fl += 1;
                }
                if (op.check) {
                    fl += 1;
                }
                if (op.border === 'all') {
                    str += ' table-bordered ';
                }
                if (op.border === 'none') {
                    str += ' no-border ';
                }
                str += '"><thead style="display:' + (op.shead === true ? '' : 'none') + ';"><tr>';
                if (op.title) {
                    str = '<div class="header"><i></i><span class="title">' + op.title + '</span></div>' + str;
                }
                if (op.hiddenRender) {
                    str += '<th class="show-hidden"></th>';
                }
                if (op.row) {
                    str += '<th class="row"></th>';
                }
                if (op.check) {
                    str += '<th class="check" style="width: 3px;"><input type="checkbox"></th>';
                }

                var opClass = '';
                $.each(op.fields, function (idx, item) {
                    var w = (item.width) ? ' style="width:' + item.width + 'px"' : '';

                    //验证是否是操作列
                    opClass = '';
                    if (item.op) {
                        opClass = '_operate';
                    }
                    if (item.center) {
                        opClass += ' text-center';
                    }
                    if (item.order) {
                        //为表头添加排序
                        str += '<th' + w + ' class="' + opClass + '"><a class="head-order" order="' + item.order + '" href="javascript:;">' + item.head + '<i class="order-ico"></i></a></th>';
                    } else {
                        str += '<th' + w + ' class="' + opClass + '">' + item.head + '</th>';
                    }
                });
                str += '</tr></thead><tbody></tbody></table>';
                if (!op.tree && op.pagesize > 0) {
                    str += '<div class="pagination pagination-right"></div>';
                }
                el.html(str);
            }, _getBody: function (rows) {//得到主体数据
                var html = '',
                     self = this,
                     el = this.$element,
                     op = this.options,
                     nowPage = op.pageindex,
                     ck = '';

                if (op.reloadHead) {
                    //是否重新加载表头
                    self._getHead();
                }

                var setId = function (json) {
                    $.each(json, function (idx, item) {
                        item['_id'] = now.uniqid();
                        if (item._child) {//有子节点
                            setId(item._child);
                        }
                    });
                };

                setId(rows);
                this._data.nowData = rows;
                
                var getHtml = function (json, indent, pClass, className, prowIndex, ttlCol) {

                    var html = '';
                    prowIndex = prowIndex ? prowIndex + '-' : '';
                    $.each(json, function (idx, item) {
                        var display = '';
                        var fid = (op.fid && item[op.fid]) ? item[op.fid] : '';

                        if (item._close && item._close == true) {
                            //如果请求数据返回结果需要展开，则已返回数据为准
                            op.spread = true;
                        }

                        if (op.tree && item[op.parentKey] != '' && !op.spread) {
                            display = 'display:none';
                        }

                        html += '<tr _id="' + fid + '" class="' + className + '" pclass="' + pClass + '" style="' + display + '">';

                        if (op.hiddenRender) {
                            html += '<td class="show-hidden"><i class="icon-plus" data-hidden="hidden-field-' + fid + '"></i></td>';
                        }
                        if (op.row) {
                            html += '<td class="row">' + indent + prowIndex + ((nowPage - 1) * op.pagesize + idx + 1) + '</td>';
                        }
                        if (op.check) {
                            ck = (item['_check']) ? ' checked="checked"' : '';
                            html += '<td class="check" style="width:3px"><input type="checkbox"' + ck + ' id="check_' + fid + '"></td>';
                        }

                        $.each(op.fields, function (i, f) {
                            var firstCol = i == 0 ? 'first' : '';
                            var style = 'style="';
                            style += f.nowrap ? 'white-space: nowrap;' : '';
                            style += (f.width) ? ('width:' + f.width + 'px;') : '';
                            style += 'text-align:' + (f.align ? f.align : 'left') + ';';
                            style += (f.color) ? ('color:' + f.color) : '';
                            style += '"';
                            var suojin = '';
                            var biaozhi = '';

                            //验证是否是操作列
                            if (f.op) {
                                firstCol += firstCol == '' ? '_operate_td' : ' _operate_td';
                            }
                            var spanClass = '';
                            if (f.name == op.treeField) {
                                suojin = indent;

                                if (item._child) {//有子节点
                                    spanClass = 'class="fdr-tgl"';
                                    if (item._close || op.spread) {
                                        biaozhi = '<a href="javascript:;"><i class="folder-open" id="' + fid + '"></i></a>';
                                    } else {
                                        biaozhi = '<a href="javascript:;"><i class="folder-close" id="' + fid + '"></i></a>';
                                    }
                                } else if (op.tree) {//gridtree
                                    biaozhi = '<a href="javascript:;"><i class="icon-docu" id="' + fid + '"></i></a>';
                                } else {
                                    biaozhi = '';
                                }
                            }
                            if ($.isFunction(f.render)) {
                                html += '<td class="' + firstCol + '" ' + style + '>' +
                                '<div style="float: left;" class="table-cell">'
                                    + suojin + '<span ' + spanClass + '>' + (op.treeOper ? biaozhi : '') + '</span>' +
                                '</div>' +
                                '<div class="table-cell">' +
                                '<span>' + f.render($.getStrText(item[f.name]), item, idx) + '</span>' +
                                '</div>' +
                                '</td>';
                            } else {
                                html += '<td class="' + firstCol + '" ' + style + '>' +
                                '<div style="float: left;" class="table-cell">'
                                    + suojin + '<span ' + spanClass + '>'+ (op.treeOper ? biaozhi : '') + '</span>' +
                                '</div>' +
                                '<div class="table-cell">' +
                                '<span>'+ $.getStrText(item[f.name]) + '</span>' +
                                '</div>' +
                                '</td>';
                            }

                            if($.isFunction(f.render_full)){
                                html = f.render_full(item);
                            }

                        });
                        html += '</tr>';
                        if (item._child) {//有子节点
                            html += getHtml(item._child, indent + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;', fid, className + ' ' + fid, idx + 1, ttlCol);
                        }
                        if (op.hiddenRender) {
                            if ($.isFunction(op.hiddenRender)) {
                                html += '<tr style="display:none;" class="hidden-field-' + fid + '" ><td colspan="' + ttlCol + '">' + op.hiddenRender(item, idx) + '</td></tr>';
                            }
                            else if (typeof op.hiddenRender == 'string') {
                                html += '<tr style="display:none;" class="hidden-field-' + fid + '" ><td colspan="' + ttlCol + '">' + item[op.hiddenRender] + '</td></tr>';
                            }
                        }
                    });

                    return html;
                };

                var field_num = op.fields.length;
                if (op.row == true) {
                    field_num++;
                }
                if (op.check == true) {
                    field_num++;
                }
                if (op.hiddenRender) {
                    field_num++;
                }
                if ($.isEmptyObject(rows)) {
                    html += '<tr><td colspan="' + field_num + '" style="text-align:center">' + op.norender + '</td></tr>';
                } else {
                    html = getHtml(rows, '', '', '', null, field_num);
                }
                $('tbody', el).html(html);
                $('tbody tr', el).click(function () {
                    $(this).toggleClass('row_hover');
                    self._event('select', $(this).attr('_id'));
                });

                $('td.check :checkbox', el).click(function () {
                    var fid = $(this).parent().parent().attr('_id');
                    if ($(this).is(':checked')) {
                        if ($('td.check :checked', el).length == $('td.check', el).length) {
                            $('th.check :checkbox', el).attr('checked', 'checked');
                        } else {
                            $('th.check :checkbox', el).removeAttr('checked', 'checked');
                        }
                        self._event('check', fid);
                    } else {
                        $('th.check :checkbox', el).removeAttr('checked', 'checked');
                        self._event('uncheck', fid);
                    }
                });

            }, _getPage: function (ttl) { //底部分页信息
                var self = this,
                     el = self.$element,
                     op = this.options;
                if (op.pagesize <= 0) {
                    return;
                }
                if (ttl != undefined && parseInt(ttl, 10) > -1) {
                    this._data.ttl = ttl;
                    this._data.ttlPage = Math.ceil(ttl / op.pagesize);
                    self._ttlHtml = '<li class="disabled"><a>共' + ttl + '条</a></li>';
                }
                var ttlPage = self._data.ttlPage,
                     nowPage = op.pageindex,
                     str = '<ul>' + self._ttlHtml,
                     fcss = (nowPage == 1) ? 'disabled' : '',
                     ecss = (nowPage == ttlPage || ttlPage == 0) ? 'disabled' : '';

                str += '<li class="' + fcss + '"><a href="javascript:void(0);">&lt;&lt;</a></li>';
                str += '<li class="' + fcss + '"><a href="javascript:void(0);">&lt;</a></li>';
                var begin = Math.max(1, nowPage - 5);
                if (ttlPage < 11) {//不到10页
                    begin = 1;
                } else if (ttlPage - nowPage < 6) {//最后5页
                    begin = Math.max(1, ttlPage - 9);
                }
                var end = Math.min(ttlPage, begin + 9);
                for (; begin <= end; begin++) {
                    if (begin == nowPage) {
                        str += '<li class="active"><a href="javascript:void(0);">' + begin + '</a></li>';
                    } else {
                        str += '<li><a href="javascript:void(0);">' + begin + '</a></li>';
                    }
                }
                str += '<li class="' + ecss + '"><a class="next" href="javascript:void(0);">&gt;</a></li>';
                str += '<li class="' + ecss + '"><a class="first" href="javascript:void(0);">&gt;&gt;</a></li></li>';
                $('.pagination', el).html(str);
                //绑定分页事件
                $('.pagination li', el).click(function () {
                    if (!$(this).hasClass('disabled') && !$(this).hasClass('active')) {
                        if (self._data._load) {
                            return;
                        }
                        self._data._load = true;
                        var n = $('a', $(this)).text()
                            , nowPage = op.pageindex
                            , ttlPage = self._data.ttlPage;
                        if (n == '>>') {
                            nowPage = ttlPage;
                        } else if (n == '<<') {
                            nowPage = 1;
                        } else if (n == '>') {
                            nowPage = Math.min(nowPage + 1, ttlPage);
                        } else if (n == '<') {
                            nowPage = Math.max(nowPage - 1, 1);
                        } else {
                            nowPage = parseInt(n, 10);
                        }
                        op.pageindex = nowPage;
                        self.reload(false);
                    }
                });
            }, getRows: function () {
                return this._data.nowData;
            }, getRow: function (id) {
                var rows = this._data.nowData,
                     fid = this.options.fid;

                var search = function (sdata) {
                    var ret = '';
                    for (var i = 0; i < sdata.length; i++) {
                        if (sdata[i][fid] == id) {
                            ret = sdata[i];
                            break;
                        } else if (sdata[i]._child) {
                            ret = search(sdata[i]._child);
                            if (ret) {
                                break;
                            }
                        }
                    }
                    return ret;
                };

                return search(rows);
            }, getSelect: function () {//得到当前点击的行的数据
                return this._select;
            }, getCheck: function () {//得到选中的数据
                var self = this,
                     el = self.$element,
                     op = self.options,
                     ret = [],
                     checked = {},
                     data = this._data.nowData,
                     len = data.length;
                $('td.check :checked', el).each(function (idx, item) {
                    checked[$(this).parent().parent().attr('_id') + ''] = 1;
                });

                for (var i = 0; i < len; i++) {
                    if (checked[ data[i][op.fid] + '' ]) {
                        ret.push(data[i]);
                    }
                }
                return ret;
            }, setCheck: function(checkedData){
                var self = this,
                    el = self.$element,
                    selector = [];
                if( checkedData.length === 0 ) return;
                for(var i= 0, row; row = checkedData[i++];){
                    selector.push('#check_'+row.cert_id);
                }
                el.find(selector.join(',')).prop('checked', true);

                if ($('td.check :checked', el).length == $('td.check', el).length) {
                    $('th.check :checkbox', el).prop('checked', true);
                } else {
                    $('th.check :checkbox', el).prop('checked', false);
                }
            }
        };

        $.fn.grid = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('grid'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.grid.defaults, typeof option == 'object' && option);
                    $this.data('grid', (data = new Grid(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.grid.defaults = {
            title: '',      //标题
            check: false,  //是否显示选择框
            border: 'line',//all 全部，line横线  none完全无边框
            strip: false, //是否显示行背景
            tree: false, //是否是表格树，true是表格树,默认是普通表格
            parentKey: 'pid',//父id的key值，默认是pid
            treeField: 'name', //作为树主节点的字段名
            spread: false,//是否展开子节点
            treeOper: true,//是否允许收展树及诶单
            hiddenRender: null, //隐藏扩展
            pagesize: 15,
            pageindex: 1,
            fid: 'id',
            row: true,
            shead: true,//是否显示表头
            fields: [],
            data: '',
            css: 'nui-grid',
            para: {},
            reloadHead: false,//是否重新加载表头
            norender: '还没有相关记录',//数据为空的情况下提示信息
            listKey: 'rows'   //返回列表的key
        };
    }(jQuery);


    /**
     * 分页组件
     *
     * @张章湫
     */
    !function ($) {
        /**
         * 分页函数
         * @param options object
         *    curpage : 1,//当前页码
         *    total: 0,//总记录数
         *    perpage: 10,//每页条数
         *    css:'nui-page//样式
         *
         * @param render string(long/short)/function
         * 自定义render分页点击必须是a标签，包含page属性，值为空/first/pre/next/last/数字/
         * 当page为空时表示不可点
         */
        $.page = function (options, render) {
            var ret = '',
                 ttl = options.total,
                 size = options.perpage,
                 nowPage = options.curpage,
                 ttlPage = Math.ceil(ttl / size);

            var tLong = function () {
                var str = '<ul>',
                     fcss = (nowPage == 1) ? 'disabled' : '',
                     ecss = (nowPage == ttlPage || ttlPage == 0) ? 'disabled' : '';

                if (ttl != undefined && parseInt(ttl, 10) > -1) {
                    str += '<li class="disabled"><a page="">共' + ttl + '条</a></li>';
                }

                str += '<li class="' + fcss + '"><a href="javascript:void(0);" page="' + (nowPage == 1 ? '' : 'first') + '">&lt;&lt;</a></li>';
                str += '<li class="' + fcss + '"><a href="javascript:void(0);" page="' + (nowPage == 1 ? '' : 'pre') + '">&lt;</a></li>';

                var begin = Math.max(1, nowPage - 5);
                if (ttlPage < 11) {//不到10页
                    begin = 1;
                } else if (ttlPage - nowPage < 6) {//最后5页
                    begin = Math.max(1, ttlPage - 9);
                }

                var end = Math.min(ttlPage, begin + 9);

                for (; begin <= end; begin++) {
                    if (begin == nowPage) {
                        str += '<li class="active"><a href="javascript:void(0);" page="">' + begin + '</a></li>';
                    } else {
                        str += '<li><a href="javascript:void(0);" page="' + begin + '">' + begin + '</a></li>';
                    }
                }

                str += '<li class="' + ecss + '"><a class="next" href="javascript:void(0);" page="' + ((nowPage == ttlPage || ttlPage == 0) ? '' : 'next') + '">&gt;</a></li>';
                str += '<li class="' + ecss + '"><a class="first" href="javascript:void(0);" page="' + ((nowPage == ttlPage || ttlPage == 0) ? '' : 'last') + '">&gt;&gt;</a></li></ul>';
                str = '<div class="pagination pagination-right page_ml" pageCon="y">' + str + '</div>';

                return str;
            };

            var tShort = function () {

            };

            if ($.isFunction(render)) {
                ret = render(options.curpage, options.total, options.perpage);
            } else if (render == 'long') {
                ret = tLong();
            } else if (render == 'short') {
                ret = tShort();
            }

            return ret;
        };
    }(jQuery);

    /**
     * N行 × N列的重复显示某个render的组件
     *
     * @金宁宝
     */
    !function ($) {
        var Repeat = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._data = {load: false};
            this._init();
        };

        Repeat.prototype = {
            constructor: Repeat, _init: function () { //初始化repeat内容
                var el = this.$element,
                     op = this.options,
                     str = '';

                if (op.col <= 0) {
                    op.col = 3;
                }
                op.itemSpan = parseInt(12 / op.col, 10);
                op.size = op.col * op.row;

                if (op.tpl == '') {
                    str += '<div>';
                    str += '<div class="repeat-head"><div class="pull-left">';
                    op.title && (str += '<span class="repeat-title">' + op.title + '</span>');
                    str += '</div>';
                    if (op.moreTitle != '') {
                        str += '<div class="pull-right"><a href="javascript:;" class="repeat-more">' + op.moreTitle + '</a></div>';
                    }
                    str += '</div>';
                    str += '<div class="repeat-body"></div>';
                    str += '</div>';
                    el.html(str);
                }

                //点击更多
                el.on('click', 'a.repeat-more', function () {
                    if ($.isFunction(op.moreFun)) {
                        op.moreFun();
                    } else if (op.moreFun != '') {
                        window.location.href = now.cfg.index + '?_c=' + op.moreFun;
                    }
                });

                if (op.showpage) {
                    this.reload();
                } else {
                    this.reload('n');
                }
            }, reload: function (ttl) { //刷新显示
                var self = this,
                     op = this.options,
                     para = op.para,
                     el = this.$element;

                para.start = (op.nowPage - 1) * op.size;
                if (typeof op.data == 'string' && op.data != '') {
                    para.ttl = ttl ? ttl : 'y';
                    if (op.size === '') {
                        para.all = 'y';
                    } else {
                        para.size = op.size;
                    }
                    now.cmd(op.data, para, function (ret) {
                    	var rows = ret.o.rows;
                        if (rows.length <= 0 && op.nowPage > 1) {
                            op.nowPage = op.nowPage - 1;
                            self.reload();
                            return;
                        }
                        self._getBody(rows);
                        if (para.ttl == 'y') {
                            op.ttl = ret.ttl;
                            self._getPage(ret.ttl);
                        } else {
                            self._getPage(op.ttl);
                        }
                        now.removeFun();//移除没有权限的功能点
                        el.trigger($.Event('reloadOk'), ret);
                        if($.isFunction(op.callback)){
                            op.callback(ret);
                        }
                    }, null, false);
                } else {
                    self._getBody(op.data);
                    self._getPage(op.data.length);
                    now.removeFun();//移除没有权限的功能点
                    el.trigger($.Event('reloadOk'), op.data);
                }
                self._data._load = false;
            }, _getBody: function (ret) {//得到主体数据
                this._data.nowData = ret;

                var html = '',
                     el = this.$element,
                     op = this.options,
                     _rowData = '',
                     _rowNum = '';

                if (op.tpl == '') {
                    var liHtml = '<div class="span' + op.itemSpan + ' repeat-item"';
                    liHtml += 'style="margin-left:0px;';
                    if (op.itemHeight) {
                        liHtml += ' height:' + op.itemHeight + 'px;';
                    }
                    if (op.itemBorder) {
                        liHtml += ' border:1px solid;';
                    }
                    liHtml += '">';


                    if (ret.length == 0) {
                        //只显示一列
                        html += '<div class="row-fluid">';
                        html += op.norender;
                        html += '</div>';
                    } else {
                        //计算行数
                        _rowNum = Math.ceil(ret.length / op.col);
                        html += '<div class="repeat-content pull-left" style="width:100%">';
                        html += '<div class="row-fluid">';
                        for (var i = 0; i < _rowNum; i++) {
                            _rowData = ret.slice(op.col * i, op.col * (i + 1));
                            $.each(_rowData, function (idx, item) {
                                if (op.close) {
                                    html += liHtml + '<a class="close _slt-close" type="button" ' + op.fid + '="' + item[op.fid] + '">×</a>';
                                } else {
                                    html += liHtml;
                                }
                                if ($.isFunction(op.render)) {
                                    if (op.isSub === false) {
                                        html += op.render(item) + '</div>';
                                    } else {
                                        html += '<div class="repeat-model-single" id="sltdata_' + item[op.fid] + '">' + op.render(item) + '</div></div>';
                                    }
                                } else {
                                    now.tpl(op.render, item, function (ret) {
                                        if (op.isSub === false) {
                                            html += ret + '</div>';
                                        } else {
                                            html += '<div class="repeat-model-single" id="sltdata_' + item[op.fid] + '">' + ret + '</div></div>';
                                        }
                                    }, false);
                                }
                            });
                            _rowData = '';
                        }
                        html += '</div>';
                        html += '</div>';
                    }

                    $('.repeat-body', el).html(html);
                } else {
                    var html = op.tpl
                        , renderHtml = '';
                    $.each(ret, function (idx, item) {
                        if ($.isFunction(op.render)) {
                            if (op.isSub === false) {
                                renderHtml += op.render(item);
                            } else {
                                renderHtml += '<div class="repeat-model-single" id="sltdata_' + item[op.fid] + '">' + op.render(item) + '</div>';
                            }
                        } else {
                            now.tpl(op.render, item, function (ret) {
                                if (op.isSub === false) {
                                    renderHtml += ret;
                                } else {
                                    renderHtml += '<div class="repeat-model-single" id="sltdata_' + item[op.fid] + '">' + ret + '</div></div>';
                                }
                            }, false);
                        }
                    });

                    var reg = /\{\w+\}/g;
                    var arr = op.tpl.match(reg);
                    if (arr) {
                        for (var i = 0; i < arr.length; i++) {
                            var k = arr[i].substring(1, arr[i].length - 1);
                            if (k != 'render') {
                                html = html.replace(arr[i], op[k]);
                            }
                        }
                    }
                    html = html.replace('{render}', renderHtml);
                    el.html(html);
                }
            }, _getPage: function (ttl) { //底部分页信息
                var self = this,
                     el = self.$element,
                     op = this.options;

                $('[pageCon=y]', el).remove();
                if (!op.showpage || ttl == 0) {
                    return;
                }

                if (ttl != undefined && parseInt(ttl, 10) > -1) {
                    self._data.ttlPage = Math.ceil(ttl / op.size);
                }

                el.append($.page({curpage: op.nowPage, total: ttl, perpage: op.size}, op.pageRender));

                //绑定分页事件
                $('[pageCon=y] a', el).click(function () {
                    var n = $(this).attr('page');
                    if (n != '') {
                        if (self._data._load) {
                            return;
                        }

                        self._data._load = true;
                        var nowPage = op.nowPage
                            , ttlPage = self._data.ttlPage;
                        if (n == 'last') {
                            nowPage = ttlPage;
                        } else if (n == 'first') {
                            nowPage = 1;
                        } else if (n == 'next') {
                            nowPage = Math.min(nowPage + 1, ttlPage);
                        } else if (n == 'pre') {
                            nowPage = Math.max(nowPage - 1, 1);
                        } else {
                            nowPage = parseInt(n, 10);
                        }
                        op.nowPage = nowPage;
                        self.reload('n');
                    }
                });
            }, getRow: function (id) {
                var rows = this._data.nowData,
                     fid = this.options.fid;

                var search = function (sdata) {
                    var ret = '';
                    for (var i = 0; i < sdata.length; i++) {
                        if (sdata[i][fid] == id) {
                            ret = sdata[i];
                            break;
                        } else if (sdata[i]._child) {
                            ret = search(sdata[i]._child);
                            if (ret) {
                                break;
                            }
                        }
                    }
                    return ret;
                };

                return search(rows);
            }
        };

        $.fn.repeat = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('repeat'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.repeat.defaults, typeof option == 'object' && option);
                    $this.data('repeat', (data = new Repeat(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.repeat.defaults = {
            title: '',    //标题
            nowPage: 1,
            render: '', //生成每个数据块函数
            data: '',
            para: {},//请求参数
            col: 3, //每行2个
            row: 2, //每页2行，如果为0表示显示所有
            showpage: true,
            fid: '',
            norender: '还没有相关记录',
            itemHeight: 0,
            itemWidth: 0,
            itemBorder: false,
            pageRender: 'long',
            tpl: '',//模板,变量使用{xx}表示
            css: 'nui-repeat',
            isSub: false, //是否被其他组件使用
            close: false, //每个模块是否需要关闭按钮
            moreTitle: '',//更多链接文字
            moreFun: '',//点击more后的处理,可以是function,也可以是调整链接
            callback: function(ret){}
        };
    }(jQuery);


    /**
     * 图片裁剪组件
     *
     * @金宁宝
     */
    !function ($) {
        var Piccut = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._data = {load: false};

            this.piccut = null;
            if (!window.Piccut) {
                $.ajax({
                    url: options.jsPath ? options.jsPath : now.cfg.nowjs + 'jquery-ui-1.10.3.custom.min.js',
                    type: "GET",
                    async: false,
                    dataType: "script"
                });
                $.ajax({
                    url: options.jsPath ? options.jsPath : now.cfg.nowjs + 'jquery.cropzoom.js',
                    type: "GET",
                    async: false,
                    dataType: "script"
                });
            }

            this._init();
        };

        Piccut.prototype = {
            constructor: Piccut, _init: function () { //初始化Piccut内容
                var self = this,
                     el = this.$element,
                     op = this.options,
                     str = '';

                str = '<div class="modal-header">';
                str += '	<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>';
                str += '	<h3 id="myModalLabel">上传照片</h3>';
                str += '</div>';
                str += '<div class="modal-body">';
                str += '<div class="crop">';
                str += '  <div class="clearfix" style="height: 34px; line-height:34px; overflow:hidden;">';
                str += '	<div id="_uploadTemps" class="pull-left"></div>';
                str += '	<div style="color:red; margin-left:10px;" class="pull-left">友情提示：只允许上传jpg和png格式的图片,图片大小不能超过' + op.fileSize / 1024 + 'M</div>';
                str += '  </div>';
                str += '	<div style="display:none"><img id="_picTemps" src="' + op.picUrl + '"></div>';
                str += '	<div id="cropzoom_container" style="float:left"></div>';
                str += '	<div id="preview" style="overflow:hidden; border:1px solid #333333;margin-left:' + (op.width + 10) + 'px;width:' + op.w + 'px;"><img id="generated" src="' + now.cfg.nowjs + 'img/head.gif"  /></div>';
                str += '	<div class="page_btn" style="margin-left:' + (op.width + 10) + 'px;margin-top:13px;">';
                str += '		<input type="button" class="btn btn-info" id="crop" value="剪切照片" />';
                str += '		<input type="button" class="btn btn-danger" id="restore" value="照片复位" />';
                str += '	</div>';
                str += '	<div class="page_btn" style="margin-left:' + (op.width + 87) + 'px;margin-top:10px;">';
                str += '		<input type="button" class="btn btn-primary" id="save" value="点击保存" />';
                str += '		<input type="hidden" id="pic_ret" value="" />';
                str += '	</div>';
                str += '	<div class="clear"></div>';
                str += '</div>';
                str += '</div>';

                if (op.dialog) {
                    el.addClass('modal hide fade').attr({
                        role: 'dialog',
                        'aria-hidden': 'true'
                    }).css({ width: "650px"}).html(str).modal({
                            backdrop: 'static',
                            keyboard: false
                        });
                } else {
                    el.html(str);
                }

                setTimeout(function () {
                    //设置图片参数
                    var theimage = new Image();
                    theimage.src = op.picUrl;
                    op.image = {};
                    op.image.source = op.picUrl;//图片的路径
                    op.image.width = theimage.width;//图片的宽度
                    op.image.height = theimage.height;//图片的高度
                    op.image.minZoom = op.minZoom;//图片的最小缩放率（百分比）
                    op.image.maxZoom = op.maxZoom;//图片的最大缩放率（百分比）

                    op.selector = {};
                    op.selector.centered = true;
                    op.selector.bgInfoLayer = '#fff';
                    op.selector.startWithOverlay = false;
                    op.selector.w = op.w;//选区的宽度
                    op.selector.h = op.h;//选区的高度
                    op.selector.borderColor = op.borderColor;//选区边框的颜色
                    op.selector.borderColorHover = op.borderColorHover;//当鼠标滑向选区时，选区边框的颜色
                    self.piccut = $('#cropzoom_container', el).cropzoom(op);

                    $("#crop", el).click(function () {
                        if (op.maxUpload) {
                            alert('请上传照片');
                            return;
                        }
                        if(op.savePara._c){
                            delete op.savePara._c;
                        }
                        self.piccut.send(now.cfg.index + '?_c=' + op.cutUrl, 'POST', op.savePara, function (imgRet) {
                            $("#generated").attr("src", imgRet + '?' + new Date().getTime());
                            $("#pic_ret").val(imgRet);
                        });
                    });
                    $("#restore", el).click(function () {
                        $("#generated").attr("src", now.cfg.nowjs + 'img/head.gif');
                        self.piccut.restore();
                    });
                    $("#save", el).click(function () {
                        if ($("#pic_ret", el).val() == '') {
                            alert('请剪切照片');
                        } else {
                            op.savePara.pic_url = $("#pic_ret", el).val();
                            now.cmd(op.saveUrl, op.savePara, function (ret) {
                                el.trigger($.Event('saveOk'), ret);
                            });
                        }
                    });

                    $('#_uploadTemps').upload({
                        fileExt: '*.png;*.jpg',
                        fileDesc: '选择图片',
                        uploadUrl: '/index.php?_c=' + op.sltUrl,
                        fileSize: op.fileSize,
                        para: {}
                    }).on('uploadOk', function (e, ret) {
                        var picTempUrl = '';
                        if (typeof(ret) == 'string') {
                            picTempUrl = ret;
                        }else{
                            picTempUrl = ret.path;
                        }

                        $('#_picTemps').attr('src', picTempUrl);
                        var imageData = {};
                        imageData.source = picTempUrl;
                        setTimeout(function () {
                            self.setImageData(imageData);
                        }, 1000);
                        op.maxUpload = false;
                    });
                }, 1000);

            }, setImageData: function (imageData) {

                var self = this;
                $('#_picTemps').parent().show();
                imageData.width = $('#_picTemps').innerWidth();//图片的宽度
                imageData.height = $('#_picTemps').innerHeight();//图片的高度
                $('#_picTemps').parent().hide();
                self.piccut.setImage(imageData);

                $('#_picTemps').attr('src', '');
            }
        };

        $.fn.piccut = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('piccut'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.piccut.defaults, typeof option == 'object' && option);
                    $this.data('piccut', (data = new Piccut(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.piccut.defaults = {
            sltUrl: '',
            cutUrl: '',
            saveUrl: '',//保存图片请求地址
            cutPara: {},//剪切图片参数
            savePara: {},//保存图片参数
            picUrl: now.cfg.nowjs + 'img/pic_default.png',
            minZoom: 10,//图片的最小缩放率（百分比）
            maxZoom: 150,//图片的最大缩放率（百分比）
            w: 150,//选区的宽度
            h: 210,//选区的高度
            borderColor: 'blue', //选区边框的颜色
            borderColorHover: 'yellow', //当鼠标滑向选区时，选区边框的颜色

            width: 400,//整个图片容器的宽度
            height: 300,//整个图片容器的高度
            bgColor: '#ccc',//容器的背景色
            enableRotation: true, //true/false,是否显示角度调节控件
            enableZoom: true, // 	true/false,是否显示缩放控件
            zoomSteps: 10,
            rotationSteps: 10,
            dialog: false,
            fileSize: 1024, //默认上传图片限制大小1M
            maxUpload: true
        };
    }(jQuery);

    /**
     * 横向的选择。类似select标签
     * @金宁宝
     */
    !function ($) {
        var HSelect = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._data = {load: false};
            this._init();
        };

        HSelect.prototype = {
            constructor: HSelect, _init: function () { //初始化HSelect内容
                var self = this,
                     el = this.$element;

                el.on('click', 'li._a_pill', function () {
                    $(this).parent().find('li.active').removeClass('active');
                    $(this).addClass('active');
                    self._event('select');
                });

                this.reload();
            }, reload: function (ttl) { //刷新显示
                var self = this,
                     op = this.options,
                     para = op.para;

                if (typeof op.data == 'string' && op.data != '') {
                    now.cmd(op.data, para, function (ret) {
                        self._getBody(ret.rows);
                    });
                } else {
                    self._getBody(op.data);
                }
            }, _event: function (type) {

                var self = this,
                     el = this.$element,
                     data = this._data.nowData,
                     _fid = self.options.fid,
                     _selectObj = '';

                _selectObj = $('li.active', el);

                var search = function (json) {
                    var ret = {};
                    $.each(json, function (key, value) {
                        $.each(value.list, function (idx, node) {
                            $.each(_selectObj, function (s, slt) {
                                if (slt.id == node[_fid]) {
                                    ret[$('li#' + slt.id, el).parent().attr('name')] = node[_fid];
                                }
                            });
                        });
                    });
                    return ret;
                };

                var selected = search(data);
                if (selected) {
                    var e = $.Event(type);
                    self.$element.trigger(e, selected);
                }
            }, _getBody: function (rows) {//得到主体数据
                this._data.nowData = rows;
                var html = '',
                     el = this.$element,
                     op = this.options,
                     _classNmae = '_a_pill';

                $.each(this._data.nowData, function (key, value) {
                    html += '<dl>';
                    if (value.title) {
                        html += '<dt>' + value.title + ' : </dt>';
                    }
                    html += '<dd><ul class="nav nav-pills" name="' + value.name + '">';
                    if (value.all) {
                        html += '<li class="' + _classNmae + '"><a href="javascript:;" id="">' + value.all + '</a></li>';
                    }
                    $.each(value.list, function (idx, item) {
                        if (value.val == item[op.fid]) {
                            _classNmae = '_a_pill active';
                        } else {
                            _classNmae = '_a_pill';
                        }
                        html += '<li id="' + item[op.fid] + '"  class="' + _classNmae + '"><a href="javascript:;">' + item.name + '</a></li>';
                    });
                    html += '</ul></dd>';
                    html += '</dl>';
                });

                el.html(html);
            }
        };

        $.fn.hselect = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('hselect'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.hselect.defaults, typeof option == 'object' && option);
                    $this.data('hselect', (data = new HSelect(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.hselect.defaults = {
            data: '',
            para: {},
            fid: 'id', //数据id名称
            css: 'nui-hselect'
        };
    }(jQuery);

    /**
     * 横向的选择。类似select标签
     * @金宁宝
     */
    !function ($) {
        var VSelect = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._data = {load: false};
            this._init();
        };

        VSelect.prototype = {
            constructor: VSelect, _init: function () { //初始化VSelect内容
                var self = this,
                    el = this.$element;
                this.reload();
            }, reload: function (ttl) { //刷新显示
                var self = this,
                    op = this.options,
                    para = op.para;

                if (typeof op.data == 'string' && op.data != '') {
                    now.cmd(op.data, para, function (ret) {
                        self._getBody(ret.rows);
                    });
                } else {
                    self._getBody(op.data);
                }
            }, _event: function (type) {

                var self = this,
                    el = this.$element,
                    data = this._data.nowData;

                var search = function (json) {
                    var ret = {};
                    $.each(json, function (key, value) {
                        $.each(value.list, function (idx, node) {
                            $.each(_selectObj, function (s, slt) {
                                if (slt.id == node[_fid]) {
                                    ret[$('li#' + slt.id, el).parent().attr('name')] = node[_fid];
                                }
                            });
                        });
                    });
                    return ret;
                };

                var selected = search(data);
                if (selected) {
                    var e = $.Event(type);
                    self.$element.trigger(e, selected);
                }
            }, _getBody: function (rows) {//得到主体数据
                this._data.nowData = rows;
                var html = '',
                    el = this.$element,
                    op = this.options,
                    selVal = op.defVal;

                el.html('');
                var slt = '';
                $.each(this._data.nowData, function (key, value) {
                    slt = selVal==key ? 'selected' : '';
                    html += '<option value="'+key+'" '+slt+'>'+value+'</option>';
                });
                el.html(html);
            }
        };

        $.fn.vselect = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                    data = $this.data('vselect'),
                    os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.vselect.defaults, typeof option == 'object' && option);
                    $this.data('vselect', (data = new VSelect(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };
        //适用于select标签
        $.fn.vselect.defaults = {
            data: '',
            para: {},
            defVal : '',  //默认选中
            all : false, //是否显示全部
            css: 'nui-vselect'
        };
    }(jQuery);

    /**
     * 简单的滚动容器
     * @金宁宝
     */
    !function ($) {
        var Scroll = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._data = {load: false};
            this._init();
        };

        Scroll.prototype = {
            constructor: Scroll, _init: function () { //初始化Scroll内容
                var el = this.$element;

                el.on('click', 'div.nui-scroll-left', function () {
                    var _allObj = ''
                        , _showObj = ''
                        , _firstId = ''
                        , _firstShowId = ''
                        , _lastShowId = '';

                    _allObj = $('div.nui-scroll-item', el);
                    _showObj = $('div.show', el);

                    if (_allObj.length > 0) {
                        _firstId = _allObj.get(0).id;
                    }

                    if (_showObj.length > 0) {
                        _firstShowId = _showObj.get(0).id;
                        _lastShowId = _showObj.get(_showObj.length - 1).id;

                        if (_firstShowId != _firstId) {
                            $('#' + _firstShowId, el).prev().removeClass('hide').addClass('show');
                            $('#' + _lastShowId, el).removeClass('show').addClass('hide');
                        }
                    }
                });

                el.on('click', 'div.nui-scroll-right', function () {
                    var _allObj = '',
                         _showObj = '',
                         _lastId = '',
                         _firstShowId = '',
                         _lastShowId = '';

                    _allObj = $('div.nui-scroll-item', el);
                    _showObj = $('div.show', el);

                    if (_allObj.length > 0) {
                        _lastId = _allObj.get(_allObj.length - 1).id;
                    }

                    if (_showObj.length > 0) {
                        _firstShowId = _showObj.get(0).id;
                        _lastShowId = _showObj.get(_showObj.length - 1).id;
                        if (_lastShowId != _lastId) {
                            $('#' + _firstShowId, el).removeClass('show').addClass('hide');
                            $('#' + _lastShowId, el).next().removeClass('hide').addClass('show');
                        }
                    }
                });

                el.on('click', '#_viewHref', function () {
                    el.trigger($.Event('viewAll'), '');
                });

                this.reload();
            }, reload: function (ttl) { //刷新显示
                var self = this,
                     op = this.options,
                     para = op.para;

                if (typeof op.data == 'string' && op.data != '') {
                    para.all = 'y';
                    para.ttl = ttl ? ttl : 'y';
                    now.cmd(op.data, para, function (ret) {
                        self._getBody(ret.rows, ret.ttl);
                    });
                } else {
                    self._getBody(op.data, op.data.length);
                }
            }, _getBody: function (rows, ttl) {//得到主体数据
                this._data.nowData = rows;
                var html = '',
                     el = this.$element,
                     op = this.options,
                     _classNmae = 'show',
                     _scrollWidth;

                _scrollWidth = op.itemWidth * op.showNum + 80;//组件宽度
                html += '<div style="width: ' + _scrollWidth + 'px;">';
                html += '	<div class="row-fluid nui-scroll-head">';
                html += '		<div class="pull-left">';
                if (op.title != '') {
                    html += op.title;
                }
                html += '		</div>';

                html += '		<div class="pull-right">';
                if (op.ttl != '') {
                    html += op.ttl.replace('{ttl}', ttl);
                }
                if (op.ttlTxt != '') {
                    html += '&nbsp;&nbsp;<a id="_viewHref" href="javascript:;">' + op.ttlTxt + '</a>';
                }
                html += '		</div>';
                html += '	</div>';
                html += '	<div class="row-fluid nui-scroll-body" style="height: ' + op.bodyHeight + 'px;width:' + _scrollWidth + 'px;">';
                html += '		<div class="pull-left nui-scroll-left" style="height: ' + op.bodyHeight + 'px;line-height:' + op.bodyHeight + 'px;">&lt;</div>';
                html += '		<div class="pull-right nui-scroll-right" style="height: ' + op.bodyHeight + 'px;line-height:' + op.bodyHeight + 'px;">&gt;</div>';
                html += '		<div class="nui-scroll-content">';
                $.each(this._data.nowData, function (idx, item) {
                    if (idx >= op.showNum) {
                        _classNmae = 'hide';
                    }
                    html += '<div class="nui-scroll-item ' + _classNmae + '" id="' + item[op.fid] + '" style="width: ' + op.itemWidth + 'px;">';
                    if ($.isFunction(op.render)) {
                        html += op.render(item);
                    } else {
                        now.tpl(op.render, item, function (ret) {
                            html += ret;
                        }, false);
                    }
                    html += '</div>';
                });
                html += '		</div>';
                html += '	</div>';
                html += '</div>';

                el.html(html);
            }
        };

        $.fn.scroll = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('scroll'),
                     os = (typeof option == 'string');

                if (!data) {
                    // 将Scroll存储到scroll
                    var options = $.extend({}, $.fn.scroll.defaults, typeof option == 'object' && option);
                    $this.data('scroll', (data = new Scroll(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }

                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.scroll.defaults = {
            title: '',      //标题
            ttl: '',  //是否显示总数 例如：'共{ttl}笔'
            ttlTxt: '', //显示所有连接
            render: '', //生成每个数据块函数
            data: '',
            para: {},
            showNum: 3,//显示的条数
            fid: 'id', //数据id名称
            bodyHeight: '150', //scroll body的高
            itemWidth: '150' //每个数据块宽度
        };
    }(jQuery);


    /**
     * 可自动匹配的下拉框
     * @金宁宝
     */
    !function ($) {
        var Drop = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']).addClass('_clear');
            this._data = {load: false};
            this._init();

        };

        Drop.prototype = {
            constructor: Drop,
            _init: function () { //初始化Drop内容
                var self = this,
                     el = self.$element,
                     op = self.options,
                     selectData = self._data,
                     str = '',
                     dropCount = $('.dropdown').length,
                     minWidth = '';

                if (op.minWidth && op.minWidth > 0) {
                    minWidth = ' min-width: ' + op.minWidth + 'px';
                }

                //fix ie7 position bug ：加降序的z-index
                str += '<div class="dropdown" style="z-index:' + (1000 - dropCount) + '";>';
                str += '<div class="drop-top">';
                str += '	<a class="dropdown-toggle" data-toggle="dropdown" style="' + minWidth + '" href="javascript:;">';
                str += '		<span>'+op.val+'</span>';
                str += '		<b class="caret"></b>';
                str += '		<input id="drop_result" type="hidden" name="' + op.name + '" value="" class="_drop-value" check="'+op.frmCheck+'">';
                str += '	</a>';
                str += '</div>';
                str += '	<div id="dropdown_con" class="dropdown_con">';
                if (!op.hide_search) {
                    str += '	<div class="dropdown_search"><input id="dropSearch" type="text"><a class="ico_search" href="javascript:;">&nbsp;</a></div>';
                }
                str += '	<ul class="dropdown-menu" style="max-height:260px;overflow-y:auto;width: '+op.listWidth+'px;"></ul></div>';
                str += '</div>';

                el.html(str);

                el.click(function (e) {
                    e.stopPropagation();
                });

                $(document).click(function () {
                    $('#dropdown_con', el).hide();
                });

                el.on('click', 'li.nui-drop-li', function () {//搜索直接选中的BUG
                    var _id = $(this).attr('id');
                    $('#dropdown_con', el).hide();

                    setTimeout(function(){
                        self.select(_id);
                    }, 500);
                });

                el.on('click', 'a.dropdown-toggle', function () {
                    $('.dropdown_con').hide();
                    $('.nui-drop .dropdown_con',el).hide();

                    $('#dropdown_con', el).show();
                    $('ul.dropdown-menu', el).show();
                    $('#dropSearch', el).focus();
                });

                el.on('keyup', '#dropSearch', function (event) {
                    var keyword = $.trim($(this).val());
                    if (keyword == '') {
                        $(this).val('');
                        $('li.nui-drop-li', el).show();
                        return false;
                    }

                    var $ul = $('ul.dropdown-menu', el),
                        ulMH = $ul.css('max-height') ? parseInt($ul.css('max-height'), 10) : 260,
                        ulH = $ul.height();
                    $ul.css({'height': 'auto'});
                    if ($('.nui-drop-li:visible', el).length < 1 && ulH >= ulMH) {
                        $ul.height(0);
                    }

                    //搜索
                    var re =new RegExp(keyword, 'i');
                    $ul.find('li a').each(function(i, el){
                        if(re.test($(el).text())){
                            $(el).parent().show();
                        } else {
                            $(el).parent().hide();
                        }
                    });

                    var $ul = $('ul.dropdown-menu', el),
                        ulMH = $ul.css('max-height') ? parseInt($ul.css('max-height'), 10) : 260,
                        ulH = $ul.height();
                    $ul.css({'height': 'auto'});
                    if ($('.nui-drop-li:visible', el).length < 1 && ulH >= ulMH) {
                        $ul.height(0);
                    }
                });

                el.on('keydown', '#dropSearch', function (event) {
                    //绑定回车
                    if (event.keyCode == "13") {
                        self.options.para[op.keyword] = $.trim($(this).val());
                        $('li.nui-drop-li', el).remove();
                        el.drop('reload', 'search');

                        $('#dropdown_con', el).show();
                        return false;
                    }
                });

                el.on('click', '.ico_search', function (event) {
                    self.options.para[op.keyword] = $.trim($('#dropSearch', el).val());
                    $('li.nui-drop-li', el).remove();
                    el.drop('reload', 'search');

                    $('#dropdown_con', el).show();
                    return false;
                });

                //绑定点击  "上下"
                el.on('keydown', function (event) {
                    var $obj = $('#dropdown_con', el);
                    if (el == null || el == undefined || $obj == null || $obj == undefined) {
                        return;
                    }
                    if ($obj.css('display') != 'none' && self._isInViewZone($obj)) {
                        var currentIndex = $('.nui-drop-li:visible', el).index($('.nui-drop-li:visible.active', el));
                        if (event.keyCode == '38') {
                            return self._setLiActive(currentIndex - 1);
                        }
                        else if (event.keyCode == '40') {
                            return self._setLiActive(currentIndex + 1);
                        }
                    }
                });

                // 功能：滚动到底部时加载分页
                this._appending = false;
                this._isLastPage = false;
                el.find('ul.dropdown-menu').on('scroll', function(e){
                    var op = self.options;
                    if(this.clientHeight + this.scrollTop >= this.scrollHeight){
                        // 滚动到底部，开始加载
                        if(self._appending || self._isLastPage){
                            // 如果已经在加载，不请求
                            // 如果到达最后一页，不再请求
                            return false;
                        }
                        self._appending = true;
                        self._currPage++;
                        op.para.curr_page = self._currPage;
                        now.cmd(op.data, op.para, function(ret){
                            if(($.isArray(ret.rows) && ret.rows.length === 0) || $.isEmptyObject(ret.rows)){
                                self._isLastPage = true;
                                return;
                            }

                            $('.dropdown-menu', el).append(self._getList(ret.rows));
                            self._data.nowData = concat(self._data.nowData, ret.rows);
                            self._appending = false;
                        }, function(){
                            self._appending = false;
                        }, false);
                    }
                });

                function concat(target, src){
                    return $.isArray(src) && $.isArray(target) ? target.concat(src) : $.extend(target, src);
                }

                this.reload();
            },
            _isInViewZone: function ($el) {
                //判断元素是否在可视范围内
                var offTop = $el.offset().top,
                    widowHeight = $(window).height(),
                    documentScroolTop = $(document).scrollTop();

                if (offTop >= documentScroolTop && offTop <= documentScroolTop + widowHeight) {
                    return true;
                }
                else {
                    return false;
                }
            },
            _setLiActive: function (index) {
                //设置明细数据项Active
                var el = this.$element,
                    subLength = $('.nui-drop-li:visible', el).length,
                    $obj = $('.nui-drop-li', el),
                    $objVisible = $('.nui-drop-li:visible', el);

                if (index >= 0 && index < subLength) {
                    $obj.removeClass('active');
                    $($objVisible[index]).addClass('active');
                    return false;
                }
                else {
                    return true;
                }
            },
            reload: function (ttl) { //刷新显示
                var self = this,
                     op = this.options,
                     para = op.para;
                // 重置状态
                para.curr_page = this._currPage = 1;
                this._isLastPage = false;
                this._appending = false;
                if (typeof op.data == 'string' && op.data != '') {
                    if (op.defVal != '') {
                        para['dval'] = op.defVal;
                        op.defVal = '';
                    }
                    now.cmd(op.data, para, function (ret) {
                        self._getBody(ret.rows);
                        if (ttl != 'search') {
                            op.callBack(ret.rows);
                        }
                    }, null, false);
                } else if (op.data != '') {
                    self._getBody(op.data);
                    if (op.data && ttl != 'search') {
                        op.callBack(op.data);
                    }
                }
            },
            settext: function (text) { //手动设置显示的文本
                var el = this.$element;
                $('.dropdown-toggle', el).find('span').html(text);
            },
            select: function (id) { //选中某个值
                var el = this.$element;
                $('li.nui-drop-li', el).each(function (i, n) {
                    if ($(this).attr('id') == id) {
                        $('.dropdown-toggle', el).find('span').html($(this).html());
                        $('.dropdown-toggle', el).find('input').val($(this).attr('id'));

                        $('#dropdown_con', el).hide();

                        $('.nui-drop-li', el).removeClass('active');
                        $(this).addClass('active');

                        el.trigger('select', id);

                        return;
                    }
                });
            },
            _event: function (type, fid) {

                var self = this,
                     data = this._data.nowData,
                     _fid = self.options.fid;


                var search = function (json) {
                    var ret = '';
                    $.each(json, function (idx, node) {
                        if (ret) return;
                        if (node[_fid] == fid) {
                            ret = node;
                        }
                    });
                    return ret;
                };

                var selected = search(data);
                if (selected) {
                    var e = $.Event(type);
                    self.$element.trigger(e, selected);
                } else {
                    var e = $.Event(type);
                    self.$element.trigger(e, {});
                }
            },
            _getBody: function (rows) {//得到主体数据
                this._data.nowData = rows;
                var html = '',
                     el = this.$element,
                     op = this.options;
                if (op.selectAll != ''&&(rows.length>0 || !$.isEmptyObject(rows))) {
                    html += '<li class="nui-drop-li"><a href="javascript:;">' + op.selectAll + '</a></li>';
                }

                html += this._getList(this._data.nowData);

                $('.dropdown-menu', el).html(html);

                if(op.reloadOk){
                    //默认抛出reloadOk事件
                    setTimeout(function () {
                        el.trigger($.Event('reloadOk'), rows);
                    }, 1000);
                }

            },
            _getList: function(rows){
                var html = '',
                    el = this.$element,
                    op = this.options,
                    _i = 0;
                $.each(rows, function (key, value) {
                    if (op.isSetDef && _i == 0 && $('.dropdown-menu', el).html() == '') {
                        //第一次的时候设置第一个值为默认值
                        $('.dropdown-toggle', el).find('span').html(value.name);
                        $('.dropdown-toggle', el).find('input[name=' + op.fid + ']').val(value[op.fid]);
                        html += '<li class="nui-drop-li active" id="' + value[op.fid] + '" title="' + value.name + '"><a href="javascript:;">' + value.name + '</a></li>';
                    } else {
                        html += '<li class="nui-drop-li" id="' + value[op.fid] + '" title="' + value.name + '"><a href="javascript:;">' + value.name + '</a></li>';
                    }
                    _i++;
                });
                return html;
            },
            setDef: function (id) {
                var el = this.$element;
                if( id === ''){
                    return;
                }
                $('.dropdown-toggle', el).find('span').html($('#' + id,el).html());
                $('.dropdown-toggle', el).find('input').val($('#' + id,el).attr('id'));

                $('#dropdown_con', el).hide();

                $('.nui-drop-li', el).removeClass('active');
                $('#' + id, el).addClass('active');
            },
            setByIdx: function (idx) {
                var self = this;
                var el = this.$element
                    , id = $('.dropdown-menu li', el).eq(idx).attr('id');
                this.setDef(id);
                self._event('select', id);
            },
            setByText: function (text) {
                var self = this
                    , el = self.$element;

                $('.dropdown-menu li', el).each(function (i, n) {
                    if ($(this).text() == text) {
                        var id = $(this).attr('id');
                        if (id && id != '') {
                            self.setDef(id);
                        } else {
                            self.settext(text);
                            $('.dropdown-toggle', el).find('input').val('');
                            self._event('select', '');
                        }
                        return false;
                    }
                });
            },
            getVal: function () {
                var self = this
                    , op = this.options
                    , el = self.$element
                    , dropValue = '';

                dropValue = el.find('input[name=' + op.name + ']').val();

                return dropValue;
            },
            getText: function() {
                var self = this,
                    el = self.$element,
                dropText = $('.dropdown-toggle', el).find('span').text();
                return dropText;
            },
            clear: function () {
                var el = this.$element;

                $('#drop_result', el).val('');
                $('.dropdown-toggle', el).find('span').html('请选择');

                //清空下拉数据
                //$("#dropdown_con ul",el).empty();
            },
            clearAll: function () {
                var el = this.$element;

                $('#drop_result', el).val('');
                $('.dropdown-toggle', el).find('span').html('请选择');

                //清空下拉数据
                $("#dropdown_con ul",el).empty();
            }
        };

        $.fn.drop = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this)
                    , data = $this.data('drop')
                    , os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.drop.defaults, typeof option == 'object' && option);
                    $this.data('drop', (data = new Drop(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.drop.defaults = {
            data: '',
            para: {},
            name: 'searchslt', //下拉选择控件的名称
            fid: 'id', //数据id名称
            width: 240, //组件宽度
            listWidth: 233, //下拉列表的宽度
            hide_search: false, //隐藏搜索
            keyword: 'keyword', //搜索关键字参数
            isSetDef: false, //是否设置第一个为默认值
            defVal: '', //默认值  //服务端获取参数名为dval
            css: 'nui-drop',
            val: '请选择',
            selectAll: '',
            callBack: function (para) {
            },//加载完成后回调的方法
            reloadOk : true,//默认抛出reloadOk事件
            frmCheck : '',//表单校验类型，比如：must必填，must int-0必填并且是整数
            minWidth: 0 //默认显示的最小宽度
        };
    }(jQuery);


    /**
     * 可自动匹配的多选下拉框
     * @金宁宝
     */
    !function ($) {
        var MutiDrop = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']).addClass('_clear').css(options.style);
            this._data = {load: false};
            this._init();
        };

        MutiDrop.prototype = {
            constructor: MutiDrop,
            _init: function () { //初始化Drop内容
                var self = this,
                     el = this.$element,
                     op = this.options,
                     selectData = this._data,
                     str = '';

                var maxHeight = 24 * op.showLine + 14;
                str += '<div class="dropdown drop-multi">';
                str += '	<div class="dropdown-toggle" data-toggle="dropdown" href="javascript:;" style="cursor: pointer">';
                str += '		<ul class="drop-choices" style=" min-height: 32px;max-height: '+maxHeight+'px;"><li class="search-field"><input class="default" type="text" style="width: 233px;" autocomplete="off" value="请点击选择" readonly=true></li></ul>';
                str += '		<input id="mutidrop_result" type="hidden" name="' + op.name + '" value="" class="_muti-drop-value" check="'+op.frmCheck+'">';
                str += '	</div>';
                str += '	<div id="mutidrop_con" style="display:none">';
                if (op.search) {
                    str += '	<div class="dropdown_search"><input id="mutiDropSearch" type="text"><a class="ico_search" href="javascript:;">&nbsp;</a></div>';
                }
                str += '	<ul class="dropdown-menu mutidrop-search" style="max-height:260px;overflow-y:auto;">';
                str += '	</ul></div>';
                str += '</div>';

                el.html(str);

                el.click(function (e) {
                    e ? e.stopPropagation() : event.cancelBubble = true;
                });
                $(document).click(function () {
                    $('#mutidrop_con', el).hide();
                    $('#mutiDropSearch', el).val('');
                    self._resetList();
                });

                //选中内容
                el.on('click', 'li.nui-drop-li', function () {

                    var result = $('#mutidrop_result', el).val();

                    if (op.max_slt > 0 && result != '') {
                        var ret_arr = result.split(',');
                        if (ret_arr.length >= op.max_slt) {
                            $.alertMsg('最多只能选择' + op.max_slt + '个');
                            return;
                        }
                    }

                    var currId = $(this).attr('id');
                    var currName = $(this).find('a').html();
                    if (result == '') {
                        $('#mutidrop_result', el).val(currId);
                    } else {
                        $('#mutidrop_result', el).val(result + ',' + currId);
                    }

                    var addLi = '<li class="search-choice"><span>' + currName + '</span><span rel="1" class="search-choice-close" del_id="' + currId + '" href="javascript:;"></span></li>';
                    $('.drop-choices', el).append(addLi);

                    $('.drop-choices', el).scrollTop($('.drop-choices', el)[0].scrollHeight);

                    $('#mutidrop_con', el).hide();
                    $('#mutiDropSearch', el).val('');
                    self._resetList();
                    $('.search-field', el).hide();
                    self._event('select', $(this).attr('id'));
                });

                //删除已选择的内容
                el.on('click', 'span.search-choice-close', function (e) {
                    var del_id = $(this).attr('del_id');

                    var result = $('#mutidrop_result', el).val();
                    result = result.replace(del_id + ',', '').replace(',' + del_id, '').replace(del_id, '');
                    $('#mutidrop_result', el).val(result);

                    $(this).parent().remove();
                    $('#' + del_id, el).show();

                    if (result == '') {
                        $('.search-field', el).show();
                    }

                    self._event('delete', del_id);

                    e.stopPropagation();
                });

                el.on('click', '.dropdown-toggle', function (e) {
                    $('#mutidrop_con', el).show();
                    $('ul.dropdown-menu', el).show();
                });

                //绑定搜索框
                el.on('keyup', '#mutiDropSearch', function (event) {
                    var keyword = $.trim($(this).val()),
                        selected = el.find("span.search-choice-close"),
                        seIds = "";

                    $.each(selected, function (i, se) {
                        seIds += $(se).attr("del_id") + ",";
                    });

                    if (keyword == '') {
                        $(this).val('');
                        self._resetList();
                        return false;
                    }
                    $.each(selectData.nowData, function (key, val) {
                        if (val.name.indexOf(keyword) >= 0 && seIds.indexOf(val[op.fid]) < 0) {
                            $('#' + val[op.fid], el).show();
                        } else {
                            $('#' + val[op.fid], el).hide();
                        }
                    });
                });


                /*
                el.on('keydown', '#dropSearch', function (event) {
                    var keyword = $.trim($(this).val());
                    //绑定回车
                    if (event.keyCode == "13") {
                        op.para[op.name] = keyword;
                        $('li.nui-drop-li', el).remove();
                        el.mutidrop('reload');
                        return false;
                    }
                });
                */

                el.on('keydown', '#mutiDropSearch', function (event) {
                    //绑定回车
                    if (event.keyCode == "13") {
                        op.para[op.keyword] = $.trim($(this).val());
                        $('li.nui-drop-li', el).remove();
                        el.mutidrop('reload');
                        return false;
                    }
                });

                el.on('click', 'a.ico_search', function(){
                    op.para[op.keyword] = $.trim(el.find('#mutiDropSearch').val());
                    $('li.nui-drop-li', el).remove();
                    el.mutidrop('reload');
                    self._appending = false;
                    self._isLastPage = false;
                    return false;
                });

                // 功能：滚动到底部时加载分页
                this._appending = false;
                this._isLastPage = false;
                el.find('ul.dropdown-menu').on('scroll', function(e){
                    var op = self.options;
                    if(this.clientHeight + this.scrollTop >= this.scrollHeight){
                        // 滚动到底部，开始加载
                        if(self._appending || self._isLastPage){
                            // 如果已经在加载，不请求
                            // 如果到达最后一页，不再请求
                            return false;
                        }
                        self._appending = true;
                        self._currPage++;
                        op.para.curr_page = self._currPage;
                        now.cmd(op.data, op.para, function(ret){
                            if(($.isArray(ret.rows) && ret.rows.length === 0) || $.isEmptyObject(ret.rows)){
                                self._isLastPage = true;
                                return;
                            }

                            $('.dropdown-menu', el).append(self._getList(ret.rows));
                            self._data.nowData = concat(self._data.nowData, ret.rows);
                            self._appending = false;
                        }, function(){
                            self._appending = false;
                        }, false);
                    }
                });
                function concat(target, src){
                    return $.isArray(src) && $.isArray(target) ? target.concat(src) : $.extend(target, src);
                }

                this.reload();
            }, _resetList: function () {//重置列表，隐藏已选项
                var self = this,
                    el = self.$element,
                    op = self.options,
                    selected = el.find("span.search-choice-close"),
                    select = el.find("li.nui-drop-li"),
                    seIds = "";

                $.each(selected, function (i, se) {
                    seIds += $(se).attr("del_id") + ",";
                });

                $.each(select, function (key, se) {
                    var id = se.id;
                    if (seIds.indexOf(id) >= 0) {
                        $('#' + id, el).hide();
                    } else {
                        $('#' + id, el).show();
                    }
                });
            }, reload: function (ttl) { //刷新显示
                var self = this,
                     op = this.options,
                     para = op.para;
                // 重置状态
                para.curr_page = this._currPage = 1;
                //刷新重新加载先清空原有数据
                //self.clear();

                if (typeof op.data == 'string' && op.data != '') {
                    now.cmd(op.data, para, function (ret) {
                        self._getBody(ret);
                        op.callBack(ret.rows);
                    });
                } else if(typeof op.data == 'object' && !$.isEmptyObject(op.data)){
                    self._getBody(op.data);
                    op.callBack(op.data);
                }
            }, settext: function (text) { //手动设置显示的文本
                var el = this.$element;
                $('.dropdown-toggle', el).find('div').html(text);
            }, _event: function (type, fid) {

                var self = this,
                     data = this._data.nowData,
                     _fid = self.options.fid;


                var search = function (json) {
                    var ret = '';
                    $.each(json, function (idx, node) {
                        if (ret) return;
                        if (node[_fid] == fid) {
                            ret = node;
                        }
                    });
                    return ret;
                };

                var selected = search(data);

                if (selected) {
                    var e = $.Event(type);
                    self.$element.trigger(e, selected);
                }

                self._resetList();
            }, _getBody: function (ret) {//得到主体数据
                this._data.nowData = ret.rows;
                var html = '',
                     el = this.$element,
                     op = this.options,
                     _result;

                _result = $('#mutidrop_result', el).val();
                $.each(this._data.nowData, function (key, value) {
                    if (_result.indexOf(value[op.fid]) == -1) {
                        html += '<li class="nui-drop-li" id="' + value[op.fid] + '"><a href="javascript:;">' + value.name + '</a></li>';
                    } else {
                        html += '<li class="nui-drop-li hide" id="' + value[op.fid] + '"><a href="javascript:;">' + value.name + '</a></li>';
                    }
                });

                $('.dropdown-menu', el).append(html);
            },_getList: function(rows){
                var html = '',
                    el = this.$element,
                    op = this.options,
                    _i = 0;
                $.each(rows, function (key, value) {
                    if (op.isSetDef && _i == 0 && $('.dropdown-menu', el).html() == '') {
                        //第一次的时候设置第一个值为默认值
                        $('.dropdown-toggle', el).find('span').html(value.name);
                        $('.dropdown-toggle', el).find('input[name=' + op.fid + ']').val(value[op.fid]);
                        html += '<li class="nui-drop-li active" id="' + value[op.fid] + '" title="' + value.name + '"><a href="javascript:;">' + value.name + '</a></li>';
                    } else {
                        html += '<li class="nui-drop-li" id="' + value[op.fid] + '" title="' + value.name + '"><a href="javascript:;">' + value.name + '</a></li>';
                    }
                    _i++;
                });
                return html;
            }, setDef: function (ret) {
                var el = this.$element,
                    str = '',
                    _defData = [],
                    _i = 0;

                $('li.search-choice', el).remove();
                $('#mutidrop_result', el).val('');
                $('li.nui-drop-li', el).show();

                if (!$.isEmptyObject(ret)) {
                    $('.search-field', el).hide();

                    $.each(ret, function (key, val) {
                        val = (typeof val === 'object' && typeof (val.name) === 'string') ? val.name : val;
                        str += '<li class="search-choice"><span>' + val + '</span><span rel="1" class="search-choice-close" del_id="' + key + '" href="javascript:;"></span></li>';
                        $('#' + key, el).hide();
                        _defData[_i++] = key;
                    });

                    $('.drop-choices', el).append(str);
                    $('#mutidrop_result', el).val(_defData.join());
                } else {
                    $('.search-field', el).attr('style', 'display: list-item;');
                }
            }, setByIdx: function (arr) {
                var el = this.$element
                    , jsonData = {};
                $.each(arr, function (key, val) {
                    $('.dropdown-menu li', el).each(function (i, n) {
                        if ($(this).attr('id') == val) {
                            jsonData[val] = $(this).find('a').text();

                        }
                    });
                });
                this.setDef(jsonData);
            }, setByText: function (arr) {
                var el = this.$element
                    , jsonData = {};
                $.each(arr, function (key, val) {
                    $('.dropdown-menu a', el).each(function (i, n) {
                        var text = $(this).text();
                        if (text == val) {
                            jsonData[$(this).parent().attr('id')] = text;
                        }
                    });
                });
                this.setDef(jsonData);
            }, clear: function (para) {
                var el = this.$element;

                $('li.search-choice', el).remove();
                $('#mutidrop_result', el).val('');
                $('li.nui-drop-li', el).show();
                $('li.search-field', el).attr('style', 'display: list-item;');
            }, clearAll: function () {
                var el = this.$element;

                $('li.search-choice', el).remove();
                $('#mutidrop_result', el).val('');
                $('li.nui-drop-li', el).show();
                $('li.nui-drop-li', el).remove();//删除选项
                $('li.search-field', el).attr('style', 'display: list-item;');
            }
        };

        $.fn.mutidrop = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('mutidrop'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.mutidrop.defaults, typeof option == 'object' && option);
                    $this.data('mutidrop', (data = new MutiDrop(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.mutidrop.defaults = {
            data: '',
            para: {},
            name: 'keyword', //下拉选择控件的名称
            fid: 'id', //数据id名称
            keyword : 'keyword', //搜索关键字
            search: false, //隐藏搜索
            max_slt: 0,//最多可以选择几个
            showLine : 7,
            frmCheck : '',//表单校验类型，比如：must必填，must int-0必填并且是整数
            css: 'nui-mutidrop',   //样式类
            style : {width:'233px;'},             //样式（覆盖样式类）
            callBack: function (data) {}    //数据加载完成的回调函数
        };
    }(jQuery);


    /**
     * 下拉框
     * @金宁宝
     */
    !function ($) {
        var DropSelect = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._data = {load: false};
            this._init();
        };

        DropSelect.prototype = {
            constructor: DropSelect, _init: function () { //初始化Drop内容
                this.reload();
            }, reload: function (ttl) { //刷新显示
                var self = this,
                     op = this.options;

                if (typeof op.data == 'string' && op.data != '') {
                    now.cmd(op.data, op.para, function (ret) {
                        self._getBody(ret.rows, ret.selected);
                    });
                } else {
                    self._getBody(op.data, op.selected);
                }
            }, _getBody: function (rows, sel_val) {//得到主体数据
                this._data.nowData = rows;
                var option = '',
                     el = this.$element,
                     op = this.options,
                     selected = 'selected';

                $.each(this._data.nowData, function (key, value) {
                    selected = sel_val===key ? 'selected' : '';
                    option += '<option value="' + key + '" '+selected+'>' + value + '</option>';
                });

                el.html(option);

                if(op.reloadOk){
                    //默认抛出reloadOk事件
                    el.trigger($.Event('reloadOk'), {});
                }
            }
        };

        $.fn.dropselect = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('dropselect'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.dropselect.defaults, typeof option == 'object' && option);
                    $this.data('dropselect', (data = new DropSelect(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.dropselect.defaults = {
            data: '',
            para: {},
            selected : '',
            fid: 'id', //数据id名称
            reloadOk : true,//默认抛出reloadOk事件
            width: 240 //组件宽度
        };
    }(jQuery);

    /**
     * 简单的所见即所得编辑器，使用的是kindeditor 属性：
     * simple = true/false 默认true，默认为简单编辑器
     * data = 数据内容
     *
     * 命令：
     */
    !function ($) {
        var Editor = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options.css).attr('_nui_dispose', 'editor');
            this.ubb = require('ubb2');
            if (!window.KindEditor) {
                /*
                 $.ajax({
                 url : options.editorJsPath ? options.editorJsPath : now.cfg.nowjs + 'ubb.js',
                 type : "GET",
                 async : false,
                 dataType : "script"
                 });*/
                $.ajax({
                    url: options.editorJsPath ? options.editorJsPath : now.cfg.nowjs + 'kindeditor/kindeditor.all.js',
                    type: "GET",
                    async: false,
                    dataType: "script"
                });
            }
            this._init();
        };

        Editor.prototype = {
            constructor: Editor, _init: function () {
                var self = this,
                     el = self.$element,
                     op = this.options;
                if (el.attr('id')) {
                    op.selector = '#' + el.attr('id');
                }
                self.dispose();
                this.options.keOpt.afterCreate = function(){// 对于webkit粘贴图片的特殊处理,以及后台的上传
                    var doc = this.edit.doc;
                    var pastePicJson = op.pastePicJson;
                    var _isUploadPastePic = false;
                    if(pastePicJson.cmd  && pastePicJson.path_kind ){
                        _isUploadPastePic = true;
                    }

                    // webkit内核 图片粘贴
                    if(KindEditor.WEBKIT){
                        $(doc.body).bind('paste', function(ev){
                            var original =  ev.originalEvent;
                            var file =  original.clipboardData.items[0].getAsFile();
                            var reader = new FileReader();
                            reader.onload = function (evt){
                                var result = evt.target.result;
                                var html = '<img src="' + result + '" alt="" />';
                                if(_isUploadPastePic){// 后台存储
                                    now.cmd(pastePicJson.cmd, {path_kind:pastePicJson.path_kind,rich_content:html}, function (resp) {
                                        self.editor.insertHtml(resp.html);
                                    });
                                }else{// // 没有配置粘贴图片的存储,图片不能粘贴
                                    //self.editor.insertHtml(html);
                                }

                            };
                            reader.readAsDataURL(file);
                        });
                    }

                    // GECKO内核火狐浏览器等
                    if(KindEditor.GECKO) {
                        KindEditor(doc.body).bind('paste', function(ev){
                            setTimeout(function() {
                                var html = self.getData();
                                if(html.search(/<img src="data:.+;base64,/) > -1){
                                    if(_isUploadPastePic) {// 后台存储
                                        now.cmd(pastePicJson.cmd, {path_kind:pastePicJson.path_kind,rich_content: html}, function (resp) {
                                            self.setData(resp.html);
                                        });
                                    }else{// 没有配置粘贴图片的存储,图片不能粘贴
                                        var reg = new RegExp("(<img src=\"data:.+;base64,\.*?\>)");
                                        html = html.replace(reg,'');
                                        self.setData(html);
                                    }
                                }
                            }, 10);
                        });
                    }
                };
                if (op.isSync) {
                    var afterChangeFunc = this.options.keOpt.afterChange;
                    this.options.keOpt.afterChange = function () {
                        if(typeof afterChangeFunc === 'function'){
                            afterChangeFunc();
                        }

                        if (self.editor && self.ubb) {
                            self.getUbb();
                        }
                    };
                }
                if (op.width != '') {
                    this.options.keOpt.width = op.width;
                }
                if (op.height != '') {
                    this.options.keOpt.height = op.height;
                }
                if (op.items != '') {
                    this.options.keOpt.items = op.items;
                }
                self.editor = KindEditor.create(
                    op.selector,
                    this.options.keOpt
                );
                self.editor.html(op.data);
//                self.editor.html(self.ubb.ubb2html(op.data));
            },
            dispose: function () {
                if (self.editor) {
                    KindEditor.remove(this.options.selector);
                }
            }, exec: function (cmd, para) {
                if (this.editor) {
                    return this.editor[cmd](para);
                } else {
                    return '';
                }
            }, getUbb: function () {
                var self = this,
                    el = self.$element;
                el.val(self.editor.html());
                //$(op.selector).val(self.ubb.html2ubb(self.editor.html()));
            }, setData: function (data) {
                var self = this
                    , op = this.options;

                op.data = data;

                //self.editor.html(self.ubb.ubb2html(data));
                self.editor.html(data);
            }, getData: function () {
                var self = this;
                return self.editor.html();
                //return self.ubb.html2ubb(self.editor.html());
            }
        };

        $.fn.editor = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('editor'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.editor.defaults, typeof option == 'object' && option);
                    $this.data('editor', (data = new Editor(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, typeof option == 'object' && option);
                    data._init();
                }
                if (os) {
                    if ($.isFunction(para)) {
                        if (data[option]) {
                            para(data[option]({}));
                        } else {
                            para(data['exec'](option));
                        }
                    } else if ($.isFunction(cb)) {
                        if (data[option]) {
                            cb(data[option](para));
                        } else {
                            cb(data['exec'](option, para));
                        }
                    } else {
                        if (data[option]) {
                            data[option](para);
                        } else {
                            data['exec'](option, para);
                        }
                    }
                }
            });
        };

        $.fn.editor.defaults = {
            keOpt: {
            	minWidth: 300,
                resizeType: 1,
                allowPreviewEmoticons: false,
                allowImageUpload: false,
                themesPath: now.cfg.nowjs + 'kindeditor/themes/',
                items: [
                    'fontname', 'fontsize', '|', 'forecolor', 'hilitecolor', 'bold', 'italic', 'underline',
                    'removeformat', '|', 'justifyleft', 'justifycenter', 'justifyright', '|', 'image', 'link']
            },
            data: '',
            css: 'nui-editor',
            //值是否同步
            isSync: true,
            items: '',
            pastePicJson:{},
            width: '500',
            height: '300'
        };
    }(jQuery);

    /**
     *
     * 下划线导航
     *
     * tabbar
     * @张章湫
     * @param data [{id:id,text:text}]
     */
    !function ($) {
        var Tabpic = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._data = {load: false};
            this._init();
        };

        Tabpic.prototype = {
            constructor: Tabpic, _init: function () { //初始化Tabbar内容
                var self = this,
                     el = this.$element;
                el.on('click', 'a', function () {
                    var $this = $(this),
                         nid = $this.attr('_id');

                    if ($this.parent().hasClass('active')) {
                        self._event('select', self._getRow(nid));
                    } else {
                        $('.active', el).removeClass('active');
                        $this.parent().addClass('active');

                        var row = self._getRow(nid);
                        self._event('select', row);
                        self._event('change', row);
                    }
                });

                this.reload();

            }, reload: function () { //刷新显示
                var self = this,
                     op = this.options;

                self._getBody(op.data);

            }, _getBody: function (rows) {//得到主体数据
                this._data.nowData = rows;
                var html = '',
                     el = this.$element;

                html += '<ul>';
                $.each(this._data.nowData, function (key, value) {
                    html += '<li class="' + (key == 0 ? 'active' : '') + '"><a _id="' + value.id + '" href="#">' + value.text + '</a><b></b></li>';
                });
                html += '</ul>';

                el.html(html);

            }, _event: function (type, row) {
                var self = this,
                     e = $.Event(type);
                self.$element.trigger(e, row);

            }, _getRow: function (id) {
                var sdata = this._data.nowData;
                for (var i = 0; i < sdata.length; i++) {
                    if (sdata[i].id == id) {
                        return sdata[i];
                    }
                }
                return null;
            }
        };

        var Tabbar = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._data = {load: false};
            this._init();
        };

        Tabbar.prototype = {
            constructor: Tabbar, _init: function () { //初始化Tabbar内容
                var self = this,
                     el = this.$element;
                el.on('click', 'li', function () {
                    var $this = $(this),
                         nid = $this.attr('_id');

                    if ($this.hasClass('active')) {
                        self._event('select', self._getRow(nid));
                    } else {
                        $('.active', el).removeClass('active');
                        $this.addClass('active');
                        self._setline();

                        var row = self._getRow(nid);
                        self._event('select', row);
                        self._event('change', row);
                    }
                });

                this.reload();

            }, reload: function () { //刷新显示
                var self = this,
                     op = this.options;

                self._getBody(op.data);

            }, _getBody: function (rows) {//得到主体数据
                this._data.nowData = rows;
                var self = this,
                     html = '',
                     el = this.$element;

                html += '<ul class="tabbar tabbar-pills ">';
                $.each(this._data.nowData, function (key, value) {
                    html += '<li _id="' + value.id + '" class="' + (key == 0 ? 'active' : '') + '"><span>' + value.text + '</span></li>';
                });
                html += '</ul><br/>';
                html += '<div class="line-sel" style="margin-left:0px;width:0px;"></div>';
                html += '<div class="line"></div>';

                el.html(html);

                this._inipos = $('.line', el).position();
                self._setline();

            }, _setline: function () {
                var el = this.$element,
                     $li = $('.active', el),
                     pos = $li.position(),
                     width = $li.width(),
                     width_line = width * parseInt(this._data.nowData.length),
                     tleft = $('.line', el).position().left;
                $('.line-sel', el).css({'margin-left': pos.left - tleft, 'width': width});
                $('.line', el).css({'width': width_line});

            }, _event: function (type, row) {
                var self = this
                    , e = $.Event(type);
                self.$element.trigger(e, row);

            }, _getRow: function (id) {
                var sdata = this._data.nowData;
                for (var i = 0; i < sdata.length; i++) {
                    if (sdata[i].id == id) {
                        return sdata[i];
                    }
                }
                return null;
            }
        };

        $.fn.tabbar = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this)
                    , data = $this.data('tabbar')
                    , os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.tabbar.defaults, typeof option == 'object' && option);
                    if (options.kind == 'tabbar') {
                        $this.data('tabbar', (data = new Tabbar(this, options)));
                    } else {
                        $this.data('tabbar', (data = new Tabpic(this, options)));
                    }
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.tabbar.defaults = {
            data: [],
            kind: 'tabbar',//tabbar,tabpic
            css: 'tabbar'//tabbar,tabpic
        };
    }(jQuery);

    /**
     * menulist
     * @张章湫
     * @param data [{id:id,text:text,_child:[]}]
     */
    !function ($) {
        var Menulist = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._data = {load: false};
        };

        Menulist.prototype = {
            constructor: Menulist,
            _init: function () { //初始化Tabbar内容
                var self = this,
                     op = this.options,
                     el = this.$element;

                el.on('click', 'div', function () {
                    var $i = $(this).find('i');
                    if (op.quickToId) {
                        //快速导航
                        $('#' + op.quickToId).quickto('set', $(this).attr('_id'));
                    }
                    if (op.closeOthers && $i.hasClass('icon-chevron-right')) {
                        $(this.parentNode).find('ul').hide();
                        $('div > i', el).removeClass('icon-chevron-down').addClass('icon-chevron-right');
                    }
                    self._selDiv(this);

                    // 添加激活的class
                    $('div',el).removeClass('active');
                    $(this).addClass('active');

					// fix 导航 效果 	
					 var pz =  $(this).index('.main-left-ul div');
					 el.find('div').removeClass('bor-b-f7f7');
					 el.find('div').eq(pz-1).addClass('bor-b-f7f7');
                });

                el.on('click', 'li', function () {
                    if (op.quickToId) {
                        //快速导航
                        $('#' + op.quickToId).quickto('set', $(this).attr('_id'));
                    }
                    self._selLi(this);
                });

                this.reload();

                $(window).on('resize', function(){
                    var pHeight = el.parent().parent().height();
                    el.parent().height(pHeight);
                }).trigger('resize');
            },
            _selDiv: function (div) {
                var self = this,
                     $this = $(div),
                     nid = $this.attr('_id'),
                     child = $this.attr('child'),
                     el = this.$element;

                if (child == 1) {
                    var $i = $this.find('i');
                    if ($i.hasClass('icon-chevron-right')) {
                        $i.removeClass('icon-chevron-right').addClass('icon-chevron-down');
                        $this.next().show();
                    } else {
                        $i.removeClass('icon-chevron-down').addClass('icon-chevron-right');
                        $this.next().hide();
                    }
                } else {
                    $('li.active', el).removeClass('active');
                    $('div > span').removeClass('nochild');
                    $this.addClass('nochild');

                    self._event('select', self._getRow(nid));
                }

            },
            _selLi: function (li) {
                var self = this,
                     $this = $(li),
                     $div = $this.parent().prev(),
                     id = $this.attr('_id'),
                     el = this.$element;

                var $i = $div.find('i');
                if ($i.hasClass('icon-chevron-right')) {
                    $i.removeClass('icon-chevron-right').addClass('icon-chevron-down');
                    $div.next().show();
                }

                $('li.active', el).removeClass('active');
                $this.addClass('active');

                $('div.active', el).removeClass('active');
                $this.closest('ul').prev('div:first').addClass('active');

                $('div', el).removeClass('nochild');

                self._event('select', self._getRow(id));

            },
            select: function (id) {//手动打开某个节点可选
                var self = this,
                     el = this.$element,
                     find = false;

                // 移除激活状态
                $('div',el).removeClass('active');

                $('div', el).each(function (i, div) {
                    if (id == $(this).attr('_id')) {
                        find = true;
                        self._selDiv(div);

                        // 添加激活的class
                        $(this).addClass('active');

                        return false;
                    }
                });

                if (!find) {
                    $('li', el).each(function (i, li) {
                        if (id == $(this).attr('_id')) {
                            self._selLi(li);
                            return false;
                        }
                    });
                }
            },
            reload: function () { //刷新显示
                var self = this,
                     op = this.options,
                     para = op.para;

                if (typeof op.data == 'string' && op.data != '') {
                    now.cmd(op.data, para, function (ret) {
                        self._getBody(ret.o.rows);
                        self._event('load', ret.o.rows);
                        if (op.quickToId) {
                            $('#' + op.quickToId).quickto({
                                data: ret.o.rows
                            });
                        }
                    });
                } else {
                    self._getBody(op.data);
                    self._event('load', op.data);
                    if (op.quickToId) {
                        $('#' + op.quickToId).quickto({
                            data: op.data
                        });
                    }
                }

            },
            _getBody: function (rows) {//得到主体数据
                this._data.nowData = rows;
                var html = '',
                     el = this.$element,
                     op = this.options,
                     len = rows.length,
                     childCss = '';

                if (len > 0) {
                    $.each(this._data.nowData, function (key, value) {
                        html += '<div _id="' + value.id + '" child="' + (value._child ? 1 : 0) + '" class="_menu_'+ value.id +'">';
                        if (value._child) {
                            if ((key == 0 && op.firstOpen) || op.open) {
                                html += '<i class="icon-chevron-down"></i>';
                            } else {
                                html += '<i class="icon-chevron-right"></i>';
                            }
                        }else{
                            html += '<i></i>';
                        }
                        html += '<span>' + value.text + '</span>';
                        html += '</div>';

                        if (value._child) {
                            childCss = op.css;
                        }

                        if ((key == 0 && op.firstOpen) || op.open) {
                            html += '<ul class="nav nav-list ' + childCss + '">';
                        } else {
                            html += '<ul class="nav nav-list ' + childCss + '" style="display:none;">';
                        }
                        if (value._child) {
                            $.each(value._child, function (k, v) {

                                if (v._child) {
                                    html += '<div _id="' + v.id + '" child="' + (v._child ? 1 : 0) + '">';
                                    html += '<i class="icon-chevron-right"></i>';
                                    html += '<span>' + v.text + '</span>';
                                    html += '</div>';
                                    html += '<ul class="nav nav-list" style="display:none;">';
                                    $.each(v._child, function (sk, sv) {
                                        html += '<li _id="' + sv.id + '">';
                                        html += '<a href="javascript:;">' + sv.text + '</a>';
                                        html += '</li>';
                                    });
                                    html += '</ul>';
                                } else {
                                    html += '<li _id="' + v.id + '">';
                                    html += '<a href="javascript:;">' + v.text + '</a>';
                                    html += '</li>';
                                }
                            });
                        }
                        html += '</ul>';

                        if (key < len - 1) {
                            html += '<hr/>';
                        }
                    });
                }
                el.html(html);

            },
            _event: function (type, row) {
                var self = this,
                     e = $.Event(type);
                self.$element.trigger(e, row);

            },
            _getRow: function (id) {
                var rows = this._data.nowData;

                var search = function (sdata) {
                    var ret = '';
                    for (var i = 0; i < sdata.length; i++) {
                        if (sdata[i].id == id) {
                            ret = sdata[i];
                            break;
                        } else if (sdata[i]._child) {
                            ret = search(sdata[i]._child);
                            if (ret) {
                                break;
                            }
                        }
                    }
                    return ret;
                };

                return search(rows);
            }
        };

        $.fn.menulist = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('menulist'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.menulist.defaults, typeof option == 'object' && option);
                    $this.data('menulist', (data = new Menulist(this, options)));
                    data._init();

                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.menulist.defaults = {
            data: [],
            para: {},
            open: false,
            firstOpen: true,
            quickToId: '',//quickto组件使用
            css: 'nui-menulist',
            closeOthers: false //即允许打开的一级标签是否只有一个
        };
    }(jQuery);


    /**
     * 快速定位(您的当前位置)
     *
     * @金宁宝
     */
    !function ($) {
        var Quickto = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options['css']);
            this._init();
        };

        Quickto.prototype = {
            constructor: Quickto, _init: function () { //初始化Goto内容

                this.reload();

            }, reload: function () {
                var self = this,
                     op = self.options,
                     para = op.para;

                if (typeof op.data == 'string' && op.data != '') {
                    now.cmd(op.data, para, function (ret) {
                        op.retData = ret.rows;
                    });
                } else {
                    op.retData = op.data;
                }
            }, set: function (id) {
                var self = this,
                     el = self.$element,
                     op = self.options,
                     isOk = false,
                     str = '您当前的位置：';

                $.each(op.retData, function (key, val) {
                    if (!isOk) {
                        if (id == val.id) {
                            if (typeof(val.mdl) == 'undefined') {
                                str += val.text;
                            } else {
                                str += '<a href="javascript:;" class="_quick-to-single" _qtid="' + val.id + '">' + val.text + '</a>';
                            }
                            isOk = true;
                        } else {
                            if (val._child) {
                                $.each(val._child, function (fkey, fval) {
                                    if (!isOk) {
                                        if (id == fval.id) {
                                            if (typeof(val.mdl) == 'undefined') {
                                                str += val.text;
                                            } else {
                                                str += '<a href="javascript:;" class="_quick-to-single" _qtid="' + val.id + '">' + val.text + '</a>';
                                            }
                                            if (typeof(fval.mdl) == 'undefined') {
                                                str += ' > ' + fval.text;
                                            } else {
                                                str += ' > ' + '<a href="javascript:;" class="_quick-to-single" _qtid="' + fval.id + '">' + fval.text + '</a>';
                                            }
                                            isOk = true;
                                        } else {
                                            if (fval._child) {
                                                $.each(fval._child, function (skey, sval) {
                                                    if (id == sval.id) {
                                                        if (typeof(val.mdl) == 'undefined') {
                                                            str += val.text;
                                                        } else {
                                                            str += '<a href="javascript:;" class="_quick-to-single" _qtid="' + val.id + '">' + val.text + '</a>';
                                                        }
                                                        if (typeof(fval.mdl) == 'undefined') {
                                                            str += ' > ' + fval.text;
                                                        } else {
                                                            str += ' > ' + '<a href="javascript:;" class="_quick-to-single" _qtid="' + fval.id + '">' + fval.text + '</a>';
                                                        }
                                                        if (typeof(sval.mdl) == 'undefined') {
                                                            str += ' > ' + sval.text;
                                                        } else {
                                                            str += ' > ' + '<a href="javascript:;" class="_quick-to-single" _qtid="' + sval.id + '">' + sval.text + '</a>';
                                                        }
                                                        isOk = true;
                                                    }
                                                });
                                            }
                                        }
                                    }
                                });
                            }
                        }
                    }
                });
                el.html(str);

                $('a._quick-to-single', el).on('click', function (ret) {
                    $('body').find('li[_id=' + $(this).attr('_qtid') + ']').click();
                    $('body').find('div[_id=' + $(this).attr('_qtid') + ']').click();
                });
            }, add: function (data) {
                var self = this,
                    el = self.$element,
                    str = '',
                    html = '',
                    mdl,
                    menu;

                if (typeof(data) == 'string') {
                    html = data;
                } else {
                    html = data.text;
                    mdl = data.mdl;
                    menu = data.menu;
                }

                str = el.html();
                if (str.indexOf(html) == -1) {
                    if (typeof(data) == 'string') {
                        str += ' > ' + html;
                    } else {
                        str += ' > <a href="javascript:;" mdl="' + mdl + '" menu="' + menu + '" class="_quick_to_nqy">' + html + '</a>';
                    }
                    el.html(str);
                } else {
                    //替换掉多余的导航
                    var str_arr = str.split(' &gt; ');
                    if (str_arr[str_arr.length - 1].indexOf(html) == -1) {
                        el.html(str.replace(' &gt; ' + str_arr[str_arr.length - 1], ''));
                    }
                }

                $('a._quick-to-single', el).on('click', function (ret) {
                    $('body').find('li[_id=' + $(this).attr('_qtid') + ']').click();
                    $('body').find('div[_id=' + $(this).attr('_qtid') + ']').click();
                });
            }
        };

        $.fn.quickto = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('quickto'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.quickto.defaults, typeof option == 'object' && option);
                    $this.data('quickto', (data = new Quickto(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, option);
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                }
            });
        };

        $.fn.quickto.defaults = {
            data: '',
            para: {},
            id: '',
            css: 'quick-to'
        };
    }(jQuery);

    /**
     * Player 播放器
     * @author 张章湫
     * @param url 文件路径
     */
    !function ($) {
        var Player = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options.css);
            this.player = null;
            if (!window.Player) {
                $.ajax({
                    url: options.jsPath ? options.jsPath : now.cfg.nowjs + 'flowplayer.min.js',
                    type: "GET",
                    async: false,
                    dataType: "script"
                });
            }
            this._init();
        };

        Player.prototype = {
            constructor: Player, _init: function () {
                var self = this,
                     el = self.$element,
                     op = this.options;
                if (el.attr('id')) {
                    op.id = el.attr('id');
                }

                if (op.url != undefined) {
                    op.clip.url = op.url;
                    op.clip.provider = 'nginx';
                }

                var plugins = {
                    nginx: {
                        url: now.cfg.nowjs + 'flowplayer.pseudostreaming-3.2.12.swf'
                    }
                };
                if(op.uncontrols){
                    plugins.controls = null;
                }

                self.player = $f(
                    op.id,
                    op.swfPath ? op.swfPath : now.cfg.nowjs + 'flowplayer.swf',
                    {
                        clip: op.clip,
                        screen: op.screen,
                        canvas: op.canvas,
                        plugins: plugins
                    }
                );
            }, getPlayer: function () {
                var self = this;
                return self.player;
            }
        };

        $.fn.player = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     os = (typeof option == 'string'),
                     data = $this.data('player');

                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                } else {
                    if (!data) {
                        var options = $.extend({}, $.fn.player.defaults, typeof option == 'object' && option);
                        $this.data('player', (data = new Player(this, options)));
                    } else {
                        data.options = $.extend({}, data.options, option);
                        data._init();
                    }
                }
            });
        };

        $.fn.player.defaults = {
            //jsPath: '',
            //swfPath: '',
            //url: '',
            clip: {
                url: '',
                autoPlay: true,
                autoBuffering: true,
                accelerated: true
                //onStart: function(){},
                //onFinish: function(){}
            },
            screen: {
                top: 10,
                left: "50pct"
            },
            canvas: {
                backgroundColor: "transparent"
            },
            uncontrols: false,//是否需要菜单
            css: 'nui-player'
        };
    }(jQuery);

    /**
     * 播放文档转过来的swf
     *
     * FlexPaper
     * @author 张章湫
     * @param url 文件路径
     */
    !function ($) {
        var FlexPaper = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options.css);
            if (!window.FlexPaper) {
                $.ajax({
                    url: options.jsPath ? options.jsPath : now.cfg.nowjs + 'flexpaper.min.js',
                    type: "GET",
                    async: false,
                    dataType: "script"
                });
            }
            this._init();
        };

        FlexPaper.prototype = {
            constructor: FlexPaper, _init: function () {
                var self = this,
                     el = self.$element,
                     op = this.options;

                if (op.url != undefined) {
                    op.config.SWFFile = op.url;
                }

                if (op.startPage != undefined) {
                    op.config.StartAtPage = op.startPage;
                }

                el.FlexPaperViewer(
                    {
                        config: op.config
                    }
                );
            }
        };

        $.fn.flexPaper = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     os = (typeof option == 'string'),
                     data = $this.data('flexPaper');

                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                } else {
                    if (!data) {
                        var options = $.extend({}, $.fn.flexPaper.defaults, typeof option == 'object' && option);
                        $this.data('flexPaper', (data = new FlexPaper(this, options)));
                    } else {
                        data.options = $.extend({}, data.options, option);
                        data._init();
                    }
                }
            });
        };

        $.fn.flexPaper.defaults = {
            //jsPath: '',
            //url: '',
            //startPage: 1,
            css: 'nui-flexpaper',
            config: {
                Scale: 0.6,
                ZoomTransition: 'easeOut',
                ZoomTime: 0.5,
                ZoomInterval: 0.2,
                FitPageOnLoad: true,
                FitWidthOnLoad: true,
                FullScreenAsMaxWindow: false,
                ProgressiveLoading: false,
                MinZoomSize: 0.2,
                MaxZoomSize: 5,
                SearchMatchAll: false,
                InitViewMode: 'Portrait',
                RenderingOrder: 'flash,html',
                StartAtPage: '',
                ViewModeToolsVisible: true,
                ZoomToolsVisible: true,
                NavToolsVisible: true,
                CursorToolsVisible: true,
                SearchToolsVisible: true,
                WMode: 'Opaque',
                localeChain: 'en_US',
                onCurrentPageChanged: function (num) {
                    console.log(num);
                }
            }
        };
    }(jQuery);

    /**
     * 报表
     *
     * @author 张章湫
     * @param url 文件路径
     */
    !function ($) {
        var Chart = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options.css);
            self.chart = null;
            if (!window.FlexPaper) {
                $.ajax({
                    url: options.jsPath ? options.jsPath : now.cfg.nowjs + 'FusionCharts.min.js',
                    type: "GET",
                    async: false,
                    dataType: "script"
                });
            }
            this._init();
        };

        var swfPathMap = {
            line: 'FCF_Line.swf', // 折线图
            multiLine: 'MSLine.swf', // 折线图
            column3D: 'Column3D.swf', // 柱状图
            pie3D: 'Pie3D.swf', // 饼图
            msColumn3D: 'MSColumn3D.swf' // 多维柱状图
        };

        Chart.prototype = {
            constructor: Chart, _init: function () {
                var self = this,
                     el = self.$element,
                     op = this.options,
                     swfPath = '';

                if (el.attr('id')) {
                    op.id = el.attr('id');
                }

                swfPath = now.cfg.nowjs + swfPathMap[op.type];

                self.chart = new FusionCharts(swfPath + '?ChartNoDataText=' + op.ChartNoDataText, op.id, op.width, op.height);
                self.chart.params.wmode = 'transparent'; //兼容IE，解决flash透明层遮挡DIV问题
                if (typeof op.data == 'string' && op.data != '' && op.data.substr(0, 1) != '<') {
                    now.cmd(op.data, op.para, function (ret) {
                        self.chart.setDataXML(ret.data);
                        self.chart.render(op.id);
                        el.trigger($.Event('reloadOk'), ret);
                    });
                } else {
                    self.chart.setDataXML(op.data);
                    self.chart.render(op.id);
                    el.trigger($.Event('reloadOk'), op);
                }

            }, getChart: function () {
                var self = this;
                return self.chart;
            }
        };

        $.fn.chart = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     os = (typeof option == 'string'),
                     data = $this.data('chart');
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                } else {
                    if (!data) {
                        var options = $.extend({}, $.fn.chart.defaults, typeof option == 'object' && option);
                        $this.data('chart', (data = new Chart(this, options)));
                    } else {
                        data.options = $.extend({}, data.options, option);
                        data._init();
                    }
                }
            });
        };

        $.fn.chart.defaults = {
            width: 850,
            height: 450,
            type: 'line', //目前提供四种: line(折线图),column3D(柱状图),pie3D(饼图),msColumn3D(多维柱状图)
            data: '',
            para: {},
            css: 'nui-chart',
            ChartNoDataText: '暂无数据'
        };
    }(jQuery);

    /**
     * 报表
     *
     * @author 张章湫
     * @param url 文件路径
     */
    !function ($) {
        var _cameraList = {};
        var Camera = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options.css);
            if (this.$element.attr('id')) {
                _cameraList[this.$element.attr('id')] = this;
            }
            self.Camera = null;
            this._init();
        };

        Camera.prototype = {
            constructor: Camera, _init: function () {
                var self = this,
                    el = self.$element,
                    op = this.options;

                if (el.attr('id')) {
                    op.id = el.attr('id');
                }
                self.Camera = el.flash({
                    bgcolor: '#869ca7',
                    width: op.width,
                    height: op.height,
                    allowscriptaccess: 'sameDomain',
                    flashvars: {
                        id: op.id,
                        width: op.width,
                        height: op.height
                    },
                    src: now.cfg.nowjs + 'fs_camera.swf?_='+new Date()
                }).data('flash').getSwf();

            }, getCamera: function () {
                var self = this;
                return self.Camera;
            }, takePhoto: function () {
                var jpgData = this.Camera.takePhoto();
                return jpgData;
            }, dispose: function () {
                this.Camera = null;
            }, cameraReady: function(ret){
                this.$element.trigger($.Event('cameraReady'), ret);
            }, cameraStatusChange: function(ret){
                this.$element.trigger($.Event('cameraStatusChange'), ret);
            }
        };

        $._cameraReady = function (id, ret) {
            if (_cameraList[id]!=undefined) {
                var el = _cameraList[id];
                el.cameraReady(ret);
            }
        };

        $._cameraStatusChange = function(id,ret) {
            if (_cameraList[id]!=undefined) {
                var el = _cameraList[id];
                el.cameraStatusChange(ret);
            }
        };

        $.fn.camera = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                    os = (typeof option == 'string'),
                    data = $this.data('camera');
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                } else {
                    if (!data) {
                        var options = $.extend({}, $.fn.camera.defaults, typeof option == 'object' && option);
                        $this.data('camera', (data = new Camera(this, options)));
                    } else {
                        data.options = $.extend({}, data.options, option);
                        data._init();
                    }
                }
            });
        };

        $.fn.camera.defaults = {
            width: 265,
            height: 300,
            css: 'nui-camera'
        };
    }(jQuery);


    /**
     * 复杂列表选择组件
     * @author 金宁宝
     */
    !function ($) {
        var MultiSlt = function (element, options) {
            this.options = options;
            this.$element = $(element).addClass(options.css).html('<ul class="nav nav-list parent"><ul>');
            this._data = {};
            this._select = null;
            this._init();
        };

        MultiSlt.prototype = {
            constructor: MultiSlt, _init: function () {

                //? 参数初始化 扩展相应参数
                this.expandPara();

                var self = this,
                     el = self.$element,
                     op = this.options;

                var tmp = '';
                tmp += '<div class="modal-header">';
                tmp += '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>';
                if (op.title) {
                    tmp += '<h3>' + op.title + '</h3>';
                }
                tmp += '</div>';
                tmp += '<div class="modal-body-middle"><div class="container-fluid"><div class="row-fluid pull-left">';
                if (!$.isEmptyObject(op.tree)) {
                    tmp += '<div class="span3 well"><div id="left_tree"></div></div>';
                    tmp += '<div class="span8">';
                } else {
                    tmp += '<div class="span12">';
                }
                tmp += '	<div id="right_repeat" class="right-repeat well"></div>';
                tmp += '	<div><h4>以下为选中内容：</h4></div>';
                tmp += '	<div class="right-slt well">';
                tmp += '		<div id="right_foot_slt" ></div>';
                tmp += '	</div>';

                tmp += '</div></div></div></div>';

                //? 修改 底部按钮布局和位置
                tmp += '<div class="modal-footer-bottom"><div class="content"><button class="btn btn-primary mutilslt_submit">保存</button></div></div>';

                el.addClass('modal_multi hide fade').attr({
                    role: 'dialog',
                    'aria-hidden': 'true'
                }).html(tmp).modal({
                        backdrop: 'static',
                        keyboard: false,
                        show: true
                    });

                this.reload();
            }, reload: function () {
                var self = this,
                    el = this.$element,
                    op = this.options,
                    para = op.para;

                op.sltData = [];

                if (!$.isEmptyObject(op.tree)) {
                    $('#left_tree', el).tree(op.tree).on('select', function (e, node) {
                        if (op.tree._deep == $(this).find('a[_id=' + node[op.tree.fid] + ']').attr('_deep')) {
                            op.repeat.para[op.tree.fid] = $(this).find('a[_id=' + node[op.tree.fid] + ']').attr('_id');
                            $('#right_repeat', el).repeat({nowPage: 1, para: op.repeat.para, isSub: true}).repeat('reload');
                        }
                    });
                }

                //初始化repeat
                if (!$.isEmptyObject(op.setDef) && op.setDef.data != '') {
                    now.cmd(op.setDef.data, op.setDef.para, function (ret) {
                        if ($.isEmptyObject(ret.rows)) {
                            op.repeat_dst.para[op.repeat_dst.fid] = '';
                        } else {
                            $.each(ret.rows, function (key, val) {
                                op.sltData = self.dealData(op.sltData, val.stu_id, null);
                            });
                            op.repeat_dst.para[op.repeat_dst.fid] = op.sltData;
                        }
                        $('#right_foot_slt', el).repeat(op.repeat_dst);

                        //初始化repeat
                        op.repeat.isSub = true;
                        $('#right_repeat', el).repeat(op.repeat);

                    });
                } else {
                    //没有获取已选数据
                    if ($.isEmptyObject(op.sltData)) {
                        op.repeat_dst.para[op.repeat_dst.fid] = '';
                    } else {
                        op.repeat_dst.para[op.repeat_dst.fid] = op.sltData;
                    }
                    $('#right_foot_slt', el).repeat(op.repeat_dst);

                    //初始化repeat
                    op.repeat.isSub = true;
                    $('#right_repeat', el).repeat(op.repeat);
                }

                //right_repeat加载后的处理
                $('#right_repeat', el).on('reloadOk', function (e, ret) {
                    //处理已选中的数据样式
                    $.each(ret.rows, function (key, val) {
                        if ($.inArray(val[op.repeat.fid], op.sltData) >= 0) {
                            //已存在
                            $('#sltdata_' + val[op.repeat.fid]).css({'color': '#3399FF'});
                        } else {
                            //没选过
                            $('#sltdata_' + val[op.repeat.fid]).css({'color': ''});
                        }
                    });
                });

                //选中数据
                el.on('click', '.repeat-model-single', function () {

                    var sltdata_id = $(this).attr('id'),
                        data_id = sltdata_id.replace('sltdata_', '');

                    if ($.inArray(data_id, op.sltData) >= 0) {
                        //已存在，则删除
                        op.sltData = self.dealData(op.sltData, null, data_id);
                        $(this).css({'color': ''});
                    } else {
                        //没选过，则添加
                        op.sltData = self.dealData(op.sltData, data_id, null);
                        $(this).css({ 'color': '#999999' });
                    }

                    if ($.isEmptyObject(op.sltData)) {
                        op.repeat_dst.para[op.repeat_dst.fid] = '';
                    } else {
                        op.repeat_dst.para[op.repeat_dst.fid] = op.sltData;
                    }
                    $('#right_foot_slt', el).repeat({ nowPage: 1, para: op.repeat_dst.para }).repeat('reload');

                    //若col为1  则将所选项一列排出
                    if (op.repeat_dst["col"] == 1 && op.repeat_dst["multiCol"]) {
                        $(".right-slt .repeat-item", el).addClass("item");
                    }
                });

                //? 修改错误 已选容器 删除标签 类名为 slt-close 而不是 _slt-close
                //绑定已选容器数据的删除
                el.on('click', '.slt-close', function () {
                    var slt_id = $(this).attr(op.repeat_dst.fid);
                    $('#sltdata_' + slt_id, el).css({'color': ''});

                    //删除已选的数据id
                    op.sltData = self.dealData(op.sltData, null, slt_id);

                    if ($.isEmptyObject(op.sltData)) {
                        op.repeat_dst.para[op.repeat_dst.fid] = '';
                    } else {
                        op.repeat_dst.para[op.repeat_dst.fid] = op.sltData;
                    }
                    //$('#right_foot_slt',el).repeat({nowPage:1,para:op.repeat_dst.para}).repeat('reload');
                    $(this).parent().remove();
                });

                $('.mutilslt_submit', el).click(function () {
                    op.submit.para[op.submit.fid] = op.sltData;
                    now.cmd(op.submit.data, op.submit.para, op.submit.okFun);
                });
            }, dealData: function (data, newId, subId) {
                if (newId) {
                    if ($.inArray(newId, data) == -1) {
                        data.push(newId + '');
                    }
                } else {
                    data.splice($.inArray(subId, data), 1);
                }

                return data;
            }

            //? 扩展应用参数
            , expandPara: function () {

                this.options.repeat_dst = $.extend({}, { multiCol: true }, this.options.repeat_dst);

                var _repeat = $.extend({}, { defaultType: 'text' }, this.options.repeat),
                    _repeat_dst = $.extend({}, { defaultType: 'text' }, this.options.repeat_dst);

                var repeatDefaultType = _repeat["defaultType"],
                    repeat_dstDefaultType = _repeat_dst["defaultType"];

                if (repeatDefaultType && repeatDefaultType.length > 0) {
                    switch (repeatDefaultType) {
                        case "text": //默认文字
                            this.options.repeat = $.extend({}, repeatDefaultModal.text, this.options.repeat);
                            break;
                        default:
                            break;
                    }
                    ;
                }

                if (repeat_dstDefaultType && repeat_dstDefaultType.length > 0) {
                    switch (repeat_dstDefaultType) {
                        case "text": //默认文字
                            this.options.repeat_dst = $.extend({}, repeat_dstDefaultModal.text, this.options.repeat_dst);
                            break;
                        default:
                            break;
                    }
                    ;
                }
            }
        };

        $.fn.multislt = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                     data = $this.data('multislt'),
                     os = (typeof option == 'string');

                if (!data) {
                    var options = $.extend({}, $.fn.multislt.defaults, typeof option == 'object' && option);
                    $this.data('multislt', (data = new MultiSlt(this, options)));
                } else if (!os) {
                    data.options = $.extend({}, data.options, typeof option == 'object' && option);
                    data._init();
                }
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        return data[option](para);
                    }
                }
            });
        };

        //? repeat 与 repeat_dst 参数中各个增加一个参数 defaultType 即为 调用模板 目前仅支持 text 
        //? repeat_dst 新增 multiCol 参数 当 col = 1 且 multicol = true 时 则会多行顺序排列显示
        $.fn.multislt.defaults = {
            title: '',//标题
            tree: {},//目录树参数,参考tree组件，比如{fid:'id',pid:'pid',txt:'name',data:'base.tree.get',para:{}}
            repeat: {},//repeat参数,参考repeat组件，比如{title:'题库',col:1,row:5,fid:'sub_id',render:'main/ksgl/sub_grid',data:'tech.sub.get',para:{km_id:km_id}}
            submit: {},//保存选择数据的请求地址和额外参数，比如：{data:'',para:{},fid:'sub_id',okFun:function(){}}，fid数据提交的参数,php使用sub_id接收数据
            setDef: {data: '', para: {}, fid: ''},//填充默认数据，比如：{data:'',para:{},fid:'sub_id',repeat_dst:{}}，fid数据提交的参数,php使用sub_id接收数据
            repeat_dst: {},//目标容器
            sltData: [] //已选择数据的ID
        };

        //? repeat 默认参数模板
        var repeatDefaultModal = {
            text: { //? 文字模板
                col: 1,
                row: 5,
                fid: 'link_id',
                render: function (node) {
                    return '<span>' + node.name + '</span>';
                },
                para: {}
            }
        };

        //? repeat_dst 默认参数模板
        var repeat_dstDefaultModal = {
            text: {
                col: 1,
                row: 5,
                fid: 'link_id',
                render: function (node) {
                    return '<span class="slt-text">' + node.name + '</span><a href="javascript:;" class="slt-close slt-del" link_id="' + node.link_id + '">&times;</a>';
                },
                para: {},
                showpage: false
            }
        };

    }(jQuery);

    /**
     * 中国地图
     * 用法：
     * $('#map').chinaMap();
     * $('#map').on('selectProvince', function(e, code){
     *     // 输出选中省份的行政代码
     *     console.log(code);
     * });
     *
     * @author 张健翔
     */
    void function($){

        function paintMap(R) {
            var china = [];
            var attr = {
                "fill": "#b0d0ec",
                "stroke": "#ddd",
                "stroke-width": 1.5,
                "stroke-linejoin": "round"
            };

            china['澳门'] = {
                name: "澳门",
                code: '901200',
                path: R.path("M413.032,414.183l-0.96,1.752c0,0,0.889,0.883,1.98,1.086s1.995-0.493,1.995-0.493L413.032,414.183z").attr(attr)
            };
            china['香港'] = {
                name: "香港",
                code: '901100',
                path: R.path("M417.745,409.005l3.394,0.773l3.453-2.558l1.666,4.582c0,0-5.521,2.673-3.691,1.785c1.828-0.884-4.641-0.355-4.641-0.355l-0.834-3.454L417.745,409.005z").attr(attr)
            };
            china['台湾'] = {
                name: "台湾",
                code: '901000',
                path: R.path("M505.438,371.203l-3.217,19.169l-1.664,6.07v5.123l-1.43,1.427l-3.451-5.119l-3.693-2.858l-3.215-8.571c0,0-0.451-5.62,0.357-7.74c0.809-2.118,5.356-14.05,5.356-14.05l6.313-5.357l4.051,1.904L505.438,371.203z").attr(attr)
            };
            china['广东'] = {
                name: "广东",
                code: '440000',
                path: R.path("M391.37,382.632l2.265-1.666v-3.45l1.188-1.072l2.859,0.235l4.879,2.264l0.834-1.667l-1.188-2.025l0.354-1.427l3.098-2.619l5.478,1.427l3.096-1.905l2.264,2.265l5.478-1.429l1.07,2.021l-1.666,2.264l-2.859,4.288v1.069l1.67,1.192l11.43-4.525l4.048,2.264l1.19-1.191l-1.19-2.499l0.28-1.663l7.459,1.663l1.431,1.072l2.022-0.24l3.094,3.929l3.451,7.502l-2.26,1.667l-2.023,3.689l-1.786,0.596l-1.664,3.455l-5.716,2.854l-2.266-1.188l-1.426,2.382v0.833h-1.787h-3.098l-2.857,2.023l-2.021-1.188l-2.5,1.663l-6.314,2.619l-5.121-4.05l-0.354,3.217l1.788,4.882l-4.646,1.904l-2.498,3.452l-4.887,1.191l-2.617,1.073h-5.119c0,0-0.869,2.545-3.453,3.452c-2.584,0.904-9.168,3.213-9.168,3.213l-4.522,3.098l-2.265,2.263l4.287,6.903l-2.856,2.501l-3.451-0.238l-4.053-7.381l0.954-5.238v-2.501l2.854-4.646l3.695-0.831l-0.355-2.501l3.809-1.785l0.476-4.288l6.668-5.119l-0.355-6.905l5.119-6.547l-0.24-2.5l1.666-2.381L391.37,382.632z").attr(attr)
            };
            china['广西'] = {
                name: "广西",
                code: '450000',
                path: R.path("M305.646,387.87l3.688,0.241l4.051-3.694l1.668,1.072l7.737,3.216l1.429-0.835l0.235-2.619l1.433-1.429l10.955-7.145l1.903,2.266l6.668,2.021l3.691-4.883l1.666,1.192h2.619v-1.431l3.453-1.192v-1.069l1.43-1.428l0.832,0.236l3.453-3.451l2.856,0.832l2.857-3.931l1.432,3.099l0.832-0.238l4.525-4.288l1.188,0.24l2.021-1.073l3.693,1.073v4.048l2.856,0.595l-0.832,3.929l-2.623,4.29l-1.188,3.688h1.188l3.101-2.854l2.616,5.713l2.025-1.43h2.26l1.666,5.717l-1.666,2.383l0.24,2.5l-5.119,6.547l0.355,6.905l-6.668,5.12l-0.476,4.285l-3.809,1.787l0.354,2.5l-3.694,0.831l-2.853,4.646l-7.502,1.071l-3.93-2.856l-4.049-1.669l-4.049,4.05l-4.158,0.241l-4.058,0.234c0,0-10.996-6.139-9.168-5.119c1.828,1.021-1.666-4.526-1.666-4.526l2.261-4.646l-2.617-1.902h-3.455l-0.832-0.952l-3.691,0.952l-3.692-2.62l1.432-4.048l2.855-0.235l1.069-0.834l0.957-3.69l-1.192-2.024l-9.765-1.785l-1.668-2.856h-2.854h-2.619l-1.906-3.099L305.646,387.87z").attr(attr)
            };
            china['海南'] = {
                name: "海南",
                code: '460000',
                path: R.path("M385.895,447.523l-5.119,8.93v3.929l-10.238,8.336l-10.598-3.689l-2.025-7.501l0.597-3.454c0,0,8.074-8.075,5.715-5.716c-2.357,2.358,2.025-1.665,2.025-1.665l9.403-1.668l4.289-0.358l1.426-1.666l3.103,0.832L385.895,447.523z").attr(attr)
            };
            china['云南'] = {
                name: "云南",
                code: '530000',
                path: R.path("M313.622,349.296l-0.836-2.858l-2.618-0.954l-4.283,2.622l-2.859-1.432l-0.238-3.688l-1.43-1.79v-1.903l-4.049-0.952l-1.07,1.188l0.599,2.859l-3.217,1.43l-0.834,2.021l0.834,2.5l-7.146,8.336l1.191,9.766l-3.453,2.854l-1.666-1.784l-6.907,4.048l-2.618-1.431c0,0-11.535-22.106-10.002-19.172c1.533,2.938-3.931-2.854-3.931-2.854l-3.213-0.834l-1.432-2.619l1.787-2.859l-2.857-2.263l-3.455,2.859l-3.451,0.598l-2.263-10.363l-0.596-2.499l-3.454,4.286l-1.427,0.831l0.356,6.551l-0.834,1.431h-1.784l-1.43-1.431l-1.667,2.026l1.667,7.381h2.024l1.667,1.19c0,0,0.468,2.396,0.594,4.521c0.125,2.125-0.833,15.719-0.833,15.719l-10.837,9.766l-0.594,3.689l-2.024,1.787l-0.238,1.903l1.669,4.643l-1.431,3.454l0.834,0.478l5.478-1.433l7.738-0.476l-0.834,3.098l1.431,2.857l0.475,4.287l0.955,1.426l4.524,0.243l2.024,1.425l-2.264,2.856l-0.356,3.453l-1.667,4.05l1.068,1.43l2.86,0.238l4.881,1.784l-0.593,1.667l3.212,4.881h4.524l5.716-3.213l1.786,0.952v1.906l0.832,2.856l1.432,1.429l3.092-0.477l1.192,0.834l1.072-1.192v-4.525l-1.906-8.333l1.431-2.5h4.762h1.192l2.62-3.213l6.783,2.261l2.858-2.503l1.431,1.432l2.858-1.786l2.615,2.618h1.073l0.953-1.428l2.261-2.263l1.073,0.833l3.213-0.595l3.099-2.502l2.619-3.811l3.688-0.831l2.023,2.021l1.43-4.048l2.857-0.235l1.069-0.834l0.957-3.691l-1.192-2.023l-9.765-1.785l-1.668-2.857h-2.854h-2.619l-1.906-3.097l0.24-1.665l1.188-6.907l-3.809-3.689l2.859-12.027l-1.908-1.667l-6.309,1.906l-1.431-2.501v-2.856l-1.428-1.43l3.094-4.286l3.453,1.07l1.43-1.429l6.313,0.951l2.856-0.951L313.622,349.296z").attr(attr)
            };
            china['福建'] = {
                name: "福建",
                code: '350000',
                path: R.path("M435.945,374.779c0,0,2.881-12.508,1.742-10.365c-1.137,2.144,1.672-2.62,1.672-2.62l0.83-2.854l2.023-4.286l-1.072-1.668l0.24-3.691l4.881-5.475l-0.357-3.691l3.215-5.478l3.095,0.834l6.311-4.524l1.193-2.26l3.69,0.593l2.025,5.118l1.666,3.454h4.047l2.498-3.215l3.693,3.69l6.454-2.276l-4.069,9.776l-2.385-1.192l-1.666,0.835l-0.238,0.954l2.262,2.501l-0.598,8.929l0.598,2.859l-0.598,0.834l-2.617-0.599l-1.668,1.667l1.191,2.384l-3.214,3.095l0.595,1.189l-3.097,1.665l0.478,2.264l-1.072,1.191h-4.285l-2.264,2.025l-0.357,0.832l2.023,1.428l-2.26,3.452l-2.859,3.691l-1.189-0.356l-3.1,3.216l-3.447-7.502l-3.098-3.929l-2.023,0.24l-1.43-1.072L435.945,374.779z").attr(attr)
            };
            china['江西'] = {
                name: "江西",
                code: '360000',
                path: R.path("M408.279,325.242l3.336,6.549l0.355,3.452l-1.785,3.454l-2.502,2.024l-0.594,6.188l0.832,0.955l1.43-0.596l0.834,0.596v3.927l1.785,4.647l2.026,0.831l0.234,6.789l-1.193,4.642l1.193,2.021l2.266,2.265l5.474-1.429l1.071,2.022l-1.665,2.263l-2.859,4.287v1.07l1.67,1.192l11.43-4.525l4.047,2.263l1.191-1.19l-1.191-2.499l0.281-1.663l1.742-10.365l1.67-2.62l0.832-2.854l2.025-4.286l-1.072-1.668l0.238-3.691l4.881-5.475l-0.355-3.691l3.213-5.478l3.095,0.834l6.311-4.524l1.193-2.263l-4.049-7.501l-2.619-3.69l1.901-1.818l-2.381-2.111H446.5l-2.5-2.855l-4.884,4.762l-1.784-0.835l2.024-3.69l-0.24-1.188l-1.784-0.479l-5.716,3.099l-4.524,1.429l-3.689-0.238c0,0-9.709,9.669-7.979,7.381c1.731-2.287-4.287,1.433-4.287,1.433L408.279,325.242z").attr(attr)
            };
            china['湖南'] = {
                name: "湖南",
                code: '430000',
                path: R.path("M408.279,325.242l-2.619-0.594l-1.188-1.667v-4.05l-1.43-2.026l-2.857,2.86l-1.43,2.023h-1.904l-0.594-3.096l-2.621,0.833l-3.451,2.263l-2.264-1.428c0,0-2.666-2.521-4.525-3.097c-1.857-0.576-13.217-0.832-13.217-0.832l-1.43,1.429l2.62,3.093l-3.096,2.026l-4.883-2.026l-5.117,4.287l-1.787,4.05c0,0-0.19,6.479,0.357,9.405c0.551,2.926,2.5,10.002,2.5,10.002l-5.717,6.31l0.596,0.833l5.121-1.428l1.785,2.263l-1.429,7.144l2.857,5.121l2.856,0.832l2.857-3.931l1.43,3.099l0.834-0.238l4.523-4.288l1.191,0.24l2.021-1.073l3.693,1.073v4.048l2.856,0.595l-0.832,3.929l-2.623,4.29l-1.188,3.688h1.188l3.103-2.854l2.617,5.713l2.021-1.43h2.264l2.263-1.666v-3.45l1.188-1.072l2.859,0.235l4.879,2.264l0.834-1.667l-1.188-2.023l0.354-1.429l3.098-2.619l5.478,1.428l3.096-1.906l-1.193-2.021l1.193-4.642l-0.234-6.789l-2.026-0.831l-1.785-4.647v-3.927l-0.834-0.596l-1.43,0.596l-0.832-0.955l0.594-6.188l2.502-2.024l1.785-3.454l-0.355-3.452L408.279,325.242z").attr(attr)
            };
            china['贵州'] = {
                name: "贵州",
                code: '520000',
                path: R.path("M313.622,349.296l-1.666,4.761l-2.856,0.951l-6.313-0.951l-1.43,1.429l-3.453-1.07l-3.094,4.286l1.428,1.43v2.856l1.431,2.501l6.309-1.906l1.908,1.667l-2.859,12.027l3.81,3.689l-1.188,6.907l3.688,0.238l4.051-3.695l1.668,1.073l7.737,3.216l1.429-0.832l0.235-2.622l1.433-1.429l10.955-7.144l1.903,2.265l6.668,2.021l3.689-4.883l1.668,1.192h2.619v-1.432l3.453-1.189v-1.071l1.43-1.428l0.832,0.237l3.453-3.452l-2.857-5.123l1.429-7.142l-1.785-2.263l-5.121,1.428l-0.596-0.834l5.717-6.309l-2.5-10.001l-3.81,2.855l-3.334-3.45l-3.215-4.29l-0.24-2.619l-2.26-0.477l-2.621,0.834l-4.287-2.024l-2.022,4.883l-3.691,0.239l-2.619,4.049l-1.666-0.834l-2.623,0.834l-3.928-2.266l-3.213,3.69v1.668l6.312,3.453l1.426,2.621c0,0-6.094,2.245-4.523,1.667C322.1,348.955,313.622,349.296,313.622,349.296z").attr(attr)
            };
            china['浙江'] = {
                name: "浙江",
                code: '330000',
                path: R.path("M483.793,336.063l-6.455,2.276l-3.693-3.69l-2.498,3.215h-4.049l-1.666-3.454l-2.023-5.118l-3.692-0.596l-4.047-7.501l-2.619-3.69l1.903-1.818l0.716-0.685c0,0,6.241-8.84,4.286-6.07c-1.954,2.769-0.239-4.882-0.239-4.882l1.43-1.906l3.336-0.594l0.951-1.43l-1.189-2.263l1.666-2.382v-4.524l2.384-1.189l3.928,2.382l4.879,0.835l3.225-3.824l3.998,3.332l-1.744,1.324l-2.021,3.096l-3.217,0.952l-1.074,0.833l3.098,1.666l5.715-2.499l9.406,3.929l0.953,7.979h-3.809l-0.24,2.382l2.024,3.332l-1.784,2.024l2.022,3.214l-3.096,3.69l-1.43-1.787l-4.644,11.788L483.793,336.063z").attr(attr)
            };
            china['安徽'] = {
                name: "安徽",
                code: '340000',
                path: R.path("M423.637,253.208l2.024-0.597l6.312,2.621c0,0,1.768,2.299,2.861,2.856c1.092,0.557,5.118,1.43,5.118,1.43l2.021,1.667l2.858-0.479l0.599,1.072l-0.957,6.074l2.858,1.666l1.189,2.263l4.287,0.831l1.666-2.854l2.621,0.356l1.666,2.265l-0.24,2.021l-5.121,0.596v2.264v1.665l-2.021,2.265l1.43,2.618l4.287,2.856l0.237,3.454l8.572,0.832v4.525l-1.666,2.381l1.191,2.263l-0.951,1.43l-3.338,0.594l-1.428,1.906l0.235,4.882l-4.285,6.072l-0.715,0.683l-2.381-2.111H446.5l-2.498-2.855l-4.884,4.761l-1.786-0.834l2.026-3.69l-0.24-1.188l-1.786-0.479l-5.718,3.099l-5.356-10.241l1.666-3.214l-0.595-1.071l-2.858-0.952l-4.76-2.858l1.903-3.928l2.855-1.43l0.598-3.214l-1.07-5.716l-0.596-0.479l-2.856,2.86c0,0-5.978-4.647-4.287-3.453c1.69,1.192-3.217-4.049-3.217-4.049l3.217-2.261l0.834-4.05l2.021-1.431l-0.354-5.359l1.43-1.188l2.619,1.787l1.664,2.26l3.453-2.26l1.192-1.434l-0.598-2.617l-4.049-2.263L423.637,253.208z").attr(attr)
            };
            china['上海'] = {
                name: "上海",
                code: '310000',
                path: R.path("M484.32,292.485l-3.998-3.332c0,0,0.867-0.375,1.898-2.251c1.031-1.875,1.426-4.286,1.426-4.286l4.287,1.788l2.027,2.854l-1.433,2.024L484.32,292.485z").attr(attr)
            };
            china['江苏'] = {
                name: "江苏",
                code: '320000',
                path: R.path("M483.646,282.616l-1.426,4.286l-1.898,2.251l-3.225,3.824l-4.879-0.835l-3.929-2.383l-2.383,1.19l-8.571-0.832l-0.238-3.454l-4.287-2.856l-1.428-2.618l2.02-2.264v-1.665v-2.267l5.121-0.594l0.24-2.023l-1.666-2.263l-2.621-0.355l-1.666,2.854l-4.287-0.831l-1.188-2.263l-2.857-1.666l0.955-6.074l-0.598-1.072l-2.859,0.479l-2.021-1.667l-5.118-1.429l-2.861-2.857l-6.307-2.621l0.592-2.856l2.5-1.069l4.645,3.927h1.433l4.284-0.476l2.5-2.022l3.453,2.856l1.427-2.62l0.358-1.43l2.857-1.667l0.834-3.45l2.854-0.597l7.148,4.881c0,0,3.365,0.754,5.117,2.025c1.754,1.271,9.766,16.313,9.766,16.313l-0.357,1.666l6.548,3.095l1.784,2.859l3.099,1.429l1.428,2.855l-2.023,0.951l-3.334-1.188h-4.645l-4.287-1.432l-1.666,1.432l3.932,1.188l3.813,1.669L483.646,282.616z").attr(attr)
            };
            china['湖北'] = {
                name: "湖北",
                code: '420000',
                path: R.path("M356.486,329.29l1.787-4.048l5.119-4.287l4.881,2.026l3.096-2.026l-2.621-3.093l1.429-1.433l13.219,0.836l4.525,3.097l2.264,1.425l3.451-2.26l2.619-0.833l0.596,3.093h1.904l1.43-2.021l2.856-2.86l1.431,2.027v4.049l1.19,1.667l2.619,0.594l2.855-2.854l4.287-1.433l7.979-7.381l3.691,0.236l4.522-1.428l-5.358-10.24l1.668-3.214l-0.593-1.071l-2.862-0.952l-4.761-2.858l-2.381-1.427l-3.098,1.427l-3.451-2.854h-4.524l-3.455-1.907l-0.83-2.856l-1.787-1.786l-2.5,1.428L395.421,285c0,0-9.509,0.927-7.146,1.071c2.363,0.146-7.736-1.666-7.736-1.666l-9.407-8.334l-2.619,2.023l-1.188-0.831h-1.907h-8.572l-1.189,1.068l2.619,2.62l2.5,0.593l2.26,0.837l-1.067,1.668l-4.287,2.021l-0.834,3.451l0.834,1.191l0.596,4.288l2.5,0.238l2.619,3.452l1.07,6.548l-0.832,2.262l-1.666-0.594l-4.883,3.812l-8.336,1.429l-2.261,2.263l1.783,2.262l0.24,4.882l2.262,0.598L356.486,329.29z").attr(attr)
            };
            china['西藏'] = {
                name: "西藏",
                code: '540000',
                path: R.path("M152.525,339.529l6.787,0.834l2.265,3.216l1.189,0.477l10.239-1.904l0.594-1.787l2.023-1.07l4.884-4.05l4.285-0.594l3.93-2.501l7.74-4.286l0.832,1.428l5.716,1.904l8.334-4.285l2.618,1.787l-2.379,3.452l0.952,0.833h3.332l0.359,1.427l-2.857,5.121l0.833,0.834h1.666l8.336,2.265l3.691-3.099l5.478,4.289l1.667-2.026l1.43,1.431h1.784l0.834-1.431l-0.356-6.549l1.429-0.835l3.451-4.284l-0.594-16.549l-3.099-8.337l2.265-1.665l-6.548-12.027l-7.979-6.311l-2.26,1.43l0.238,3.692l-6.908,8.931l-11.428-2.384l-0.833-4.286l-7.146-5.716l-15.717-2.498l-6.787-1.188l-2.619-0.241l-5.952-4.883l-12.385-2.857l-8.57-16.551l-0.238-4.642l3.451-1.672v-5.117l2.502-6.311l-2.858-2.858l3.811-3.095l-2.383-1.425h-5.716l-10.238-4.644h-6.549l-5.118,1.43h-5.715l-8.931,4.879l-7.144-0.834l-7.146,2.5l-5.954-1.906l-3.688-3.212l-9.526-1.427l-6.192,4.28l-3.45-1.425l-2.859-2.263l-6.908-1.667l-1.065-1.189l-2.857-0.237l-9.782,5.945l-10.412-1.297l2.428,4.398l2.68,1.995l-0.821,3.842l-0.231,3.758l0.256,2.672l0,0l-0.193,3.208l3.451,3.454l0.239,4.884l-1.072,1.907l-5.119,0.591l-2.621-2.854l-2.619,0.355l-0.476,2.265l1.428,3.688l0.479,2.62v3.333l-0.833,2.381l0.354,1.909l3.336,0.356l1.785,3.094l7.739,6.071v1.906l5.716,6.311l1.902,2.262l1.79,0.595l3.451-3.451l3.096,2.856c0,0,15.395,13.684,13.098,11.193c-2.297-2.491,2.381,5.715,2.381,5.715h2.859l1.667-1.665l1.426,1.428v5.117l7.741,4.287l1.667-0.357l1.191,4.287l6.548,3.81l0.238,2.501l1.188,0.835l5.717-0.24h3.098l4.644,3.457l10.24-0.359l5.476-0.239l1.428,2.265l-1.188,4.883l1.427,1.664l5.715-4.761l7.146-5.239l5.12,0.95L152.525,339.529z").attr(attr)
            };
            china['青海'] = {
                name: "青海",
                code: '630000',
                path: R.path("M153.954,234.989l4.523,1.667l7.742-2.617l-1.193-1.429h-2.021l-0.834-2.259l0.594-2.024l3.692-1.668l2.617-4.286l-7.977-6.548l-0.356-6.906c0,0,2.08-2.545,3.689-2.856c1.609-0.313,26.317-5.119,26.317-5.119l1.783-1.428l3.931,0.594l13.215,3.095l3.93,2.265l5.714,3.451l3.692-0.237l5.714-3.452l7.501,1.188l4.882-2.616l9.999,6.309l5.36-2.023l2.856,6.903l6.907,4.286l5.119,4.527l3.691-1.431l7.146,9.405l1.426,6.071l2.619,3.688l-0.832,7.146l-5.715,4.524l1.073,2.856l-2.265,2.263l-2.856,2.62v4.879l-2.621,2.027l-3.097-0.954l-2.617-2.5l-1.431,2.859l1.787,2.499l4.524,0.594l4.287,4.288h2.618l2.024,2.265l-1.787,4.046v0.239l-3.688-0.479l-2.621,1.905l-1.666-2.856l-4.524,1.785l-1.19,2.264v3.451l-4.524,0.836l-5.715-5.716l-1.787-2.856l-4.524,1.428l-7.738-2.262l-11.67,1.666l-0.952,1.189l-1.071,3.69l-2.263,1.43l0.238,3.692l-6.906,8.931l-11.43-2.383l-0.833-4.285l-7.146-5.718l-15.717-2.498l-6.786-1.19l-2.621-0.238l-5.953-4.883l-12.384-2.857l-8.57-16.551l-0.238-4.642l3.451-1.672v-5.117l2.502-6.313l-2.858-2.856L153.954,234.989z").attr(attr)
            };
            china['甘肃'] = {
                name: "甘肃",
                code: '620000',
                path: R.path("M196.462,200.108l-1.43-16.55l0.836-3.453l4.879-2.262c0,0,5.209-5.03,6.903-5.717c1.696-0.686,10.6-4.285,10.6-4.285l4.285-2.025v-4.047l1.905-2.262l1.788,0.237l7.144,1.192l-0.358,3.095l1.43,4.88l-0.834,7.978l6.072,8.929l3.097,2.026l4.883-3.812h10.237l2.623,0.953l1.429,2.264l-1.193,2.499l-5.714,4.645l0.597,2.261l6.549,4.882h2.618l0.834,1.07l-0.596,2.025l4.05,3.217l9.404,1.429l4.525-1.19l5.718-5.719l6.903,0.598l2.855,4.287l-1.664,3.929l0.475,2.382l-3.688,2.263l-1.668,2.024l0.596,4.763l6.545,4.646l2.875-0.653l7.725,9.458l1.668,7.145l-0.834,3.451l5.357,2.859v2.26l4.883,1.192h1.426v-4.05l3.693-0.595l0.951-4.763l-2.619-2.026l-2.025-2.021l0.834-9.166l2.023-1.071l3.453,1.43l1.432-0.598l0.834,1.667l9.166,4.639l4.883,3.1l0.832,2.856l-2.619,3.691l1.431,4.285l-0.834,2.021l-6.785,0.6l-1.433,0.833l0.478,1.191v1.903l-5.355,0.594l-2.856-1.428h-3.691l-0.596,0.834l0.596,2.024l-1.789,2.023l-0.592,2.262l3.449,2.857l-2.498,5.119l1.072,2.622l-0.238,1.188h-4.051l-2.854,1.435l2.26,3.091l-1.428,4.051l-4.287,1.07l0.357,2.023l-1.189,1.428l-7.383-0.596l-2.854-2.021l-0.601-4.525l-1.664-1.669l-2.62,1.669l-4.523-4.521l-3.455-2.385l-0.354-3.335l-1.074-2.619h-1.787l-7.144,3.096l0.356,3.454h-2.618l-4.287-4.288l-4.525-0.593l-1.786-2.499l1.431-2.86l2.617,2.5l3.097,0.954l2.62-2.025v-4.881l2.857-2.62l2.265-2.263l-1.073-2.856l5.716-4.522l0.832-7.147l-2.619-3.688l-1.429-6.071l-7.144-9.405l-3.692,1.431l-5.118-4.524l-6.907-4.287l-2.856-6.905l-5.359,2.023l-9.999-6.309l-4.883,2.616l-7.501-1.188l-5.714,3.452l-3.691,0.237l-5.715-3.451l-3.93-2.265L196.462,200.108z").attr(attr)
            };
            china['新疆'] = {
                name: "新疆",
                code: '650000',
                path:R.path("M153.889,69.508l2.327-0.014l-1.428,4.525l2.025,2.38l0.236,1.666l4.525,4.524l1.191,3.453l5.953,0.357l2.62,2.265h1.429l3.453,7.379l3.451,8.931l-1.784,5.357l0.358,2.025l-3.215,5.477l0.833,4.286l11.192,4.763l12.025,1.788l12.503,8.571l4.049,1.429l0.237,2.261l2.619,5.478l2.619,7.146l3.333,5.953l-1.903,2.263v4.047l-4.286,2.026l-10.596,4.284l-6.907,5.719l-4.881,2.26l-0.834,3.453l1.43,16.55l-3.931-0.596l-1.783,1.431l-26.315,5.119l-3.691,2.856l0.358,6.906l7.978,6.548l-2.62,4.286l-3.69,1.668l-0.597,2.024l0.835,2.259h2.021l1.192,1.429l-7.738,2.617l-4.525-1.667l-2.382-1.424h-5.715l-10.24-4.642h-6.546l-5.121,1.428h-5.714l-8.931,4.879l-7.143-0.834l-7.145,2.5l-5.956-1.907l-3.689-3.211l-9.525-1.427l-6.191,4.28l-3.452-1.425l-2.858-2.263l-6.907-1.667l-1.07-1.189l-2.857-0.237l-9.782,5.945l-10.037-1.25l-0.822-0.359l1.113-8.623l-4.524-1.191l-9.406-6.902l-2.62-0.241l-2.023-4.525l1.427-4.643l-0.477-2.26l-3.451-2.266l-1.192-2.26l-7.143-4.049v-1.189l3.452-1.431l2.023,1.19l2.025-2.025l-0.598-6.785l0.598-5.716l-4.646-4.642l-3.095,0.833l-1.189-3.336l1.785-3.452l-0.952-3.214l3.038-2.749l1.248-1.182v-2.618l4.285-2.024l4.286-0.833l3.811-1.429l3.099,0.832l2.26-0.832l0.833,0.595l0.356,2.859l2.022,0.832l4.765-0.238c0,0,3.566-4.729,5.478-6.073c1.911-1.346,11.19,2.381,11.19,2.381l5.717-4.048l16.552-3.689l1.069-2.264l1.43-6.31l4.643-3.69h1.433v-1.906l0.236-15.836l0.833-3.212l-4.521-1.668l-0.24-1.191l4.762-1.428l12.384-1.073l1.905,2.501l4.287,0.952l1.192,0.239l1.665-2.262l-2.265-2.263l9.169-18.574l1.431-0.953l8.335,4.047h3.689l1.667,2.264l7.979-2.502l2.023-13.212l3.452-2.265l4.048-0.238l2.859-3.452l1.071-3.454l2.263-1.191L153.889,69.508z").attr(attr)
            };
            china['陕西'] = {
                name: "陕西",
                code: '610000',
                path: R.path("M363.393,259.519l-1.428-3.454l2.498-8.81l-3.689-16.903c0,0,3.262-5.777,2.619-4.286c-0.646,1.49,1.07-3.929,1.07-3.929l-3.334-6.905l3.096-4.884l0.594-4.525l1.67-3.452l-0.238-4.046l-1.432-1.074l-2.26,2.502l-6.072,0.356l-3.096,4.526l0.236,2.62l-0.593,1.665l-2.502,0.834l-9.406,13.454l-1.19-0.836l-2.617-0.594l-5.359,0.359l-1.789,2.499l-0.233,5.478l0.834,1.667l9.166,4.637l4.883,3.099l0.832,2.855l-2.619,3.691l1.431,4.286l-0.835,2.021l-6.784,0.6l-1.433,0.835l0.478,1.188v1.904l-5.355,0.596l-2.856-1.428h-3.691l-0.596,0.832l0.596,2.026l-1.789,2.022l-0.592,2.261l3.451,2.861l-2.5,5.117l1.071,2.619l-0.237,1.191h-4.052l-2.854,1.429l2.262,3.098l-1.43,4.048l2.383,0.478l0.238,2.617l2.021,0.237l7.386-1.427l1.43,0.593l0.354,2.025l3.099,0.835l6.547,3.216l3.69-1.434l8.097,2.857l1.666,2.26l4.051-0.829l-0.596-4.288l-0.834-1.191l0.834-3.451l4.286-2.021l1.068-1.668l-2.26-0.837l-2.5-0.593l-2.619-2.62l1.189-1.068h8.571h1.908l1.189,0.83l2.616-2.022v-3.454l-7.737-11.668L363.393,259.519L363.393,259.519z").attr(attr)
            };
            china['河南'] = {
                name: "河南",
                code: '410000',
                path: R.path("M371.131,276.068l9.405,8.336l7.742,1.665l7.144-1.072l2.262,1.072l2.5-1.43l1.783,1.787l0.834,2.856l3.455,1.905h4.524l3.451,2.857l3.098-1.428l2.382,1.428l1.903-3.929l2.855-1.43l0.598-3.216l-1.07-5.715l-0.596-0.479l-2.856,2.86l-4.287-3.453l-3.218-4.049l3.218-2.263l0.834-4.048l2.021-1.431l-0.354-5.359l1.43-1.188l2.619,1.787l1.664,2.26l3.455-2.26l1.19-1.434l-0.598-2.617l-4.049-2.263l-0.834-2.619l-7.142,0.835l-4.524-3.93l-2.021-0.596v-2.621l10-11.074l-3.69,0.834l-2.261,1.669l-0.957-1.429v-1.666l-1.663-0.6l-4.525,1.785l-11.43-1.428l-0.597,9.408c0,0-6.604,5.169-5.479,4.287c1.129-0.884-7.381,1.429-7.381,1.429l-10.359,7.142l-8.215,2.264v1.43l7.738,11.666L371.131,276.068L371.131,276.068z").attr(attr)
            };
            china['山西'] = {
                name: "山西",
                code: '140000',
                path: R.path("M363.393,259.519l8.217-2.265l10.357-7.142l7.381-1.431l5.477-4.287l0.599-9.405l-2.623-3.449l0.836-2.856l1.787-2.619l0.477-3.929l1.785-2.86l-2.859-5.713l-4.047-4.645l2.262-7.383l4.886-3.212l0.83-4.765l-1.904-4.286v-5.242l-3.215-2.854l-7.381,3.809l-1.191-1.189l-3.93,2.855l-3.213-0.235l-6.312,9.048h-1.906l-3.452,2.858l0.237,4.046l-1.67,3.452l-0.594,4.525l-3.096,4.884l3.334,6.905l-1.07,3.929l-2.619,4.286c0,0,4.146,18.996,3.689,16.903s-2.498,8.81-2.498,8.81L363.393,259.519z").attr(attr)
            };
            china['山东'] = {
                name: "山东",
                code: '370000',
                path: R.path("M425.661,252.611l0.597-2.856l2.5-1.069l4.645,3.927h1.433l4.284-0.476l2.5-2.022l3.453,2.856l1.429-2.62l0.356-1.43l2.857-1.667l0.834-3.45l2.854-0.595l7.98-13.454l-1.429-2.264l1.429-1.427l1.666,0.595l2.619-1.429l1.432-3.094l6.545-6.073l5.121-1.666l2.381-2.266l-0.592-4.88l-3.457-0.355l-7.738,0.952l-5.356-2.62l-3.216,0.596l-7.977,10.239l-2.262,1.429l-5.117-2.263l-0.359-2.619l-1.069-4.523l-2.859-1.669l-4.643,1.073l-2.882-1.971l-4.266,5.423l-4.523,1.188c0,0-8.514,7.798-6.313,5.718c2.201-2.081-6.902,12.5-6.902,12.5l1.783,3.45l-0.357,1.431v1.666l0.957,1.429l2.261-1.669l3.69-0.834l-10.002,11.074v2.621l2.023,0.596l4.524,3.93l7.146-0.835L425.661,252.611z").attr(attr)
            };
            china['河北'] = {
                name: "河北",
                code: '130000',
                path: R.path("M413.04,235.229l0.357-1.426l-1.783-3.453l6.902-12.5c0,0,8.725-7.9,6.313-5.718c-2.411,2.185,4.523-1.188,4.523-1.188l4.268-5.423l-1.647-1.125l-1.56-3.907l-3.319,1.286l-5.479-1.428l-0.237-1.428l-0.238-9.17l3.69-1.667l-0.419-1.563l-0.177-0.104l0.81-3.557l-5.094,2.128l0.832,1.905l0.178,1.424l0.18,1.433l-2.857,1.19l-1.785,1.667l-3.692-1.071l-4.881,0.834l-0.832-2.264l0.594-4.287l3.693-4.286l0.831-4.88l3.691-3.691l6.666,3.454h1.668l1.189,4.762l1.905,0.95l0.953,3.1l-0.356,2.024l4.047,2.854l0.594,2.264l3.338,1.428l8.332-4.523v-2.621l4.883-7.143l-3.45-4.881l-2.621-0.238l-4.763-3.218l1.668-5.118l-7.387-0.595l-3.213-4.765l0.357-2.619l-6.31-6.906l-4.051,2.026l-3.451,3.452l1.191,2.62l-0.834,1.667l-4.882,0.237l-2.264,2.022l-2.022-0.835l-2.021,2.026l-4.527,3.453l-2.024-1.43v-4.644l-1.666-0.832l-2.619,1.189l-3.096,6.547l-1.189,6.311l3.689,6.19l3.215,2.858v5.24l1.904,4.286l-0.834,4.764l-4.884,3.213l-2.26,7.382l4.049,4.645l2.857,5.717l-1.785,2.857l-0.477,3.928l-1.787,2.619l-0.834,2.859l2.621,3.446l11.43,1.431l4.524-1.787L413.04,235.229z").attr(attr)
            };
            china['天津'] = {
                name: "天津",
                code: '120000',
                path: R.path("M430.413,200.491c0,0-1.832,1.672-3.319,1.284c-1.49-0.388-5.479-1.428-5.479-1.428l-0.237-1.429l-0.238-9.169l3.69-1.667l-0.596-1.666l0.81-3.557l0.385-1.683l1.901,0.95l0.953,3.098l-0.356,2.022l4.047,2.858l0.594,2.263l-2.379,1.668l-0.834,3.809L430.413,200.491z").attr(attr)
            };
            china['北京'] = {
                name: "北京",
                code: '110000',
                path: R.path("M421.139,189.75l-0.357-2.856l-0.832-1.905l5.095-2.126l0.381-1.683l-1.189-4.767h-1.668l-6.666-3.449l-3.69,3.69c0,0-1.125,6.585-0.832,4.88c0.289-1.704-3.693,4.288-3.693,4.288l-0.594,4.286l0.832,2.263l4.881-0.834l3.693,1.071l1.784-1.667L421.139,189.75z").attr(attr)
            };
            china['宁夏'] = {
                name: "宁夏",
                code: '640000',
                path: R.path("M329.934,230.111l0.24-5.476l1.785-2.499l-0.832-2.62l-9.168-3.454l0.594-3.691l2.858-4.049l-1.786-6.19l-0.835-0.952l-5.954,4.049l-2.855,9.168l-0.953,6.31l-4.762,3.93l-2.859,1.189l-3.438,0.779c0,0,9.184,11.236,7.729,9.458c-1.455-1.78,1.664,7.146,1.664,7.146l-0.834,3.449l5.357,2.859v2.262l4.881,1.19h1.43v-4.048l3.691-0.597l0.951-4.763l-2.619-2.026l-2.023-2.021l0.832-9.163l2.023-1.073l3.453,1.43L329.934,230.111z").attr(attr)
            };
            china['内蒙古'] = {
                name: "内蒙古",
                code: '150000',
                path: R.path("M301.969,226.604l3.438-0.779l2.859-1.188l4.762-3.932l0.953-6.31l2.855-9.168l5.954-4.048l0.835,0.951l1.786,6.19l-2.858,4.049l-0.594,3.691l9.168,3.453l0.832,2.621l5.358-0.359l2.617,0.596l1.191,0.834l9.405-13.454l2.502-0.835l0.593-1.664l-0.236-2.623l3.096-4.523l6.072-0.358l2.262-2.5l1.431,1.074l3.452-2.86h1.904l6.312-9.046l3.215,0.235l3.93-2.855l1.191,1.189l7.381-3.809l-3.689-6.193l1.189-6.311l3.096-6.549l2.619-1.188l1.666,0.832v4.644l2.025,1.43l4.526-3.453l2.021-2.026l2.022,0.835l2.265-2.025l4.881-0.234l0.834-1.667l-1.191-2.622l3.453-3.452l4.049-2.024l6.31,6.908l-0.355,2.618l3.213,4.763l7.385,0.595l1.666-3.453l-1.903-10.002l1.666-1.667l3.453,2.262l3.096,4.644l4.881-7.5l2.264-0.596l3.689-3.692h1.787l3.094-3.095h1.787l1.666-2.856h4.527l4.047-4.051l2.021-2.021v-2.264l-3.213-1.667v-4.286l-5.953-7.502l-5.717,5.478l-1.666-1.19l-0.357-3.096l-2.264-3.453l-0.594-3.931v-4.046l-3.928-2.858l-0.601-1.667l1.074-2.382l7.502,2.022l0.238-3.452l2.621-3.099l-1.789-1.666l0.358-3.214l3.689-1.667l0.834-1.19l-0.598-1.07l-3.927,1.07l-7.145-4.882l-0.357-1.43l3.453-4.524l7.146-10.597l0.594-1.072h1.193l2.26,2.858l0.596,0.479l0.238-13.336l2.26-1.191v-5.121l-0.594-8.333l2.858-11.668l-8.575-6.312l-6.548,5.478l-3.45,0.476l-1.668,1.786l-5.718-0.832l-2.854-3.215l-1.664-5.122l0.236-2.021l-4.525-2.858l-2.621,4.289l-4.881-3.096l-0.834-0.835l2.856-7.5L433.4,7.223h-2.024l-5.119,3.688l-4.285,6.311l1.668,1.071l3.211,0.359l2.504,6.548l-1.43,2.618l-2.502,3.689l-4.644,17.147l1.785,2.856l-1.428,2.498l-10.599,7.742l-5.713-1.071l-3.215-1.191l-0.479,1.667l-4.642,18.577l-2.264,2.378l1.191,3.335l2.854,2.382l4.764-2.623l7.74,0.598l2.26-3.692l4.052-0.951l7.737,2.856l9.408,9.765v2.023l-2.024,1.429l-10.836,0.599l-3.691,2.854l-2.857-0.355l-2.022,3.214l-5.121,1.07l-3.457,5.12l-0.592,3.81l-7.379,4.763l-4.646,0.598l-5.119,6.904l-4.883,2.859l-9.408-2.025l-3.092-1.431l-3.692,3.694l-1.785,6.548l5.119,7.501l-3.335,3.451l-4.643,2.859c0,0-8.422,10.638-6.787,8.571c1.637-2.064-6.619,3.36-8.93,3.93c-2.31,0.568-14.525,1.429-14.525,1.429l-2.264-0.237l-16.906,7.144l-7.742,4.881l-2.262-1.19l-0.83-2.262l-10.36-0.597l-11.909-3.688l-3.211-3.69l-17.385-2.025l-3.217,1.43l-21.072-2.022l-0.358,3.095l1.43,4.883l-0.834,7.976l6.073,8.929l3.096,2.026l4.883-3.812h10.237l2.623,0.953l1.426,2.262l-1.19,2.5l-5.714,4.646l0.597,2.261l6.549,4.88h2.618l0.834,1.072l-0.596,2.023l4.05,3.217l9.403,1.429l4.527-1.19l5.716-5.719l6.903,0.598l2.857,4.289l-1.664,3.926l0.473,2.382l-3.688,2.263l-1.668,2.024l0.596,4.76l6.545,4.647L301.969,226.604z").attr(attr)
            };
            china['辽宁'] = {
                name: "辽宁",
                code: '210000',
                path: R.path("M491.15,173.2l6.783-10.002l4.287-4.881l-0.595-4.763l-4.524-5.239l-0.594-4.286l-8.216-11.075l-0.358,1.074l-1.666,1.786l-3.453-4.05l-4.883-1.429l-0.236,1.429v2.264l-2.022,2.022l-4.047,4.05H467.1l-1.666,2.856h-1.789l-3.094,3.096h-1.787l-3.691,3.691l-2.262,0.596l-4.881,7.5l-3.096-4.644l-3.453-2.262l-1.666,1.667l1.903,10.002l-1.666,3.453l-1.668,5.12l4.763,3.215l2.621,0.238l3.45,4.881l2.5-1.429c0,0,2.857-2.881,4.05-4.882c1.192-2.002,4.049-6.788,4.049-6.788l6.787-1.429l4.287,4.286l-3.099,6.787l-4.049,6.311l3.688,2.62l-0.233,3.098l-2.857,2.855l0.597,1.19l4.881-2.619l7.143-9.407l10.836-6.072L491.15,173.2z").attr(attr)
            };
            china['吉林'] = {
                name: "吉林",
                code: '220000',
                path: R.path("M544.896,113.042l-2.07-0.088h-2.858l-4.285-1.431l-1.43-2.619l-1.431,2.024l-2.022-1.428l-4.523,5.12l-0.834,1.427l-2.022,0.357l-2.859-3.214l-2.621-1.072l-2.854-5.715l-2.027,1.431l0.594,5.12l-1.426,0.833l-3.689-4.05l-1.432-1.903h-2.854l-1.666-4.288l-3.813-2.023l-5.354,2.023l-2.025-0.357l-3.098-3.094l-4.285,2.5h-2.383l-2.857,1.191l-4.285-3.096l-2.854-5.24l-6.787,1.189l-2.621,3.099l-0.238,3.45l-7.502-2.023l-1.074,2.381l0.601,1.667l3.928,2.859v4.046l0.594,3.929l2.265,3.456l0.356,3.095l1.666,1.191l5.717-5.479l5.953,7.502v4.288l3.213,1.667l0.238-1.431l4.885,1.431l3.451,4.046l1.666-1.784l0.357-1.074l8.217,11.075l0.594,4.286l4.527,5.239l0.592,4.761l4.051-2.499l3.689-10.598l1.67-0.595l4.047,2.263l6.549-0.834l2.26-2.024l-3.092-4.763l0.832-1.191c0,0,7.84-2.611,6.072-2.022c-1.766,0.588,2.5-4.883,2.5-4.883l3.215-1.428l0.238-4.766l0.832-3.212l1.785-0.596l1.668,1.789l1.668,1.426l4.287-5.715l1.188-4.288L544.896,113.042z").attr(attr)
            };
            china['黑龙江'] = {
                name: "黑龙江",
                code: '230000',
                path: R.path("M464.838,96.639l6.787-1.19l2.854,5.241l4.285,3.095l2.856-1.188h2.386l4.285-2.501l3.094,3.094l2.024,0.357l5.357-2.023l3.813,2.023l1.666,4.288h2.857l1.43,1.904l3.689,4.049l1.426-0.833l-0.594-5.12l2.026-1.432l2.854,5.716l2.621,1.074l2.858,3.212l2.021-0.357l0.836-1.427l4.523-5.12l2.022,1.428l1.43-2.022l1.431,2.619l4.283,1.429h2.86l2.07,0.088l-1.238-2.113l-0.598-6.906l-5.115-7.978l2.855-2.857l2.616-4.883h9.646l1.785-1.665l-0.597-3.69l2.025-3.691l-0.596-2.024l0.832-3.451l-0.236-17.742l2.855-5.715l-3.214-3.692l0.595-2.261l-1.427-2.024l-3.69,1.429l-4.289,4.884l-4.283,2.023l-4.289,5.951l-10.598,3.692l-4.879-3.692l0.594-2.262l-2.5-3.689l-1.191-3.811l-4.047-0.239l-7.145-3.69l-2.859,1.071l-3.33-1.667l-4.887,0.834l-4.283-1.429l-2.621-3.69l-2.498-2.857l-0.951-2.857l-3.334-3.452l-2.026-3.099l-4.644-6.31l-1.428-3.69l-5.119-6.548l-1.432-3.454l-6.549-3.216l-4.287,1.429l-3.689-0.833l-8.336-1.668l-11.07,3.932l-2.024,1.786l2.262,3.096l-2.856,7.499l0.834,0.835l4.881,3.096l2.621-4.286l4.524,2.856l-0.235,2.022l1.664,5.119l2.854,3.218l5.717,0.833l1.668-1.787l3.451-0.477l6.547-5.476l8.576,6.31l-2.858,11.669l0.594,8.333v5.119l-2.26,1.191l-0.238,13.335l-0.597-0.476l-2.26-2.858h-1.192l-0.595,1.073c0,0-8.797,13.044-7.146,10.596c1.652-2.448-3.451,4.523-3.451,4.523l0.357,1.428l7.145,4.886l3.926-1.071l0.599,1.071l-0.834,1.189l-3.689,1.667l-0.359,3.214L464.838,96.639z").attr(attr)
            };
            china['重庆'] = {
                name: "重庆",
                code: '500000',
                path: R.path("M318.986,317.871l5.58-2.092l4.564,2.092l1.014-5.811l5.346-3.72l1.351-7.671l3.932-1.059v-7.47l3.689-1.432l8.096,2.857l1.666,2.26l4.051-0.829l2.5,0.238l2.619,3.452l1.07,6.548l-0.832,2.262l-1.666-0.594c0,0-3.072,3.501-4.884,3.812c-1.809,0.311-8.336,1.429-8.336,1.429l-2.26,2.263l1.783,2.262l0.24,4.883l2.262,0.597l5.715,7.142l0.357,9.407l-3.218,2.412l-0.592,0.445l-3.334-3.451l-3.215-4.29l-0.24-2.619l-2.26-0.476l-2.621,0.834l-4.287-2.025l-2.022,4.883l-3.691,0.239l-2.619,4.049l-1.666-0.834l-2.28-6.977l-6.507-4.07L318.986,317.871z").attr(attr)
            };
            china['四川'] = {
                name: "四川",
                code: '510000',
                path: R.path("M279.33,280.95l1.788-4.05l-2.025-2.261l-0.357-3.454l7.145-3.096h1.787l1.069,2.619l0.357,3.336l3.455,2.384l4.522,4.521l2.621-1.669l1.664,1.669l0.6,4.525l2.855,2.021l7.383,0.599l1.189-1.431l-0.357-2.023l4.287-1.07l2.381,0.478l0.238,2.617l2.024,0.237l7.382-1.427l1.43,0.593l0.354,2.026l3.099,0.834l6.549,3.214v7.47l-3.932,1.059l-1.351,7.671l-5.346,3.72l-1.014,5.811l-4.564-2.092l-5.58,2.092l-0.231,4.416l-0.464,4.53l6.507,4.07l2.28,6.977l-2.623,0.833l-3.928-2.263l-3.213,3.691v1.666l6.311,3.452l1.427,2.62l-4.523,1.666l-6.904-0.238l-0.831-2.855l-2.621-0.954l-4.285,2.623l-2.859-1.429l-0.238-3.694l-1.43-1.786v-1.903l-4.047-0.952l-1.072,1.188l0.599,2.858l-3.215,1.431l-0.836,2.021l0.836,2.504l-7.146,8.334l1.191,9.764l-3.453,2.855l-1.666-1.784l-6.906,4.049l-2.621-1.431l-10.001-19.17l-3.931-2.854l-3.216-0.835l-1.428-2.623l1.787-2.854l-2.857-2.264l-3.455,2.858l-3.45,0.595l-2.264-10.359l-0.597-2.499c0,0-2.04-22.568-0.594-16.55c1.446,6.017-3.099-8.336-3.099-8.336l2.267-1.665l-6.55-12.027l-7.977-6.311l1.069-3.692l0.953-1.188l11.669-1.666l7.738,2.262l4.523-1.431l1.785,2.859l5.717,5.715l4.524-0.833v-3.453l1.191-2.266l4.522-1.783l1.667,2.856l2.621-1.903l3.689,0.478v-0.241H279.33z").attr(attr)
            };
            return china;
        }

        var ChinaMap = function($container, options){
            var that = this;
            that.wrapperId = 'map-'+Date.now().toString().slice(-4)+Math.round(Math.random()*1000);

            $container.html('');
            $container.append('<div id="'+that.wrapperId+'" style="margin:0 auto;position:relative;width:'+options.width+'px;height:'+options.height+'px;"><img style="position:absolute;left:0;bottom:0;width:16%;" src="'+now.cfg.nowjs+'img/south_islands.jpg" /></div>');
            Raphael.getColor.reset();
            var china;
            var province;

            that.R = Raphael(that.wrapperId, options.width, options.height);
            that.current = null;
            that.china = china = paintMap(that.R);

            for (var state in china) {
                if(!china.hasOwnProperty(state)){ return; }
                // 省份着色，预备
                province = china[state];
                province['path'].color = Raphael.getColor(1);
                province['path'].transform("t30,0");
                // 默认填充色
                if(options.defaultColor){
                    china[state]['path'].animate({fill: china[state]['path'].color, stroke: "#eee" }, 500);
                }
                // 闭包环境
                (function (state, province) {
                    var path = province['path'];
                    var name = province['name'];
                    // 获取当前图形的中心坐标
                    var xx = path.getBBox().x + (path.getBBox().width / 2);
                    var yy = path.getBBox().y + (path.getBBox().height / 2);
                    // 修改部分地图文字偏移坐标
                    switch (name) {
                        case "江苏":
                            xx += 5;
                            yy -= 10;
                            break;
                        case "河北":
                            xx -= 10;
                            yy += 20;
                            break;
                        case "天津":
                            xx += 20;
                            yy += 10;
                            break;
                        case "上海":
                            xx += 20;
                            break;
                        case "广东":
                            yy -= 10;
                            break;
                        case "澳门":
                            yy += 10;
                            break;
                        case "香港":
                            xx += 20;
                            yy += 5;
                            break;
                        case "甘肃":
                            xx -= 40;
                            yy -= 30;
                            break;
                        case "陕西":
                            xx += 5;
                            yy += 20;
                            break;
                        case "内蒙古":
                            xx -= 15;
                            yy += 65;
                            break;
                        default:
                            break;
                    }
                    // 写入地名
                    var text = province['text'] = that.R.text(xx, yy, name).attr({
                        "fill": "#000",
                        "font-size": "13px",
                        "cursor": "pointer"
                    });
                    // 事件绑定

                    text[0].onclick = path[0].onclick = function(){
                        /*if (that.current == state)
                            return;*/
                        //重置上次点击的图形
                        if(that.current != state){
                            that.current && china[that.current]['path'].animate({
                                transform: "t30,0",
                                fill: china[that.current]['isClick'] ? china[that.current]['path'].color : "#b0d0ec",
                                stroke: "#ddd"
                            }, 2000, "elastic");
                        }

                        // 重置当前点击的省份
                        that.current = state;
                        // 当前点击的图形放大
                        path.animate({
                            transform: "t30,0 s1.03 1.03",
                            fill: path.color,
                            stroke: "#000"
                        }, 1200, "elastic");
                        // 当前省份图形推到最顶部
                        path.toFront();
                        // 当前省份对应的文本推到最顶部
                        text.toFront();
                        $container.trigger('selectProvince', province.code);
                    }
                })(state, province);
            }
        };

        $.fn.chinaMap = function(options){
            var args = Array.prototype.slice.call(arguments, 1);
            return this.each(function () {
                var $this = $(this);
                var data = $this.data('ChinaMap');
                var os = (typeof options == 'string');
                if(typeof data === 'undefined'){
                    options = $.extend({}, $.fn.chinaMap.defaults, typeof options == 'object' && options);
                    $this.data('ChinaMap', data = new ChinaMap($this, options));
                }else if(!os){
                    data.options = $.extend(data.options, options);
                }
                if(os){
                    data[options].apply(data, args);
                }
            });
        };

        $.fn.chinaMap.defaults = {
            width: 650,
            height: 500,
            defaultColor: false
        };
    }(jQuery);

    /**
     * 大数据导出
     * @author 陈梅妹
     */
    !function ($) {
        var ExportData = function (element, options) {
            this.options = options;
            this.$element = $(element);
            this._init();
        };

        ExportData.prototype = {
            constructor: ExportData, _init: function () {
                var self = this,
                    el = self.$element,
                    op = this.options;

                el.click(function(){
                    var para = op.paraFun(),
                        paths = [];
                    if(para===false){// 参数校验不通过
                        return;
                    }
                    now.cmd(op.cmdTtl, para, function(ret){
                        var total = parseInt(ret.ttl);
                        var file_name = ret.file_name===undefined ? '' : ret.file_name;
                        delete  para._c;

                        if(total <= 0){
                            $.alertMsg('提示', '没有可导出的数据');
                            return false;
                        }

                        $.alertMsg('提示', '正在生成数据：0%', true);
                        self._export(para, total, function(param, ret){
                            var hasNum = parseInt(param.start)+parseInt(param.size);

                            var percent = (Number(hasNum/total)*100).toFixed(2);
                            percent = percent>100 ? 100 : percent;
                            $('#_nui_alertmsg').find('span').html('正在生成数据：'+percent+'%');

                            var path = ret.path;
                            param.path = path;
                            if($.isArray(path)){
                                paths = $.unique($.merge(paths, path));
                            }else{
                                if($.inArray(path, paths) === -1){
                                    paths.push(path);
                                }
                            }

                            if(total <= hasNum){
                                setTimeout(function(){
                                    $('#_nui_alertmsg').find('.close').trigger('click');

                                    var downUrl = op.downFun(paths, file_name);
                                    window.location.href = downUrl;
                                    op.cbFun();
                                }, 1000);
                            }
                        })
                    });
                });
            }, _export: function (para, total, okFun) {
                var self = this,
                    op = self.options,
                    cmd = op.cmd,
                    size = op.size;

                //如果cmd为空，则返回false
                if (!cmd) {
                    return false;
                }

                //如果总条数为空，则返回false
                if (!total) {
                    return false;
                }

                if (!para) {
                    para = {};
                }

                var newData = function(okFun){
                    now.cmd(cmd, para, function(ret){
                        para.start = parseInt(para.start);
                        para.size = parseInt(para.size);

                        if($.isFunction(okFun)){
                            okFun(para, ret);
                        }

                        var start = para.start;
                        start += para.size;
                        if(start < total){
                            para.start = start;
                            newData(okFun);
                        }

                    });
                };

                para.start = 0;
                para.size = size;
                newData(okFun);
            }
        };

        $.fn.exportData = function (option, para, cb) {
            return this.each(function () {
                var $this = $(this),
                    os = (typeof option == 'string'),
                    data = $this.data('exportData');
                if (os) {
                    if ($.isFunction(para)) {
                        para(data[option]({}));
                    } else if ($.isFunction(cb)) {
                        cb(data[option](para));
                    } else {
                        data[option](para);
                    }
                } else {
                    if (!data) {
                        var options = $.extend({}, $.fn.exportData.defaults, typeof option == 'object' && option);
                        $this.data('exportData', (data = new ExportData(this, options)));
                    } else {
                        data.options = $.extend({}, data.options, option);
                        data._init();
                    }
                }
            });
        };

        $.fn.exportData.defaults = {
            cmd:'',
            cmdTtl:'',
            paraFun:function(){
                return {};
            },
            downFun: function(paths){
                return '';
            },
            cbFun:function(){
                return '';
            },
            size:1000
        };
    }(jQuery);
});